/* --- ABSOLUTE STANDALONE RELEASE --- */
const WORDS = {
  "7A": [
    {
      "w": "ring",
      "p": "/rɪŋ/",
      "t": "n.",
      "m": "环状物; 圈形的东西",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "collection",
      "p": "/kəˈlekʃn/",
      "t": "n.",
      "m": "(常指同类的) 收集物",
      "e": [],
      "u": "8",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "own",
      "p": "/əʊn/",
      "t": "v.",
      "m": "拥有; 有",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "valuable",
      "p": "/ˈvæljuəbl/",
      "t": "adj.",
      "m": "贵重的",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "handle",
      "p": "/ˈhændl/",
      "t": "v.",
      "m": "(用手) 接触",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "glove",
      "p": "/ɡlʌv/",
      "t": "n.",
      "m": "(分手指的) 手套",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "add",
      "p": "/æd/",
      "t": "v.",
      "m": "增加; 添加",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "envelope",
      "p": "/ˈenvələʊp/",
      "t": "n.",
      "m": "信封",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "absolutely",
      "p": "/ˈæbsəluːtli/",
      "t": "adv.",
      "m": "极其",
      "e": [],
      "u": "8",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "seem",
      "p": "/siːm/",
      "t": "v.",
      "m": "好像; 似乎",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "impossible",
      "p": "/ɪmˈpɒsəbl/",
      "t": "adj.",
      "m": "不可能的",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "single",
      "p": "/ˈsɪŋɡl/",
      "t": "adj.",
      "m": "单个的",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "nail",
      "p": "/neɪl/",
      "t": "n.",
      "m": "钉子",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "explain",
      "p": "/ɪkˈspleɪn/",
      "t": "v.",
      "m": "解释; 说明",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "bit",
      "p": "/bɪt/",
      "t": "n.",
      "m": "一部分",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "similar",
      "p": "/ˈsɪmələ(r)/",
      "t": "adj.",
      "m": "相似的; 类似的",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "coin",
      "p": "/kɔɪn/",
      "t": "n.",
      "m": "(一枚) 硬币",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "soft",
      "p": "/sɒft/",
      "t": "adj.",
      "m": "软的",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "sticker",
      "p": "/ˈstɪkə(r)/",
      "t": "n.",
      "m": "贴纸",
      "e": [],
      "u": "8",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "waste",
      "p": "/weɪst/",
      "t": "n.",
      "m": "白费; 浪费",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "item",
      "p": "/ˈaɪtəm/",
      "t": "n.",
      "m": "一件商品 (或物品)",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "result",
      "p": "/rɪˈzʌlt/",
      "t": "n.",
      "m": "后果; 结果",
      "e": [],
      "u": "8",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "wrapper",
      "p": "/ˈræpə(r)/",
      "t": "n.",
      "m": "包装纸",
      "e": [],
      "u": "8",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "bar",
      "p": "/bɑː(r)/",
      "t": "n.",
      "m": "(长方形) 条",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "include",
      "p": "/ɪnˈkluːd/",
      "t": "v.",
      "m": "包括; 包含",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "size",
      "p": "/saɪz/",
      "t": "n.",
      "m": "大小",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "pattern",
      "p": "/ˈpætn/",
      "t": "n.",
      "m": "图案; 花样",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "produce",
      "p": "/prəˈdjuːs/",
      "t": "v.",
      "m": "生产; 製造",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "attract",
      "p": "/əˈtrækt/",
      "t": "v.",
      "m": "吸引; 招引",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "insect",
      "p": "/ˈɪnsekt/",
      "t": "n.",
      "m": "昆虫",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "object",
      "p": "/ˈɒbdʒɪkt/",
      "t": "n.",
      "m": "物体; 物品",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "flat",
      "p": "/flæt/",
      "t": "adj.",
      "m": "平滑的",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "language",
      "p": "/ˈlæŋɡwɪdʒ/",
      "t": "n.",
      "m": "语言",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "treasure",
      "p": "/ˈtreʒə(r)/",
      "t": "n.",
      "m": "珍宝; 宝物",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "unlock",
      "p": "/ʌnˈlɒk/",
      "t": "v.",
      "m": "发现; 揭示",
      "e": [],
      "u": "8",
      "tags": [
        "pre_un"
      ]
    },
    {
      "w": "real",
      "p": "/ˈriːəl/",
      "t": "adj.",
      "m": "真的",
      "e": [],
      "u": "8",
      "tags": [
        "suf_al",
        "pre_re"
      ]
    },
    {
      "w": "add to...",
      "p": "",
      "t": "v. phr.",
      "m": "把......添加到...",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "soft drink",
      "p": "",
      "t": "n. phr.",
      "m": "饮料 (不含酒精)",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "a waste of time",
      "p": "",
      "t": "n. phr.",
      "m": "浪费时间",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "pocket money",
      "p": "",
      "t": "n. phr.",
      "m": "零花钱 (父母给孩子的)",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "all over the world",
      "p": "",
      "t": "phrase",
      "m": "全世界",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "literature",
      "p": "/'lɪtrətʃə(r)/",
      "t": "n.",
      "m": "文学; 文学作品",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "act",
      "p": "/ækt/",
      "t": "v.",
      "m": "扮演 (戏剧、电影中的角色)",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "appreciate",
      "p": "/ə'priːʃieɪt/",
      "t": "v.",
      "m": "欣赏; 赏识",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "join",
      "p": "/dʒɔɪn/",
      "t": "v.",
      "m": "成为......的一员; 参加",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "definitely",
      "p": "/'defɪnətli/",
      "t": "adv.",
      "m": "肯定; 没问题; 当然; 确实",
      "e": [],
      "u": "7",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "fair",
      "p": "/feə(r)/",
      "t": "n.",
      "m": "集市",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "rocket",
      "p": "/'rɒkɪt/",
      "t": "n.",
      "m": "火箭",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "launch",
      "p": "/lɔːntʃ/",
      "t": "v.",
      "m": "发射",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "shout",
      "p": "/ʃaʊt/",
      "t": "v.",
      "m": "大声; 叫喊",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "power",
      "p": "/'paʊə(r)/",
      "t": "n.",
      "m": "力量; 能量",
      "e": [],
      "u": "7",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "moment",
      "p": "/'məʊmənt/",
      "t": "n.",
      "m": "片刻; 瞬间",
      "e": [],
      "u": "7",
      "tags": [
        "suf_ment"
      ]
    },
    {
      "w": "member",
      "p": "/'membə(r)/",
      "t": "n.",
      "m": "成员",
      "e": [],
      "u": "7",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "shape",
      "p": "/ʃeɪp/",
      "t": "n.",
      "m": "形状; 外形",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "hike",
      "p": "/haɪk/",
      "t": "n.",
      "m": "远足; 徒步旅行",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "recently",
      "p": "/'riːsntli/",
      "t": "adv.",
      "m": "不久前; 最近",
      "e": [],
      "u": "7",
      "tags": [
        "suf_ly",
        "pre_re"
      ]
    },
    {
      "w": "attend",
      "p": "/ə'tend/",
      "t": "v.",
      "m": "出席; 参加",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "charity",
      "p": "/'tʃærəti/",
      "t": "n.",
      "m": "慈善机构 (或组织)",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "sale",
      "p": "/seɪl/",
      "t": "n.",
      "m": "出售; 销售",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "snack",
      "p": "/snæk/",
      "t": "n.",
      "m": "点心; 小吃",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "decorate",
      "p": "/'dekəreɪt/",
      "t": "v.",
      "m": "装饰; 装潢",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "difference",
      "p": "/'dɪfrəns/",
      "t": "n.",
      "m": "差别; 差异",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "compete",
      "p": "/kəm'piːt/",
      "t": "v.",
      "m": "参加比赛 (或竞赛)",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "event",
      "p": "/ɪ'vent/",
      "t": "n.",
      "m": "公开活动; 社交场合",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "spook",
      "p": "/spuːk/",
      "t": "v.",
      "m": "吓; 使惊恐",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "annual",
      "p": "/'ænjuəl/",
      "t": "adj.",
      "m": "每年的; 一年一度的",
      "e": [],
      "u": "7",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "fantastic",
      "p": "/fæn'tæstɪk/",
      "t": "adj.",
      "m": "极好的",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "dollar",
      "p": "/'dɒlə(r)/",
      "t": "n.",
      "m": "元 (美国、加拿大、澳大利亚等国的货币单位)",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "pie",
      "p": "/paɪ/",
      "t": "n.",
      "m": "馅饼; 派",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "equipment",
      "p": "/ɪ'kwɪpmənt/",
      "t": "n.",
      "m": "设备; 器材",
      "e": [],
      "u": "7",
      "tags": [
        "suf_ment"
      ]
    },
    {
      "w": "clubs fair",
      "p": "/'klʌbz feə(r)/",
      "t": "n. phr.",
      "m": "社团招募展",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "solar power",
      "p": "/'səʊlə ˈpaʊə(r)/",
      "t": "n. phr.",
      "m": "太阳能",
      "e": [],
      "u": "7",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "of course",
      "p": "/əv ˈkɔːs/",
      "t": "phr.",
      "m": "当然",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "remote control",
      "p": "/rɪ'məʊt kən'trəʊl/",
      "t": "n. phr.",
      "m": "遥控器",
      "e": [],
      "u": "7",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "turn around",
      "p": "/tɜːn ə'raʊnd/",
      "t": "v. phr.",
      "m": "转身",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "make a difference",
      "p": "/meɪk ə ˈdɪfrəns/",
      "t": "v. phr.",
      "m": "有作用; 有影响",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "primary school",
      "p": "/'praɪməri skuːl/",
      "t": "n. phr.",
      "m": "小学",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "fall off",
      "p": "/fɔːl ɒf/",
      "t": "v. phr.",
      "m": "掉落",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "guide",
      "p": "/ɡaɪd/",
      "t": "n.",
      "m": "指南；手册",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "east",
      "p": "/iːst/",
      "t": "n./adv.",
      "m": "东；东方 (adv: 朝东)",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "west",
      "p": "/west/",
      "t": "n./adv.",
      "m": "西；西方 (adv: 朝西)",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "mix",
      "p": "/mɪks/",
      "t": "v.",
      "m": "混合；结合",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "western",
      "p": "/'westən/",
      "t": "adj.",
      "m": "西方的；西方的国家",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "style",
      "p": "/staɪl/",
      "t": "n.",
      "m": "风格",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "popular",
      "p": "/'pɒpjələ(r)/",
      "t": "adj.",
      "m": "大众喜爱的；受欢迎的",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "site",
      "p": "/saɪt/",
      "t": "n.",
      "m": "(建筑物、城镇等的)地点",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "across",
      "p": "/ə'krɒs/",
      "t": "prep.",
      "m": "在......对面; 在......对过",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "adult",
      "p": "/'ædʌlt/",
      "t": "n.",
      "m": "成年人",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "senior",
      "p": "/'siːniə(r)/",
      "t": "n.",
      "m": "老年人；长者",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "customer",
      "p": "/'kʌstəmə(r)/",
      "t": "n.",
      "m": "顾客；主顾",
      "e": [],
      "u": "6",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "design",
      "p": "/dɪ'zaɪn/",
      "t": "v.",
      "m": "设计",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "else",
      "p": "/els/",
      "t": "adv.",
      "m": "其他的；别的",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "suggestion",
      "p": "/sə'dʒestʃən/",
      "t": "n.",
      "m": "建议；提议",
      "e": [],
      "u": "6",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "tip",
      "p": "/tɪp/",
      "t": "n.",
      "m": "点拨；实用的提示",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "historic",
      "p": "/hɪ'stɒrɪk/",
      "t": "adj.",
      "m": "历史著名的 (或重要的); 可名垂青史的",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "list",
      "p": "/lɪst/",
      "t": "n.",
      "m": "名单；目录",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "landscape",
      "p": "/'lændskeɪp/",
      "t": "n.",
      "m": "风景；景色",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "memory",
      "p": "/'meməri/",
      "t": "n.",
      "m": "记忆",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "general",
      "p": "/'dʒenərəl/",
      "t": "adj.",
      "m": "将军",
      "e": [],
      "u": "6",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "sight",
      "p": "/saɪt/",
      "t": "n.",
      "m": "看见",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "natural",
      "p": "/'nætʃrəl/",
      "t": "adj.",
      "m": "自然的；天然的",
      "e": [],
      "u": "6",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "almost",
      "p": "/'ɔːlməʊst/",
      "t": "adv.",
      "m": "几乎；差不多",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "become",
      "p": "/bɪ'kʌm/",
      "t": "v.",
      "m": "开始变得；变成",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "gateway",
      "p": "/'ɡeɪtweɪ/",
      "t": "n.",
      "m": "(通往其他地区的)门户",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "influence",
      "p": "/'ɪnfluəns/",
      "t": "v.",
      "m": "影响；对......起作用",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "artwork",
      "p": "/'ɑːtwɜːk/",
      "t": "n.",
      "m": "艺术作品",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "painting",
      "p": "/'peɪntɪŋ/",
      "t": "n.",
      "m": "绘画",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "scene",
      "p": "/siːn/",
      "t": "n.",
      "m": "场面；情景",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "wife",
      "p": "/waɪf/",
      "t": "n.",
      "m": "妻子",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "stone",
      "p": "/stəʊn/",
      "t": "n.",
      "m": "石头；石料",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "reflection",
      "p": "/rɪ'fle'(ʃ)n/",
      "t": "n.",
      "m": "映像；图像",
      "e": [],
      "u": "6",
      "tags": [
        "suf_tion",
        "pre_re"
      ]
    },
    {
      "w": "tourist spot",
      "p": "",
      "t": "n. phr.",
      "m": "旅游景点",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "light up",
      "p": "",
      "t": "v. phr.",
      "m": "照亮",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "in the centre",
      "p": "",
      "t": "phrase",
      "m": "在......的中心",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "be interested in",
      "p": "",
      "t": "v. phr.",
      "m": "对......有兴趣",
      "e": [],
      "u": "6",
      "tags": [
        "phr_be_in"
      ]
    },
    {
      "w": "historic site",
      "p": "",
      "t": "n. phr.",
      "m": "古迹",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "in memory of",
      "p": "",
      "t": "phrase",
      "m": "作为......的纪念",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "spacesuit",
      "p": "/spetsuːt/",
      "t": "n.",
      "m": "航天; 宇宙航行",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "collect",
      "p": "/kə'lekt/",
      "t": "v.",
      "m": "收集; 采集",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "leave",
      "p": "/liːv/",
      "t": "v.",
      "m": "离开",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "kilometre",
      "p": "/kɪlə'miːtə(r)/",
      "t": "n.",
      "m": "千米; 公里",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "weak",
      "p": "/wiːk/",
      "t": "adj.",
      "m": "虚弱的; 无力的",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "tie",
      "p": "/taɪ/",
      "t": "v.",
      "m": "(用线、绳等) 拴",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "low",
      "p": "/ləʊ/",
      "t": "adj.",
      "m": "低于(通常或平均)的数量(或水平)的",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "weight",
      "p": "/weɪ/",
      "t": "n.",
      "m": "有......重; 重",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "breathe",
      "p": "/briːð/",
      "t": "v.",
      "m": "呼吸",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "facility",
      "p": "/fə'sɪləti/",
      "t": "n.",
      "m": "设施; 设备",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "adventure",
      "p": "/əd'ventʃə(r)/",
      "t": "n.",
      "m": "冒险经历; 奇遇",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "pass",
      "p": "/pɑːs/",
      "t": "v.",
      "m": "向某方向前进; 向某方向移动",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "experiment",
      "p": "/ɪk'sperɪmənt/",
      "t": "n.",
      "m": "实验; 试验",
      "e": [],
      "u": "5",
      "tags": [
        "suf_ment"
      ]
    },
    {
      "w": "prepare",
      "p": "/pri'peə(r)/",
      "t": "v.",
      "m": "做好准备; 把......准备好",
      "e": [],
      "u": "5",
      "tags": [
        "pre_pre"
      ]
    },
    {
      "w": "dream",
      "p": "/driːm/",
      "t": "n./v.",
      "m": "梦想; 理想 / 做梦; 梦见",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "chance",
      "p": "/tʃɑːns/",
      "t": "n.",
      "m": "机会; 机遇",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "send",
      "p": "/send/",
      "t": "v.",
      "m": "邮寄; 发送",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "introduction",
      "p": "/ˌɪntrə'dʌkʃn/",
      "t": "n.",
      "m": "序言; 引言",
      "e": [],
      "u": "5",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "view",
      "p": "/vjuː/",
      "t": "n.",
      "m": "景色; 风景",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "someday",
      "p": "/'sʌmdeɪ/",
      "t": "adv.",
      "m": "总有一天",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "experience",
      "p": "/ɪk'spɪəriəns/",
      "t": "n.",
      "m": "经历",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "lander",
      "p": "/'lændə(r)/",
      "t": "n.",
      "m": "着陆器",
      "e": [],
      "u": "5",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "island",
      "p": "/'aɪlənd/",
      "t": "n.",
      "m": "岛",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "ancient",
      "p": "/'eɪnʃənt/",
      "t": "adj.",
      "m": "古老的; 很老的",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "poem",
      "p": "/'pəʊm/",
      "t": "n.",
      "m": "诗",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "express",
      "p": "/ɪk'spres/",
      "t": "v.",
      "m": "表示; 表达; 表露",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "determination",
      "p": "/dɪˌtɜːmɪ'neɪʃn/",
      "t": "n.",
      "m": "决心; 坚定",
      "e": [],
      "u": "5",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "universe",
      "p": "/juː'nɪvɜːs/",
      "t": "n.",
      "m": "宇宙",
      "e": [],
      "u": "5",
      "tags": [
        "pre_un"
      ]
    },
    {
      "w": "already",
      "p": "/ɔːl'redi/",
      "t": "adv.",
      "m": "已经; 完毕",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "century",
      "p": "/'sentʃəri/",
      "t": "n.",
      "m": "世纪",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "neighbour",
      "p": "/'neɪbə(r)/",
      "t": "n.",
      "m": "邻居",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "circle",
      "p": "/'sɜːkl/",
      "t": "v.",
      "m": "环行",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "surface",
      "p": "/'sɜːfɪs/",
      "t": "n.",
      "m": "表面; 层面",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "release",
      "p": "/ri'liːs/",
      "t": "v.",
      "m": "放; 松开; (使......)释放",
      "e": [],
      "u": "5",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "kilogram",
      "p": "/'kɪləɡræm/",
      "t": "n.",
      "m": "千克; 公斤",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "programme",
      "p": "/'prəʊɡræm/",
      "t": "n.",
      "m": "计划; 方案",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "space station",
      "p": "",
      "t": "n. phr.",
      "m": "空间站",
      "e": [],
      "u": "5",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "do exercise",
      "p": "",
      "t": "v. phr.",
      "m": "锻炼; 运动",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "take photos",
      "p": "",
      "t": "v. phr.",
      "m": "拍照",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "get into orbit",
      "p": "",
      "t": "v. phr.",
      "m": "进入轨道",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "thousands of",
      "p": "",
      "t": "phrase",
      "m": "数以千计; 成千上万",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "system",
      "p": "/'sɪstəm/",
      "t": "n.",
      "m": "系统",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "file",
      "p": "/faɪl/",
      "t": "n.",
      "m": "档案",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "amazing",
      "p": "/ə'meɪzɪŋ/",
      "t": "adj.",
      "m": "令人大为惊奇的",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "planet",
      "p": "/'plænɪt/",
      "t": "n.",
      "m": "行星",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "billion",
      "p": "/'bɪljən/",
      "t": "n.",
      "m": "十亿",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "cover",
      "p": "/'kʌvə(r)/",
      "t": "v.",
      "m": "覆盖",
      "e": [],
      "u": "4",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "support",
      "p": "/sə'pɔːt/",
      "t": "v.",
      "m": "支持",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "area",
      "p": "/'eəriə/",
      "t": "n.",
      "m": "地区",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "freezing",
      "p": "/'friːzɪŋ/",
      "t": "adj.",
      "m": "冰冻的",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "north",
      "p": "/nɔːθ/",
      "t": "n.",
      "m": "北",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "pole",
      "p": "/pəʊl/",
      "t": "n.",
      "m": "(行星的)极地",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "desert",
      "p": "/'dezət/",
      "t": "n.",
      "m": "沙漠",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "reach",
      "p": "/riːtʃ/",
      "t": "v.",
      "m": "足够达到",
      "e": [],
      "u": "4",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "grain",
      "p": "/ɡreɪn/",
      "t": "n.",
      "m": "谷粒",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "wide",
      "p": "/waɪd/",
      "t": "adj.",
      "m": "宽的",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "whale",
      "p": "/weɪl/",
      "t": "n.",
      "m": "鲸",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "butterfly",
      "p": "/'bʌtəflaɪ/",
      "t": "n.",
      "m": "蝴蝶",
      "e": [],
      "u": "4",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "provide",
      "p": "/prə'vaɪd/",
      "t": "v.",
      "m": "提供",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "explore",
      "p": "/ɪk'splɔː(r)/",
      "t": "v.",
      "m": "探索",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "disappear",
      "p": "/ˌdɪsə'pɪə(r)/",
      "t": "v.",
      "m": "消失；不见",
      "e": [],
      "u": "4",
      "tags": [
        "pre_dis"
      ]
    },
    {
      "w": "plastic",
      "p": "/'plæstɪk/",
      "t": "n.",
      "m": "塑料",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "pollution",
      "p": "/pə'luːʃn/",
      "t": "n.",
      "m": "污染",
      "e": [],
      "u": "4",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "protect",
      "p": "/prə'tekt/",
      "t": "v.",
      "m": "保护",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "chemical",
      "p": "/'kemɪkl/",
      "t": "n.",
      "m": "化学品",
      "e": [],
      "u": "4",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "emperor",
      "p": "/'empərə(r)/",
      "t": "n.",
      "m": "皇帝",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "hunt",
      "p": "/hʌnt/",
      "t": "v.",
      "m": "打猎",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "war",
      "p": "/wɔː(r)/",
      "t": "n.",
      "m": "战争",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "sandstorm",
      "p": "/'sændstɔːm/",
      "t": "n.",
      "m": "沙暴",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "solution",
      "p": "/sə'luːʃn/",
      "t": "n.",
      "m": "解决方法",
      "e": [],
      "u": "4",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "blow",
      "p": "/bləʊ/",
      "t": "v.",
      "m": "吹",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "generation",
      "p": "/ˌdʒenə'reɪʃn/",
      "t": "n.",
      "m": "(统称)一代人",
      "e": [],
      "u": "4",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "groundwater",
      "p": "/'ɡraʊndwɔːtə(r)/",
      "t": "n.",
      "m": "地下水",
      "e": [],
      "u": "4",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "burn",
      "p": "/bɜːn/",
      "t": "v.",
      "m": "燃烧",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "oil",
      "p": "/ɔɪl/",
      "t": "n.",
      "m": "石油；原油",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "gas",
      "p": "/ɡæs/",
      "t": "n.",
      "m": "天然气",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "harmful",
      "p": "/'hɑːmfʊl/",
      "t": "adj.",
      "m": "有害的",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "website",
      "p": "/'websaɪt/",
      "t": "n.",
      "m": "网站",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "own",
      "p": "/əʊn/",
      "t": "adj.",
      "m": "自己的；本人的",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "however",
      "p": "/haʊ'evə(r)/",
      "t": "adv.",
      "m": "然而；不过",
      "e": [],
      "u": "4",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "solar system",
      "p": "",
      "t": "n. phr.",
      "m": "太阳系",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "be covered by",
      "p": "",
      "t": "v. phr.",
      "m": "被......覆盖",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "as far as we know",
      "p": "",
      "t": "phrase",
      "m": "据我们所知",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "sea level",
      "p": "",
      "t": "n. phr.",
      "m": "海平面",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "what's more",
      "p": "",
      "t": "phrase",
      "m": "更重要的是",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "provide with",
      "p": "",
      "t": "v. phr.",
      "m": "提供；给予",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "take care of",
      "p": "",
      "t": "v. phr.",
      "m": "照料",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "plastic pollution",
      "p": "",
      "t": "n. phr.",
      "m": "塑料污染",
      "e": [],
      "u": "4",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "hundreds of",
      "p": "",
      "t": "phrase",
      "m": "数百；以百计",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "blow away",
      "p": "",
      "t": "v. phr.",
      "m": "吹走",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "footprint",
      "p": "/'fʊtprɪnt/",
      "t": "n.",
      "m": "脚印；足迹",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "wet",
      "p": "/wet/",
      "t": "adj.",
      "m": "潮湿的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "deep",
      "p": "/diːp/",
      "t": "adj.",
      "m": "深的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "sandy",
      "p": "/'sændi/",
      "t": "adj.",
      "m": "铺满沙子的；含沙的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "kick",
      "p": "/kɪk/",
      "t": "v.",
      "m": "踢；踹",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "town",
      "p": "/taʊn/",
      "t": "n.",
      "m": "镇；市镇",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "feature",
      "p": "/'fiːtʃə(r)/",
      "t": "n.",
      "m": "特色；特征",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "south",
      "p": "/saʊθ/",
      "t": "n.",
      "m": "南；南方",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "clear",
      "p": "/klɪə(r)/",
      "t": "adj.",
      "m": "晴朗的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "loudly",
      "p": "/'laʊdli/",
      "t": "adv.",
      "m": "响地；响亮地",
      "e": [],
      "u": "3",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "thunder",
      "p": "/'θʌndə(r)/",
      "t": "n.",
      "m": "雷",
      "e": [],
      "u": "3",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "lightning",
      "p": "/'laɪtnɪŋ/",
      "t": "n.",
      "m": "闪电",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "fresh",
      "p": "/freʃ/",
      "t": "adj.",
      "m": "新鲜的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "gather",
      "p": "/'ɡæðə(r)/",
      "t": "v.",
      "m": "采集；聚集；水果等",
      "e": [],
      "u": "3",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "peaceful",
      "p": "/'piːsfl/",
      "t": "adj.",
      "m": "安静的；平静的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "snake",
      "p": "/sneɪk/",
      "t": "n.",
      "m": "蛇",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "through",
      "p": "/θruː/",
      "t": "prep.",
      "m": "自始至；从...穿过",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "from head to toe",
      "p": "",
      "t": "phrase",
      "m": "从头到脚",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "part",
      "p": "/pɑːt/",
      "t": "n.",
      "m": "区域；部分",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "article",
      "p": "/'ɑːtɪkl/",
      "t": "n.",
      "m": "文章",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "reason",
      "p": "/'riːzn/",
      "t": "n.",
      "m": "原因",
      "e": [],
      "u": "3",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "alive",
      "p": "/ə'laɪv/",
      "t": "adj.",
      "m": "有活力的；活着的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "sandcastle",
      "p": "/'sændkɑːsl/",
      "t": "n.",
      "m": "沙堡",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "crop",
      "p": "/krɒp/",
      "t": "n.",
      "m": "庄稼；作物",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "culture",
      "p": "/'kʌltʃə(r)/",
      "t": "n.",
      "m": "文化",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "local",
      "p": "/'ləʊkl/",
      "t": "adj.",
      "m": "地方的；当地的",
      "e": [],
      "u": "3",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "tradition",
      "p": "/trə'dɪʃn/",
      "t": "n.",
      "m": "传统",
      "e": [],
      "u": "3",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "continue",
      "p": "/kən'tɪnjuː/",
      "t": "v.",
      "m": "持续；继续",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "have picnics",
      "p": "",
      "t": "v. phr.",
      "m": "野餐",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "work one's land",
      "p": "",
      "t": "v. phr.",
      "m": "耕种土地",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "take a trip",
      "p": "",
      "t": "v. phr.",
      "m": "去旅行",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "solar term",
      "p": "",
      "t": "n. phr.",
      "m": "节气",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "in fact",
      "p": "",
      "t": "phrase",
      "m": "事实上",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "be divided into",
      "p": "",
      "t": "v. phr.",
      "m": "被分成...",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "be based on",
      "p": "",
      "t": "v. phr.",
      "m": "基于...",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "play an important part in",
      "p": "",
      "t": "v. phr.",
      "m": "在...发挥重要作用",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "geography",
      "p": "/dʒi'ɒɡrəfi/",
      "t": "n.",
      "m": "地理",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "corner",
      "p": "/'kɔːnə(r)/",
      "t": "n.",
      "m": "角；角落",
      "e": [],
      "u": "2",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "activity",
      "p": "/æk'tɪvəti/",
      "t": "n.",
      "m": "活动",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "club",
      "p": "/klʌb/",
      "t": "n.",
      "m": "社团",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "practice",
      "p": "/'præktɪs/",
      "t": "v.",
      "m": "练习",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "solve",
      "p": "/sɒlv/",
      "t": "v.",
      "m": "解答；解决",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "develop",
      "p": "/dɪ'veləp/",
      "t": "v.",
      "m": "培养；发展",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "skill",
      "p": "/skɪl/",
      "t": "n.",
      "m": "技术；技能",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "teen",
      "p": "/tiːn/",
      "t": "n.",
      "m": "青少年",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "magazine",
      "p": "/ˌmæɡə'ziːn/",
      "t": "n.",
      "m": "杂志",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "teenager",
      "p": "/'tiːneɪdʒə(r)/",
      "t": "n.",
      "m": "(13至19岁)的少年",
      "e": [],
      "u": "2",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "greeting",
      "p": "/'ɡriːtɪŋ/",
      "t": "n.",
      "m": "问候；招呼",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "great",
      "p": "/ɡreɪt/",
      "t": "adj.",
      "m": "伟大的；极好的",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "energy",
      "p": "/'enədʒi/",
      "t": "n.",
      "m": "精力；活力",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "drama",
      "p": "/'drɑːmə/",
      "t": "n.",
      "m": "戏；戏剧",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "sincerely",
      "p": "/'sɪnsəli/",
      "t": "adv.",
      "m": "诚地；真诚地",
      "e": [],
      "u": "2",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "diary",
      "p": "/'daɪəri/",
      "t": "n.",
      "m": "日记；日记本",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "project",
      "p": "/'prɒdʒekt/",
      "t": "n.",
      "m": "课题",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "realize",
      "p": "/'rɪəlaɪz/",
      "t": "v.",
      "m": "意识到",
      "e": [],
      "u": "2",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "luckily",
      "p": "/'lʌkɪli/",
      "t": "adv.",
      "m": "幸运地",
      "e": [],
      "u": "2",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "just",
      "p": "/dʒʌst/",
      "t": "adv.",
      "m": "恰好",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "presentation",
      "p": "/ˌprezn'teɪʃn/",
      "t": "n.",
      "m": "演示",
      "e": [],
      "u": "2",
      "tags": [
        "suf_tion",
        "pre_pre"
      ]
    },
    {
      "w": "mood",
      "p": "/muːd/",
      "t": "n.",
      "m": "情绪；心情",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "mind",
      "p": "/maɪnd/",
      "t": "n.",
      "m": "意介",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "main",
      "p": "/meɪn/",
      "t": "adj.",
      "m": "主要的",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "pack",
      "p": "/pæk/",
      "t": "v.",
      "m": "(把......)打包",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "celebrate",
      "p": "/'selɪbreɪt/",
      "t": "v.",
      "m": "庆祝",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "success",
      "p": "/sək'ses/",
      "t": "n.",
      "m": "成功",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "rocky",
      "p": "/'rɒki/",
      "t": "adj.",
      "m": "困难的",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "do the dishes",
      "p": "",
      "t": "v. phr.",
      "m": "洗碗",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "junior high school",
      "p": "",
      "t": "n. phr.",
      "m": "初级中学",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "take part in",
      "p": "",
      "t": "v. phr.",
      "m": "参与",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "look forward to",
      "p": "",
      "t": "v. phr.",
      "m": "期待；盼望",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "daily life",
      "p": "",
      "t": "n. phr.",
      "m": "日常生活",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "go to bed",
      "p": "",
      "t": "v. phr.",
      "m": "去睡觉",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "get up",
      "p": "",
      "t": "v. phr.",
      "m": "起床",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "on foot",
      "p": "",
      "t": "phrase",
      "m": "步行",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "full of energy",
      "p": "",
      "t": "phrase",
      "m": "充满活力",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "get ready for",
      "p": "",
      "t": "v. phr.",
      "m": "准备好......",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "put on",
      "p": "",
      "t": "v. phr.",
      "m": "穿上；上演",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "clean up",
      "p": "",
      "t": "v. phr.",
      "m": "打扫",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "pick up",
      "p": "",
      "t": "v. phr.",
      "m": "捡起；取(某物)",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "guitar",
      "p": "/ɡɪ'tɑː(r)/",
      "t": "n.",
      "m": "吉他",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "honest",
      "p": "/'ɒnɪst/",
      "t": "adj.",
      "m": "诚实的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "patient",
      "p": "/'peɪʃnt/",
      "t": "adj.",
      "m": "有耐心的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "improve",
      "p": "/ɪm'pruːv/",
      "t": "v.",
      "m": "改进；改善",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "convenient",
      "p": "/kən'viːniənt/",
      "t": "adj.",
      "m": "方便的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "confident",
      "p": "/'kɒnfɪdənt/",
      "t": "adj.",
      "m": "自信的；有自信心的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "courage",
      "p": "/'kʌrɪdʒ/",
      "t": "n.",
      "m": "勇气；胆量",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "friendship",
      "p": "/'frendʃɪp/",
      "t": "n.",
      "m": "友谊；朋友关系",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "trust",
      "p": "/trʌst/",
      "t": "n.",
      "m": "信任；信赖",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "survey",
      "p": "/'sɜːveɪ/",
      "t": "n.",
      "m": "调查",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "personal",
      "p": "/'pɜːsənl/",
      "t": "adj.",
      "m": "个性的；个人的",
      "e": [],
      "u": "1",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "quality",
      "p": "/'kwɒləti/",
      "t": "n.",
      "m": "品质；品德",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "caring",
      "p": "/'keərɪŋ/",
      "t": "adj.",
      "m": "关心他人的；体贴的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "describe",
      "p": "/dɪ'skraɪb/",
      "t": "v.",
      "m": "描述；形容",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "appearance",
      "p": "/ə'pɪərəns/",
      "t": "n.",
      "m": "外貌",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "straight",
      "p": "/streɪt/",
      "t": "adj.",
      "m": "直的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "dark",
      "p": "/dɑːk/",
      "t": "adj.",
      "m": "褐色的；乌色的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "same",
      "p": "/seɪm/",
      "t": "adj.",
      "m": "同的；相同的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "basic",
      "p": "/'beɪsɪk/",
      "t": "adj.",
      "m": "基本的；基础的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "thought",
      "p": "/θɔːt/",
      "t": "n.",
      "m": "想法",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "glad",
      "p": "/ɡlæd/",
      "t": "adj.",
      "m": "高兴的；愉快的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "rise",
      "p": "/raɪz/",
      "t": "v.",
      "m": "上升",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "end",
      "p": "/end/",
      "t": "n.",
      "m": "结局",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "heart",
      "p": "/hɑːt/",
      "t": "n.",
      "m": "心",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "be good at",
      "p": "",
      "t": "v. phr.",
      "m": "擅长......",
      "e": [],
      "u": "1",
      "tags": [
        "phr_be_at"
      ]
    },
    {
      "w": "turn to somebody for help",
      "p": "",
      "t": "v. phr.",
      "m": "求助于某人",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "after school",
      "p": "",
      "t": "phrase",
      "m": "放学后",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "be willing to do something",
      "p": "",
      "t": "v. phr.",
      "m": "愿意做某事",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "medium height",
      "p": "",
      "t": "n. phr.",
      "m": "中等身高",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "modern dance",
      "p": "",
      "t": "n. phr.",
      "m": "现代舞",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "watch over",
      "p": "",
      "t": "v. phr.",
      "m": "照看；监督",
      "e": [],
      "u": "1",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "rise into",
      "p": "",
      "t": "v. phr.",
      "m": "上升到",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "cry out",
      "p": "",
      "t": "v. phr.",
      "m": "大叫；叫喊",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "cry oneself to sleep",
      "p": "",
      "t": "v. phr.",
      "m": "哭着睡着",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "wake up",
      "p": "",
      "t": "v. phr.",
      "m": "醒来",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "come and go",
      "p": "",
      "t": "v. phr.",
      "m": "来去",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "take somebody's place",
      "p": "",
      "t": "v. phr.",
      "m": "替代某人",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "come along",
      "p": "",
      "t": "v. phr.",
      "m": "出现",
      "e": [],
      "u": "1",
      "tags": []
    }
  ],
  "7B": {
    "1": [
      {
        "w": "advice",
        "p": "/əd'vaɪs/",
        "t": "n.",
        "m": "建议",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "encourage",
        "p": "/ɪn'kʌrɪdʒ/",
        "t": "v.",
        "m": "鼓励",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "retire",
        "p": "/rɪ'taɪə(r)/",
        "t": "v.",
        "m": "(使) 退休",
        "e": [],
        "u": "1",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "cheerful",
        "p": "/'tʃɪəfl/",
        "t": "adj.",
        "m": "快乐的; 高兴的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "community",
        "p": "/kə'mjuːnəti/",
        "t": "n.",
        "m": "社区",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "medical",
        "p": "/'medɪkl/",
        "t": "adj.",
        "m": "医学的",
        "e": [],
        "u": "1",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "fried",
        "p": "/fraɪd/",
        "t": "adj.",
        "m": "油腻的; 油炸的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "wherever",
        "p": "/weə'evə(r)/",
        "t": "conj.",
        "m": "各处; 处处",
        "e": [],
        "u": "1",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "future",
        "p": "/fjuːtʃə(r)/",
        "t": "n.",
        "m": "将来; 未来",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "soon",
        "p": "/suːn/",
        "t": "adv.",
        "m": "很快; 马上; 不久",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "smart",
        "p": "/smɑːt/",
        "t": "adj.",
        "m": "聪明的; 机敏的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "attention",
        "p": "/ə'tenʃn/",
        "t": "n.",
        "m": "专心; 注意力",
        "e": [],
        "u": "1",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "seldom",
        "p": "/'seldəm/",
        "t": "adv.",
        "m": "不常; 很少",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "bored",
        "p": "/bɔːrd/",
        "t": "adj.",
        "m": "(对某人/事物) 厌烦的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "strict",
        "p": "/strɪkt/",
        "t": "adj.",
        "m": "要求严格的; 严厉的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "relative",
        "p": "/'relətɪv/",
        "t": "n.",
        "m": "亲戚; 亲属",
        "e": [],
        "u": "1",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "uniform",
        "p": "/'juːnɪfɔːm/",
        "t": "n.",
        "m": "制服",
        "e": [],
        "u": "1",
        "tags": [
          "pre_un"
        ]
      },
      {
        "w": "personality",
        "p": "/ˌpɜːsə'næləti/",
        "t": "n.",
        "m": "性格; 个性",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "characteristic",
        "p": "/ˌkærəktə'rɪstɪk/",
        "t": "n.",
        "m": "特点; 品质",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "topic",
        "p": "/'tɒpɪk/",
        "t": "n.",
        "m": "话题; 标题",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "active",
        "p": "/'æktɪv/",
        "t": "adj.",
        "m": "忙碌的; 活跃的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "smartphone",
        "p": "/'smɑːtfəʊn/",
        "t": "n.",
        "m": "智能手机",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "give up",
        "p": "/ɡɪv ʌp/",
        "t": "v. phr.",
        "m": "放弃",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "used to",
        "p": "/juːst tu/",
        "t": "v.",
        "m": "曾经",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "be strict about",
        "p": "/bi strɪkt ə'baʊt/",
        "t": "v. phr.",
        "m": "要求严格",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "be worried about",
        "p": "/bi 'wʌrid ə'baʊt/",
        "t": "v. phr.",
        "m": "担心",
        "e": [],
        "u": "1",
        "tags": []
      }
    ],
    "2": [
      {
        "w": "wine",
        "p": "/waɪn/",
        "t": "n.",
        "m": "葡萄酒",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "match",
        "p": "/mætʃ/",
        "t": "v.",
        "m": "对比",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "rich",
        "p": "/rɪtʃ/",
        "t": "adj.",
        "m": "丰富的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "lie",
        "p": "/laɪ/",
        "t": "v.",
        "m": "坐着",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "cafe",
        "p": "/'kæfeɪ/",
        "t": "n.",
        "m": "咖啡馆; 小咖啡馆",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "excellent",
        "p": "/'eksələnt/",
        "t": "adj.",
        "m": "优秀的; 很好的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "sea",
        "p": "/siː/",
        "t": "n.",
        "m": "海",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "coast",
        "p": "/kəʊst/",
        "t": "n.",
        "m": "海岸",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "perfect",
        "p": "/'pɜːfɪkt/",
        "t": "adj.",
        "m": "完美的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "mostly",
        "p": "/'məʊstli/",
        "t": "adv.",
        "m": "主要; 通常",
        "e": [],
        "u": "2",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "receive",
        "p": "/rɪ'siːv/",
        "t": "v.",
        "m": "接收; 收到",
        "e": [],
        "u": "2",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "key",
        "p": "/kiː/",
        "t": "adj.",
        "m": "主要的; 关键的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "remain",
        "p": "/rɪ'meɪn/",
        "t": "v.",
        "m": "仍然是; 保持不变",
        "e": [],
        "u": "2",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "lift",
        "p": "/lɪft/",
        "t": "n.",
        "m": "电梯",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "step",
        "p": "/step/",
        "t": "n.",
        "m": "台阶",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "stair",
        "p": "/steə(r)/",
        "t": "n.",
        "m": "楼梯",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "motorcycle",
        "p": "/'məʊtəsaɪkl/",
        "t": "n.",
        "m": "摩托车",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "sightseeing",
        "p": "/'saɪtˌsiːɪŋ/",
        "t": "n.",
        "m": "观光; 游览",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "imagine",
        "p": "/ɪ'mædʒɪn/",
        "t": "v.",
        "m": "想象; 设想",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "destination",
        "p": "/ˌdestɪ'neɪʃn/",
        "t": "n.",
        "m": "目的地; 终点",
        "e": [],
        "u": "2",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "address",
        "p": "/ə'dres/",
        "t": "n.",
        "m": "地址",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "government",
        "p": "/'ɡʌvənmənt/",
        "t": "n.",
        "m": "政府",
        "e": [],
        "u": "2",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "unique",
        "p": "/ju'niːk/",
        "t": "adj.",
        "m": "特有的; 独特的",
        "e": [],
        "u": "2",
        "tags": [
          "pre_un"
        ]
      },
      {
        "w": "endangered",
        "p": "/ɪn'deɪndʒəd/",
        "t": "adj.",
        "m": "濒危的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "stretch",
        "p": "/streʧ/",
        "t": "v.",
        "m": "延伸",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "huge",
        "p": "/hjuːdʒ/",
        "t": "adj.",
        "m": "巨大的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "wild",
        "p": "/waɪld/",
        "t": "adj.",
        "m": "野的; 野生的; 自然环境",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "discover",
        "p": "/dɪ'skʌvə(r)/",
        "t": "v.",
        "m": "了解到; 查明",
        "e": [],
        "u": "2",
        "tags": [
          "suf_er",
          "pre_dis"
        ]
      },
      {
        "w": "volcano",
        "p": "/vɒl'keɪnəʊ/",
        "t": "n.",
        "m": "火山",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "reindeer",
        "p": "/'reɪndɪə(r)/",
        "t": "n.",
        "m": "驯鹿",
        "e": [],
        "u": "2",
        "tags": [
          "suf_er",
          "pre_re"
        ]
      },
      {
        "w": "snowmobile",
        "p": "/'snəʊməbaɪl/",
        "t": "n.",
        "m": "雪地摩托车",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "wolf",
        "p": "/wʊlf/",
        "t": "n.",
        "m": "狼",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "department store",
        "p": "/dɪ'pɑːtmənt stɔː(r)/",
        "t": "n. phr.",
        "m": "百货商店",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "prefer to",
        "p": "/prɪ'fɜː(r) tu/",
        "t": "v. phr.",
        "m": "更喜欢",
        "e": [],
        "u": "2",
        "tags": [
          "pre_pre"
        ]
      },
      {
        "w": "by hand",
        "p": "/baɪ hænd/",
        "t": "phrase",
        "m": "用手工",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "go sightseeing",
        "p": "/ɡəʊ 'saɪtˌsiːɪŋ/",
        "t": "v. phr.",
        "m": "去观光",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "go on a trip",
        "p": "/ɡəʊ ɒn ə trɪp/",
        "t": "v. phr.",
        "m": "去旅行",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "set up",
        "p": "/set ʌp/",
        "t": "v. phr.",
        "m": "建立",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "giant panda",
        "p": "/'dʒaɪənt ˈpændə/",
        "t": "n. phr.",
        "m": "大熊猫",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "in the wild",
        "p": "/ɪn ðə waɪld/",
        "t": "phrase",
        "m": "在野外",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "all year round",
        "p": "/ɔːl jɪə(r) raʊnd/",
        "t": "phrase",
        "m": "全年",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "golden monkey",
        "p": "/'ɡəʊldən ˈmʌŋki/",
        "t": "n. phr.",
        "m": "金丝猴",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "go hiking",
        "p": "/ɡəʊ ˈhaɪkɪŋ/",
        "t": "v. phr.",
        "m": "去远足",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "hot spring",
        "p": "/hɒt sprɪŋ/",
        "t": "n. phr.",
        "m": "温泉",
        "e": [],
        "u": "2",
        "tags": []
      }
    ],
    "3": [
      {
        "w": "branch",
        "p": "/brɑːntʃ/",
        "t": "n.",
        "m": "树枝",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "root",
        "p": "/ruːt/",
        "t": "n.",
        "m": "根; 根茎",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "silent",
        "p": "/'saɪlənt/",
        "t": "adj.",
        "m": "不说话的; 沉默的",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "overlook",
        "p": "/ˌəʊvə'lʊk/",
        "t": "v.",
        "m": "忽略; 未注意到",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "human",
        "p": "/'hjuːmən/",
        "t": "n.",
        "m": "人",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "oxygen",
        "p": "/'ɒksɪdʒən/",
        "t": "n.",
        "m": "氧气",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "create",
        "p": "/kri'eɪt/",
        "t": "v.",
        "m": "创造",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "environment",
        "p": "/ɪn'vaɪrənmənt/",
        "t": "n.",
        "m": "自然环境",
        "e": [],
        "u": "3",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "convenient",
        "p": "/kən'viːniənt/",
        "t": "adj.",
        "m": "便利的; 方便的",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "furniture",
        "p": "/'fɜːrnɪtʃə(r)/",
        "t": "n.",
        "m": "(可移动的) 家具",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "wood",
        "p": "/wʊd/",
        "t": "n.",
        "m": "木; 木头",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "treat",
        "p": "/triːt/",
        "t": "v.",
        "m": "以......态度对待; 以......方式对待",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "communicate",
        "p": "/kə'mjuːnɪkeɪt/",
        "t": "v.",
        "m": "交流; 沟通",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "species",
        "p": "/'spiːʃiːz/",
        "t": "n.",
        "m": "种; 物种",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "product",
        "p": "/'prɒdʌkt/",
        "t": "n.",
        "m": "产品; 制品",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "side",
        "p": "/saɪd/",
        "t": "n.",
        "m": "一面",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "borrow",
        "p": "/'bɒrəʊ/",
        "t": "v.",
        "m": "借; 借用",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "dig",
        "p": "/dɪɡ/",
        "t": "v.",
        "m": "掘; 挖 (地); 凿 (洞)",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "hole",
        "p": "/həʊl/",
        "t": "n.",
        "m": "洞; 孔; 坑",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "stick",
        "p": "/stɪk/",
        "t": "n.",
        "m": "条; 杆",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "accident",
        "p": "/'æksɪdənt/",
        "t": "n.",
        "m": "意外; 偶然的事",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "knowledge",
        "p": "/'nɒlɪdʒ/",
        "t": "n.",
        "m": "知识; 学问",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "character",
        "p": "/'kærəktə(r)/",
        "t": "n.",
        "m": "文字",
        "e": [],
        "u": "3",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "spread",
        "p": "/spred/",
        "t": "v.",
        "m": "传播",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "translation",
        "p": "/træn'sleɪʃn/",
        "t": "n.",
        "m": "译本; 译文",
        "e": [],
        "u": "3",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "take in",
        "p": "",
        "t": "v. phr.",
        "m": "吸收",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "greenhouse gas",
        "p": "",
        "t": "n. phr.",
        "m": "温室气体",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "begin with",
        "p": "",
        "t": "v. phr.",
        "m": "首先; 第一点",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "come from",
        "p": "",
        "t": "v. phr.",
        "m": "来自",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "look around",
        "p": "",
        "t": "v. phr.",
        "m": "环视; 四下察看",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "be made of",
        "p": "",
        "t": "v. phr.",
        "m": "由......制成",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "for example",
        "p": "",
        "t": "phrase",
        "m": "例如; 譬如",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "communicate with",
        "p": "",
        "t": "v. phr.",
        "m": "与......沟通",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "call on",
        "p": "",
        "t": "v. phr.",
        "m": "号召; 动员; 要求",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "according to",
        "p": "",
        "t": "phrase",
        "m": "按 (......所说)",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "by accident",
        "p": "",
        "t": "phrase",
        "m": "偶然; 意外地",
        "e": [],
        "u": "3",
        "tags": []
      }
    ],
    "4": [
      {
        "w": "dolphin",
        "p": "/'dɒlfɪn/",
        "t": "n.",
        "m": "海豚",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "hen",
        "p": "/hen/",
        "t": "n.",
        "m": "母鸡",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "blind",
        "p": "/blaɪnd/",
        "t": "adj.",
        "m": "adj. 瞎的; 失明的",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "wool",
        "p": "/wʊl/",
        "t": "n.",
        "m": "羊毛",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "sir",
        "p": "/sɜː(r)/",
        "t": "n.",
        "m": "先生",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "receptionist",
        "p": "/rɪ'sepʃənɪst/",
        "t": "n.",
        "m": "接待员",
        "e": [],
        "u": "4",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "allow",
        "p": "/ə'laʊ/",
        "t": "v.",
        "m": "允许进入 (或 出去、通过)",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "asleep",
        "p": "/ə'sliːp/",
        "t": "adj.",
        "m": "睡着的",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "smoke",
        "p": "/sməʊk/",
        "t": "n.",
        "m": "烟",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "fireman",
        "p": "/'faɪəmən/",
        "t": "n.",
        "m": "消防队员",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "type",
        "p": "/taɪp/",
        "t": "n.",
        "m": "类型",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "search-and-rescue",
        "p": "/sɜːtʃ ænd 'rezkjuː/",
        "t": "n.",
        "m": "搜索救援",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "search",
        "p": "/sɜːtʃ/",
        "t": "n.",
        "m": "搜索",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "service",
        "p": "/'sɜːvɪs/",
        "t": "n.",
        "m": "服务",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "disaster",
        "p": "/dɪ'zɑːstə(r)/",
        "t": "n.",
        "m": "灾难; 灾害",
        "e": [],
        "u": "4",
        "tags": [
          "suf_er",
          "pre_dis"
        ]
      },
      {
        "w": "guest",
        "p": "/ɡest/",
        "t": "n.",
        "m": "旅客; 房客",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "team",
        "p": "/tiːm/",
        "t": "n.",
        "m": "组; 团队",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "transport",
        "p": "/træn'spɔːt/",
        "t": "n.",
        "m": "运输; 交通",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "guard",
        "p": "/ɡɑːd/",
        "t": "n.",
        "m": "守卫; 保卫",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "honey",
        "p": "/'hʌni/",
        "t": "n.",
        "m": "蜂蜜",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "material",
        "p": "/mə'tɪəriəl/",
        "t": "n.",
        "m": "材料; 原料",
        "e": [],
        "u": "4",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "either",
        "p": "/'aɪðə(r)/",
        "t": "adj.",
        "m": "也",
        "e": [],
        "u": "4",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "shark",
        "p": "/ʃɑːk/",
        "t": "n.",
        "m": "鲨鱼",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "scared",
        "p": "/skeəd/",
        "t": "adj.",
        "m": "害怕的; 恐惧的",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "grey",
        "p": "/ɡreɪ/",
        "t": "adj.",
        "m": "灰色的",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "somewhere",
        "p": "/'sʌmweə(r)/",
        "t": "adv.",
        "m": "在某处; 到某处",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "probably",
        "p": "/'prɒbəbli/",
        "t": "adv.",
        "m": "很可能; 大概",
        "e": [],
        "u": "4",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "source",
        "p": "/sɔːs/",
        "t": "n.",
        "m": "来源; 出处",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "sometime",
        "p": "/'sʌmtaɪm/",
        "t": "adv.",
        "m": "在某时",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "extinct",
        "p": "/ɪk'stɪŋkt/",
        "t": "adj.",
        "m": "已灭绝的",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "effort",
        "p": "/'efət/",
        "t": "n.",
        "m": "努力; 试图",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "lead (somebody)",
        "p": "",
        "t": "v. phr.",
        "m": "带着(某人)",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "fall asleep",
        "p": "",
        "t": "v. phr.",
        "m": "睡着",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "get down",
        "p": "",
        "t": "v. phr.",
        "m": "俯身; 趴下",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "fire engine",
        "p": "",
        "t": "n. phr.",
        "m": "消防车",
        "e": [],
        "u": "4",
        "tags": []
      }
    ],
    "5": [
      {
        "w": "everyday",
        "p": "/'evrideɪ/",
        "t": "adj.",
        "m": "每天的; 日常的",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "journey",
        "p": "/'dʒɜːni/",
        "t": "n.",
        "m": "(尤指长途) 旅行",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "drop",
        "p": "/drɒp/",
        "t": "v.",
        "m": "滴; 洒; 掉落",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "tap",
        "p": "/tæp/",
        "t": "n.",
        "m": "水龙头; 敲击声",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "voice",
        "p": "/vɔɪs/",
        "t": "n.",
        "m": "说话声; 声音",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "eventually",
        "p": "/ɪ'ventʃuəli/",
        "t": "adv.",
        "m": "最后; 终于",
        "e": [],
        "u": "5",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "pipe",
        "p": "/paɪp/",
        "t": "n.",
        "m": "管子; 管道",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "return",
        "p": "/rɪ'tɜːn/",
        "t": "v.",
        "m": "回去; 返回",
        "e": [],
        "u": "5",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "rush",
        "p": "/rʌʃ/",
        "t": "v.",
        "m": "迅速移动; 冲",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "bath",
        "p": "/bɑːθ/",
        "t": "n.",
        "m": "洗澡; 沐浴",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "salt",
        "p": "/sɔːlt/",
        "t": "n.",
        "m": "盐; 含盐的",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "bream",
        "p": "/briːm/",
        "t": "n.",
        "m": "鲻鱼",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "fix",
        "p": "/fɪks/",
        "t": "v.",
        "m": "修理",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "public",
        "p": "/'pʌblɪk/",
        "t": "adj.",
        "m": "公共的; 公开的",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "population",
        "p": "/ˌpɒpju'leɪʃn/",
        "t": "n.",
        "m": "人口",
        "e": [],
        "u": "5",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "agriculture",
        "p": "/'æɡrɪkʌltʃə(r)/",
        "t": "n.",
        "m": "农业",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "trade",
        "p": "/treɪd/",
        "t": "n.",
        "m": "贸易; 交易",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "industry",
        "p": "/'ɪndəstri/",
        "t": "n.",
        "m": "工业; 产业",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "goods",
        "p": "/ɡʊdz/",
        "t": "n.",
        "m": "商品; 货物",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "overseas",
        "p": "/ˌəʊvə'siːz/",
        "t": "adv.",
        "m": "在国外; 向海外",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "global",
        "p": "/'ɡləʊbl/",
        "t": "adj.",
        "m": "全球的; 全世界的",
        "e": [],
        "u": "5",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "income",
        "p": "/'ɪnkʌm/",
        "t": "n.",
        "m": "收入; 收益",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "nearly",
        "p": "/'nɪəli/",
        "t": "adv.",
        "m": "几乎; 差不多; 将近",
        "e": [],
        "u": "5",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "business",
        "p": "/'bɪznəs/",
        "t": "n.",
        "m": "买卖; 生意",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "leisure",
        "p": "/'leʒə(r)/",
        "t": "n.",
        "m": "闲暇; 休闲",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "throughout",
        "p": "/θruː'aʊt/",
        "t": "prep.",
        "m": "自始至终; 贯穿",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "duty",
        "p": "/'djuːti/",
        "t": "n.",
        "m": "责任; 义务",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "a bit",
        "p": "",
        "t": "phrase",
        "m": "有点儿; 稍微",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "at once",
        "p": "",
        "t": "phrase",
        "m": "立即; 马上",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "drinking water",
        "p": "",
        "t": "n. phr.",
        "m": "饮用水",
        "e": [],
        "u": "5",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "play a role in",
        "p": "",
        "t": "v. phr.",
        "m": "在......中起作用",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "steam engine",
        "p": "",
        "t": "n. phr.",
        "m": "蒸汽机",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "as a result",
        "p": "",
        "t": "phrase",
        "m": "作为结果; 因此",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "make sure",
        "p": "",
        "t": "v. phr.",
        "m": "确保; 保证",
        "e": [],
        "u": "5",
        "tags": []
      }
    ],
    "6": [
      {
        "w": "battery",
        "p": "/'bætəri/",
        "t": "n.",
        "m": "电池",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "electricity",
        "p": "/ɪ'lektrɪsəti/",
        "t": "n.",
        "m": "电; 电能",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "switch off",
        "p": "/swɪtʃ ɒf/",
        "t": "v. phr.",
        "m": "关闭 (电灯、机器等)",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "task",
        "p": "/tɑːsk/",
        "t": "n.",
        "m": "任务; 活动",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "while",
        "p": "/waɪl/",
        "t": "conj.",
        "m": "在......期间; 一会儿",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "tablet",
        "p": "/'tæblət/",
        "t": "n.",
        "m": "平板电脑",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "fridge",
        "p": "/frɪdʒ/",
        "t": "n.",
        "m": "冰箱",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "yogurt",
        "p": "/'jɒɡət/",
        "t": "n.",
        "m": "酸奶",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "apartment",
        "p": "/ə'pɑːtmənt/",
        "t": "n.",
        "m": "公寓; 套房",
        "e": [],
        "u": "6",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "household",
        "p": "/'haʊshəʊld/",
        "t": "n.",
        "m": "家庭; 家政",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "against",
        "p": "/ə'ɡenst/",
        "t": "prep.",
        "m": "反对; 抵抗; 紧靠",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "speed",
        "p": "/spiːd/",
        "t": "n.",
        "m": "速度; 速率",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "safety",
        "p": "/'seɪfti/",
        "t": "n.",
        "m": "安全; 安全性",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "instruction",
        "p": "/ɪn'strʌkʃn/",
        "t": "n.",
        "m": "用语说明; 指示",
        "e": [],
        "u": "6",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "connect",
        "p": "/kə'nekt/",
        "t": "v.",
        "m": "(使) 连接",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "rule",
        "p": "/ruːl/",
        "t": "n.",
        "m": "规则; 条例",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "climate",
        "p": "/'klaɪmət/",
        "t": "n.",
        "m": "气候",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "amount",
        "p": "/ə'maʊnt/",
        "t": "n.",
        "m": "数量",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "power",
        "p": "/'paʊə(r)/",
        "t": "v.",
        "m": "推动 (机器或车辆)",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "television",
        "p": "/'telɪvɪʒn/",
        "t": "n.",
        "m": "电视",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "landmark",
        "p": "/'lændmɑːk/",
        "t": "n.",
        "m": "地标; 特征",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "have something in common",
        "p": "",
        "t": "v. phr.",
        "m": "有相同的特征 (或特点)",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "light bulb",
        "p": "",
        "t": "n. phr.",
        "m": "电灯泡",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "air conditioner",
        "p": "",
        "t": "n. phr.",
        "m": "空调; 空调设备",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "video game",
        "p": "",
        "t": "n. phr.",
        "m": "电子游戏",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "run out of",
        "p": "",
        "t": "v. phr.",
        "m": "用完; 耗尽",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "cover",
        "p": "/'kʌvə(r)/",
        "t": "v.",
        "m": "覆盖",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "average",
        "p": "/'ævərɪdʒ/",
        "t": "n.",
        "m": "平均; 平均数",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "culture",
        "p": "/'kʌltʃə(r)/",
        "t": "n.",
        "m": "文化",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "global",
        "p": "/'ɡləʊbl/",
        "t": "adj.",
        "m": "全球的; 全世界的",
        "e": [],
        "u": "6",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "history",
        "p": "/'hɪstri/",
        "t": "n.",
        "m": "历史",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "together",
        "p": "/tə'ɡeðə(r)/",
        "t": "adv.",
        "m": "一起; 共同",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "language",
        "p": "/'læŋɡwɪdʒ/",
        "t": "n.",
        "m": "语言",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "science",
        "p": "/'saɪəns/",
        "t": "n.",
        "m": "科学",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "fact",
        "p": "/fækt/",
        "t": "n.",
        "m": "事实",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "study",
        "p": "/'stʌdi/",
        "t": "v.",
        "m": "学习; 研究",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "special",
        "p": "/'speʃl/",
        "t": "adj.",
        "m": "特别的; 特殊的",
        "e": [],
        "u": "6",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "useful",
        "p": "/'juːsfl/",
        "t": "adj.",
        "m": "有用的",
        "e": [],
        "u": "6",
        "tags": []
      }
    ],
    "7": [
      {
        "w": "contribution",
        "p": "/ˌkɒntrɪ'bjuːʃn/",
        "t": "n.",
        "m": "贡献",
        "e": [],
        "u": "7",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "hero",
        "p": "/'hɪərəʊ/",
        "t": "n.",
        "m": "英雄",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "pioneer",
        "p": "/ˌpaɪə'niːə(r)/",
        "t": "n.",
        "m": "先锋; 先驱",
        "e": [],
        "u": "7",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "technology",
        "p": "/tek'nɒlədʒi/",
        "t": "n.",
        "m": "科技",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "receive",
        "p": "/rɪ'siːv/",
        "t": "v.",
        "m": "拿到; 收到",
        "e": [],
        "u": "7",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "award",
        "p": "/ə'wɔːd/",
        "t": "n.",
        "m": "奖; 奖项",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "engineering",
        "p": "/ˌendʒɪ'nɪərɪŋ/",
        "t": "n.",
        "m": "工程学",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "education",
        "p": "/ˌedju'keɪʃn/",
        "t": "n.",
        "m": "教育",
        "e": [],
        "u": "7",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "spend",
        "p": "/spend/",
        "t": "v.",
        "m": "花 (时间); 度过",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "earn",
        "p": "/ɜːn/",
        "t": "v.",
        "m": "赚 (钱); 赢得",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "common",
        "p": "/'kɒmən/",
        "t": "adj.",
        "m": "共同的; 普通的",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "difference",
        "p": "/'dɪfrəns/",
        "t": "n.",
        "m": "差别; 差异",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "habit",
        "p": "/'hæbɪt/",
        "t": "n.",
        "m": "习惯",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "useful",
        "p": "/'juːsfl/",
        "t": "adj.",
        "m": "有用的",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "environment",
        "p": "/ɪn'vaɪrənmənt/",
        "t": "n.",
        "m": "环境",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "improve",
        "p": "/ɪm'pruːv/",
        "t": "v.",
        "m": "改善; 提高",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "success",
        "p": "/sək'ses/",
        "t": "n.",
        "m": "成功",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "change",
        "p": "/tʃeɪndʒ/",
        "t": "v.",
        "m": "改变",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "life",
        "p": "/laɪf/",
        "t": "n.",
        "m": "生活; 生命",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "important",
        "p": "/ɪm'pɔːtənt/",
        "t": "adj.",
        "m": "重要的",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "goal",
        "p": "/ɡəʊl/",
        "t": "n.",
        "m": "目标",
        "e": [],
        "u": "7",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "achieve",
        "p": "/ə'tʃiːv/",
        "t": "v.",
        "m": "达到; 实现",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "decision",
        "p": "/dɪ'sɪʒn/",
        "t": "n.",
        "m": "决定",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "/plan/",
        "p": "/plæn/",
        "t": "n.",
        "m": "计划",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "hard",
        "p": "/hɑːd/",
        "t": "adj.",
        "m": "努力的; 困难的",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "way",
        "p": "/weɪ/",
        "t": "n.",
        "m": "方式; 方法",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "focus",
        "p": "/'fəʊkəs/",
        "t": "v.",
        "m": "集中; 专注",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "effort",
        "p": "/'efət/",
        "t": "n.",
        "m": "努力",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "result",
        "p": "/rɪ'zʌlt/",
        "t": "n.",
        "m": "结果",
        "e": [],
        "u": "7",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "succeed",
        "p": "/sək'siːd/",
        "t": "v.",
        "m": "成功",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "build",
        "p": "/bɪld/",
        "t": "v.",
        "m": "建立; 建设",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "together",
        "p": "/tə'ɡeðə(r)/",
        "t": "adv.",
        "m": "一起",
        "e": [],
        "u": "7",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "experience",
        "p": "/ɪk'spɪəriəns/",
        "t": "n.",
        "m": "经验; 经历",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "understand",
        "p": "/ˌʌndə'stænd/",
        "t": "v.",
        "m": "理解; 明白",
        "e": [],
        "u": "7",
        "tags": [
          "pre_un"
        ]
      },
      {
        "w": "knowledge",
        "p": "/'nɒlɪdʒ/",
        "t": "n.",
        "m": "知识",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "ability",
        "p": "/ə'bɪləti/",
        "t": "n.",
        "m": "能力",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "grow",
        "p": "/ɡrəʊ/",
        "t": "v.",
        "m": "增长; 成长",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "development",
        "p": "/dɪ'veləpmənt/",
        "t": "n.",
        "m": "发展",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "progress",
        "p": "/'prəʊɡres/",
        "t": "n.",
        "m": "进步",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "future",
        "p": "/fjuːtʃə(r)/",
        "t": "n.",
        "m": "未来",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "society",
        "p": "/sə'saɪəti/",
        "t": "n.",
        "m": "社会",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "individual",
        "p": "/ˌɪndɪ'vɪdʒuəl/",
        "t": "n.",
        "m": "个人",
        "e": [],
        "u": "7",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "well-respected",
        "p": "/wel rɪ'spektɪd/",
        "t": "adj.",
        "m": "受人尊敬的",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "found",
        "p": "/faʊnd/",
        "t": "v.",
        "m": "建立; 创立",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "mission",
        "p": "/'mɪʃn/",
        "t": "n.",
        "m": "使命; 任务",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "honour",
        "p": "/'ɒnə(r)/",
        "t": "n.",
        "m": "荣誉; 尊敬",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "approval",
        "p": "/ə'pruːvl/",
        "t": "n.",
        "m": "赞成; 认可",
        "e": [],
        "u": "7",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "prize",
        "p": "/praɪz/",
        "t": "n.",
        "m": "奖品; 奖金",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "beauty",
        "p": "/'bjuːti/",
        "t": "n.",
        "m": "美丽; 美",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "female",
        "p": "/'fiːmeɪl/",
        "t": "adj.",
        "m": "女性的",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "admire",
        "p": "/əd'maɪə(r)/",
        "t": "v.",
        "m": "钦佩; 羡慕",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "inspire",
        "p": "/ɪn'spaɪə(r)/",
        "t": "v.",
        "m": "鼓舞; 激发",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "regular",
        "p": "/'reɡjələ(r)/",
        "t": "adj.",
        "m": "规则的; 定期的",
        "e": [],
        "u": "7",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "normal",
        "p": "/'nɔːml/",
        "t": "adj.",
        "m": "正常的; 普通的",
        "e": [],
        "u": "7",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "simple",
        "p": "/'sɪmpl/",
        "t": "adj.",
        "m": "简单的",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "easy",
        "p": "/'iːzi/",
        "t": "adj.",
        "m": "容易的",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "often",
        "p": "/'ɒfn/",
        "t": "adv.",
        "m": "经常",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "usually",
        "p": "/'juːʒuəli/",
        "t": "adv.",
        "m": "通常",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "sometimes",
        "p": "/'sʌmtaɪmz/",
        "t": "adv.",
        "m": "有时",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "always",
        "p": "/'ɔːlweɪz/",
        "t": "adv.",
        "m": "总是",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "never",
        "p": "/'nevə(r)/",
        "t": "adv.",
        "m": "从不",
        "e": [],
        "u": "7",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "maybe",
        "p": "/'meibi/",
        "t": "adv.",
        "m": "也许",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "perhaps",
        "p": "/pə'hæps/",
        "t": "adv.",
        "m": "也许",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "really",
        "p": "/'riːəli/",
        "t": "adv.",
        "m": "真正地",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly",
          "pre_re"
        ]
      },
      {
        "w": "indeed",
        "p": "/ɪn'diːd/",
        "t": "adv.",
        "m": "的确; 确实",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "certainly",
        "p": "/'sɜːtnli/",
        "t": "adv.",
        "m": "当然; 肯定",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "surely",
        "p": "/'ʃʊəli/",
        "t": "adv.",
        "m": "无疑地; 肯定地",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "definitely",
        "p": "/'defɪnətli/",
        "t": "adv.",
        "m": "肯定地; 明确地",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "probably",
        "p": "/'prɒbəbli/",
        "t": "adv.",
        "m": "很可能",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "possibly",
        "p": "/'pɒsəbli/",
        "t": "adv.",
        "m": "可能地",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "extremely",
        "p": "/ɪk'striːmli/",
        "t": "adv.",
        "m": "极端地; 非常",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "very",
        "p": "/'veri/",
        "t": "adv.",
        "m": "非常",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "quite",
        "p": "/kwaɪt/",
        "t": "adv.",
        "m": "相当; 相当地",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "a bit",
        "p": "",
        "t": "phrase",
        "m": "一点点",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "slightly",
        "p": "/'slaɪtli/",
        "t": "adv.",
        "m": "轻微地",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "moderately",
        "p": "/'mɒdərətli/",
        "t": "adv.",
        "m": "适度地",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "somewhat",
        "p": "/'sʌmwɒt/",
        "t": "adv.",
        "m": "稍微; 有点",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "rather",
        "p": "/'rɑːðə(r)/",
        "t": "adv.",
        "m": "相当; 颇",
        "e": [],
        "u": "7",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "fairly",
        "p": "/'feəli/",
        "t": "adv.",
        "m": "相当地",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "pretty",
        "p": "/'prɪti/",
        "t": "adv.",
        "m": "相当; 很",
        "e": [],
        "u": "7",
        "tags": [
          "pre_pre"
        ]
      },
      {
        "w": "role model",
        "p": "/rəʊl 'mɒdl/",
        "t": "n. phr.",
        "m": "榜样; 模范",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "in the field of",
        "p": "",
        "t": "phrase",
        "m": "在......领域",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "devote oneself to",
        "p": "",
        "t": "v. phr.",
        "m": "献身于; 致力于",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "college entrance examination",
        "p": "",
        "t": "n. phr.",
        "m": "大学入学考试",
        "e": [],
        "u": "7",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "look up to",
        "p": "",
        "t": "v. phr.",
        "m": "敬仰; 钦佩",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "sugar pill",
        "p": "",
        "t": "n. phr.",
        "m": "糖丸",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "chief engineer",
        "p": "",
        "t": "n. phr.",
        "m": "总工程师",
        "e": [],
        "u": "7",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "put out",
        "p": "",
        "t": "v. phr.",
        "m": "熄灭; 扑灭",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "be able to",
        "p": "",
        "t": "v. phr.",
        "m": "能够",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "be proud of",
        "p": "",
        "t": "v. phr.",
        "m": "为......而自豪",
        "e": [],
        "u": "7",
        "tags": [
          "phr_be_of"
        ]
      }
    ],
    "8": [
      {
        "w": "possible",
        "p": "/'pɒsəbl/",
        "t": "adj.",
        "m": "可能的",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "athlete",
        "p": "/'æθliːt/",
        "t": "n.",
        "m": "运动员",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "biologist",
        "p": "/baɪ'ɒlədʒɪst/",
        "t": "n.",
        "m": "生物学家",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "instrument",
        "p": "/'ɪnstrəmənt/",
        "t": "n.",
        "m": "乐器; 仪器",
        "e": [],
        "u": "8",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "interest",
        "p": "/'ɪntrəst/",
        "t": "n.",
        "m": "兴趣; 爱好",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "career",
        "p": "/kə'rɪə(r)/",
        "t": "n.",
        "m": "职业",
        "e": [],
        "u": "8",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "lifetime",
        "p": "/'laɪftaɪm/",
        "t": "n.",
        "m": "终身; 一生",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "diamond",
        "p": "/'daɪəmənd/",
        "t": "n.",
        "m": "钻石",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "belt",
        "p": "/belt/",
        "t": "n.",
        "m": "腰带",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "shoot",
        "p": "/ʃuːt/",
        "t": "v.",
        "m": "冲; 飞驰",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "extremely",
        "p": "/ɪk'striːmli/",
        "t": "adv.",
        "m": "极其; 非常",
        "e": [],
        "u": "8",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "curious",
        "p": "/'kjʊəriəs/",
        "t": "adj.",
        "m": "求知欲强的; 好奇的",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "increase",
        "p": "/ɪn'kriːs/",
        "t": "v.",
        "m": "增长; 增多; 增加",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "host",
        "p": "/həʊst/",
        "t": "v.",
        "m": "主持",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "beyond",
        "p": "/bɪ'jɒnd/",
        "t": "prep.",
        "m": "在......另一边; (在)更远处",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "audience",
        "p": "/'ɔːdiəns/",
        "t": "n.",
        "m": "观众; 听众",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "actuality",
        "p": "/ˌæktʃu'æləti/",
        "t": "n.",
        "m": "事实",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "sail",
        "p": "/seɪl/",
        "t": "v.",
        "m": "驾驶 (或乘坐) 帆船航行",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "ability",
        "p": "/ə'bɪləti/",
        "t": "n.",
        "m": "才能; 本领",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "review",
        "p": "/rɪ'vjuː/",
        "t": "n.",
        "m": "评论",
        "e": [],
        "u": "8",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "perform",
        "p": "/pə'fɔːm/",
        "t": "v.",
        "m": "演出; 表演",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "kiss",
        "p": "/kɪs/",
        "t": "n.",
        "m": "吻",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "hug",
        "p": "/hʌɡ/",
        "t": "v.",
        "m": "拥抱; 搂抱",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "method",
        "p": "/'meθəd/",
        "t": "n.",
        "m": "方法; 办法",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "compare",
        "p": "/kəm'peə(r)/",
        "t": "v.",
        "m": "将......比作",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "lecture",
        "p": "/'lektʃə(r)/",
        "t": "n.",
        "m": "讲座; 讲课; 演讲",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "go outside",
        "p": "",
        "t": "v. phr.",
        "m": "外出",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "more and more",
        "p": "",
        "t": "phrase",
        "m": "越来越多",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "body language",
        "p": "",
        "t": "n. phr.",
        "m": "肢体语言",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "dream of",
        "p": "",
        "t": "v. phr.",
        "m": "梦想",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "come true",
        "p": "",
        "t": "v. phr.",
        "m": "实现; 成为现实",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "nowadays",
        "p": "/'naʊədeɪz/",
        "t": "adv.",
        "m": "现今; 目前",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "human being",
        "p": "/'hjuːmən ˌbiːɪŋ/",
        "t": "n.",
        "m": "人",
        "e": [],
        "u": "8",
        "tags": []
      }
    ]
  },
  "8A": [
    {
      "w": "dinosaur",
      "p": "/'daɪnəsɔː(r)/",
      "t": "n.",
      "m": "恐龙",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "intelligent",
      "p": "/ɪn'telɪdʒənt/",
      "t": "adj.",
      "m": "有才智的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "talented",
      "p": "/'tæləntɪd/",
      "t": "adj.",
      "m": "有才能的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "artistic",
      "p": "/ɑː'tɪstɪk/",
      "t": "(尤指)有艺术天赋的",
      "m": "有艺术天赋的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "perhaps",
      "p": "/pə'hæps/",
      "t": "adv.",
      "m": "可能；大概；也许",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "notebook",
      "p": "/'nəʊtbʊk/",
      "t": "n.",
      "m": "笔记本",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "vehicle",
      "p": "/'viːɪkl/",
      "t": "n.",
      "m": "交通工具",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "prehistoric",
      "p": "/pri:'hɪstɒrɪk/",
      "t": "adj.",
      "m": "史前的",
      "e": [],
      "u": "1",
      "tags": [
        "pre_pre"
      ]
    },
    {
      "w": "completely",
      "p": "/kəm'pliːtli/",
      "t": "adv.",
      "m": "完全地；彻底地",
      "e": [],
      "u": "1",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "original",
      "p": "/ə'rɪdʒənl/",
      "t": "adj.",
      "m": "原来的；起初的",
      "e": [],
      "u": "1",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "birth",
      "p": "/bɜːθ/",
      "t": "n.",
      "m": "出生",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "suffering",
      "p": "/'sʌfərɪŋ/",
      "t": "n.",
      "m": "苦难；疼痛",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "artist",
      "p": "/'ɑːtɪst/",
      "t": "n.",
      "m": "艺术家；(尤指)画家",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "death",
      "p": "/deθ/",
      "t": "n.",
      "m": "死亡",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "whole",
      "p": "/həʊl/",
      "t": "adj.",
      "m": "全部的；所有的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "piece",
      "p": "/piːs/",
      "t": "n.",
      "m": "一首，一篇 (作品)",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "editor",
      "p": "/'edɪtə(r)/",
      "t": "n.",
      "m": "(书籍的) 编辑",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "organize",
      "p": "/'ɔːɡənaɪz/",
      "t": "v.",
      "m": "安排；组织",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "record",
      "p": "/rɪ'kɔːd/",
      "t": "v.",
      "m": "记录",
      "e": [],
      "u": "1",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "die out",
      "p": "",
      "t": "v. phr.",
      "m": "灭绝",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "a type of",
      "p": "",
      "t": "phrase",
      "m": "一种",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "be related to",
      "p": "",
      "t": "phrase",
      "m": "与……属于同一种类",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "flight",
      "p": "/flaɪt/",
      "t": "n.",
      "m": "旅程 (尤指飞机的)",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "schedule",
      "p": "/'ʃedjuːl/",
      "t": "n.",
      "m": "日程安排",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "everywhere",
      "p": "/'evriweə(r)/",
      "t": "adv.",
      "m": "到处; 各个地方",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "challenge",
      "p": "/'tʃælɪndʒ/",
      "t": "v.",
      "m": "向(某人)挑战",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "prize",
      "p": "/praɪz/",
      "t": "n.",
      "m": "奖；奖励",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "promise",
      "p": "/'prɒmɪs/",
      "t": "v.",
      "m": "承诺；保证",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "chessboard",
      "p": "/'tʃesbɔːd/",
      "t": "n.",
      "m": "国际象棋棋盘",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "silver",
      "p": "/'sɪlvə(r)/",
      "t": "n.",
      "m": "银；回复",
      "e": [],
      "u": "2",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "reply",
      "p": "/rɪ'plaɪ/",
      "t": "v.",
      "m": "回答；答复",
      "e": [],
      "u": "2",
      "tags": [
        "suf_ly",
        "pre_re"
      ]
    },
    {
      "w": "hesitation",
      "p": "/ˌhezɪ'teɪʃn/",
      "t": "n.",
      "m": "犹豫",
      "e": [],
      "u": "2",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "wonder",
      "p": "/'wʌndə(r)/",
      "t": "v.",
      "m": "想知道；琢磨",
      "e": [],
      "u": "2",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "agree",
      "p": "/ə'ɡriː/",
      "t": "v.",
      "m": "同意；赞成",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "per cent",
      "p": "/pə 'sent/",
      "t": "n.",
      "m": "百分之...",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "currently",
      "p": "/'kʌrəntli/",
      "t": "adv.",
      "m": "当前；目前",
      "e": [],
      "u": "2",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "check",
      "p": "/tʃek/",
      "t": "v.",
      "m": "检查；查验",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "budget",
      "p": "/'bʌdʒɪt/",
      "t": "n.",
      "m": "预算",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "province",
      "p": "/'prɒvɪns/",
      "t": "n.",
      "m": "省；省份",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "sharply",
      "p": "/'ʃɑːpli/",
      "t": "adv.",
      "m": "急剧地；突然地",
      "e": [],
      "u": "2",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "count",
      "p": "/kaʊnt/",
      "t": "v.",
      "m": "计算 (或清点) 总数",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "system",
      "p": "/'sɪstəm/",
      "t": "n.",
      "m": "系统",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "symbol",
      "p": "/'sɪmbl/",
      "t": "n.",
      "m": "符号；记号",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "represent",
      "p": "/ˌreprɪ'zent/",
      "t": "v.",
      "m": "代表",
      "e": [],
      "u": "2",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "exactly",
      "p": "/ɪɡ'zæktli/",
      "t": "adv.",
      "m": "准确地；确切地",
      "e": [],
      "u": "2",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "flight schedule",
      "p": "",
      "t": "n. phr.",
      "m": "航班时刻表",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "price tag",
      "p": "",
      "t": "n. phr.",
      "m": "价格标签",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "for a moment",
      "p": "",
      "t": "phrase",
      "m": "片刻；一会儿",
      "e": [],
      "u": "2",
      "tags": [
        "suf_ment"
      ]
    },
    {
      "w": "without hesitation",
      "p": "",
      "t": "phrase",
      "m": "毫不犹豫",
      "e": [],
      "u": "2",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "go up",
      "p": "",
      "t": "v. phr.",
      "m": "上升",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "go down",
      "p": "",
      "t": "v. phr.",
      "m": "下降",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "write down",
      "p": "",
      "t": "v. phr.",
      "m": "写下；记下",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "instead of",
      "p": "",
      "t": "prep.",
      "m": "代替；作为......的替换",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "network",
      "p": "/'netwɜːk/",
      "t": "n.",
      "m": "网络",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "flood",
      "p": "/flʌd/",
      "t": "n.",
      "m": "洪水",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "multimedia",
      "p": "/ˌmʌlti'miːdiə/",
      "t": "n.",
      "m": "多媒体",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "expert",
      "p": "/'ekspɜːt/",
      "t": "n.",
      "m": "专家",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "mobile",
      "p": "/'məʊbaɪl/",
      "t": "adj.",
      "m": "可移动的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "payment",
      "p": "/'peɪmənt/",
      "t": "n.",
      "m": "付款；支付",
      "e": [],
      "u": "3",
      "tags": [
        "suf_ment"
      ]
    },
    {
      "w": "warm",
      "p": "/wɔːm/",
      "t": "v.",
      "m": "提醒；注意；警告",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "treatment",
      "p": "/'triːtmənt/",
      "t": "n.",
      "m": "疗法",
      "e": [],
      "u": "3",
      "tags": [
        "suf_ment"
      ]
    },
    {
      "w": "data",
      "p": "/'deɪtə/",
      "t": "n.",
      "m": "数据",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "company",
      "p": "/'kʌmpəni/",
      "t": "n.",
      "m": "公司",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "traffic",
      "p": "/'træfɪk/",
      "t": "n.",
      "m": "路上行驶的车辆；交通",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "flow",
      "p": "/fləʊ/",
      "t": "v.",
      "m": "流动；流",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "smoothly",
      "p": "/'smuːðli/",
      "t": "adv.",
      "m": "平稳地；连续而流畅地",
      "e": [],
      "u": "3",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "laptop",
      "p": "/'læptɒp/",
      "t": "n.",
      "m": "笔记本电脑",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "screen",
      "p": "/skriːn/",
      "t": "n.",
      "m": "屏幕",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "weight",
      "p": "/weɪt/",
      "t": "n.",
      "m": "重量",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "digital",
      "p": "/'dɪdʒɪtl/",
      "t": "adj.",
      "m": "数码的",
      "e": [],
      "u": "3",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "social",
      "p": "/'səʊʃl/",
      "t": "adj.",
      "m": "社交的",
      "e": [],
      "u": "3",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "message",
      "p": "/'mesɪdʒ/",
      "t": "n.",
      "m": "(书面或口头的) 信息",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "interview",
      "p": "/'ɪntəvjuː/",
      "t": "v.",
      "m": "采访",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "positive",
      "p": "/'pɒzətɪv/",
      "t": "adj.",
      "m": "积极的；正面的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "negative",
      "p": "/'neɡətɪv/",
      "t": "adj.",
      "m": "消极的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "effect",
      "p": "/ɪ'fekt/",
      "t": "n.",
      "m": "影响",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "opinion",
      "p": "/ə'pɪnjən/",
      "t": "n.",
      "m": "意见；看法",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "novel",
      "p": "/'nɒvl/",
      "t": "n.",
      "m": "(长篇) 小说",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "comment",
      "p": "/'kɒment/",
      "t": "n.",
      "m": "评论",
      "e": [],
      "u": "3",
      "tags": [
        "suf_ment"
      ]
    },
    {
      "w": "basis",
      "p": "/'beɪsɪs/",
      "t": "n.",
      "m": "基础",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "microprocessor",
      "p": "/ˌmaɪkrəʊ'prəʊsesə(r)/",
      "t": "n.",
      "m": "微处理器",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "microchip",
      "p": "/'maɪkrəʊtʃɪp/",
      "t": "n.",
      "m": "微芯片；芯片",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "major",
      "p": "/'meɪdʒə(r)/",
      "t": "adj.",
      "m": "主要的；重要的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "breakthrough",
      "p": "/'breɪkθruː/",
      "t": "n.",
      "m": "重大进展",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "electronic",
      "p": "/ɪˌlek'trɒnɪk/",
      "t": "adj.",
      "m": "电子的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "software",
      "p": "/'sɒftweə(r)/",
      "t": "n.",
      "m": "软件；应用程序",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "era",
      "p": "/'ɪərə/",
      "t": "n.",
      "m": "时代；纪元",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "download",
      "p": "/ˌdaʊn'ləʊd/",
      "t": "v.",
      "m": "下载",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "tiny",
      "p": "/'taɪni/",
      "t": "adj.",
      "m": "微小的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "connect to",
      "p": "",
      "t": "v. phr.",
      "m": "连接",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "bring big changes to",
      "p": "",
      "t": "v. phr.",
      "m": "带来重大变化",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "mobile payment",
      "p": "",
      "t": "n. phr.",
      "m": "移动支付",
      "e": [],
      "u": "3",
      "tags": [
        "suf_ment"
      ]
    },
    {
      "w": "take ... for example",
      "p": "",
      "t": "phrase",
      "m": "以......为例",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "rubbish bin",
      "p": "",
      "t": "n. phr.",
      "m": "垃圾箱",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "social media",
      "p": "",
      "t": "n. phr.",
      "m": "社交媒体",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "in person",
      "p": "",
      "t": "adv. phr.",
      "m": "亲自；亲身",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "the general public",
      "p": "",
      "t": "n. phr.",
      "m": "公众；大众",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "balloon",
      "p": "/bə'luːn/",
      "t": "n.",
      "m": "气球",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "wheel",
      "p": "/wiːl/",
      "t": "n.",
      "m": "轮；轮车",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "central",
      "p": "/'sentrəl/",
      "t": "adj.",
      "m": "中心的",
      "e": [],
      "u": "4",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "although",
      "p": "/ɔːl'ðəʊ/",
      "t": "conj.",
      "m": "虽然；尽管；即使",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "attach",
      "p": "/ə'tætʃ/",
      "t": "v.",
      "m": "固定；附上 (在...上)",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "pull",
      "p": "/pʊl/",
      "t": "v.",
      "m": "拉；拖；扯",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "international",
      "p": "/ˌɪntə'næʃnəl/",
      "t": "adj.",
      "m": "国际的",
      "e": [],
      "u": "4",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "path",
      "p": "/pɑːθ/",
      "t": "n.",
      "m": "小路；小径",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "technique",
      "p": "/tek'niːk/",
      "t": "n.",
      "m": "技巧；艺技",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "depend",
      "p": "/dɪ'pend/",
      "t": "v.",
      "m": "需要；依靠",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "doubt",
      "p": "/daʊt/",
      "t": "n.",
      "m": "疑惑",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "personally",
      "p": "/'pɜːsənəli/",
      "t": "adv.",
      "m": "个人；就个人而言",
      "e": [],
      "u": "4",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "prediction",
      "p": "/prɪ'dɪkʃn/",
      "t": "n.",
      "m": "预测；预言",
      "e": [],
      "u": "4",
      "tags": [
        "suf_tion",
        "pre_pre"
      ]
    },
    {
      "w": "statement",
      "p": "/'steɪtmənt/",
      "t": "n.",
      "m": "说明",
      "e": [],
      "u": "4",
      "tags": [
        "suf_ment"
      ]
    },
    {
      "w": "benefit",
      "p": "/'benɪfɪt/",
      "t": "n.",
      "m": "益处；优势",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "wing",
      "p": "/wɪŋ/",
      "t": "n.",
      "m": "(飞航器的) 翅膀；机翼",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "distance",
      "p": "/'dɪstəns/",
      "t": "n.",
      "m": "距离",
      "e": [],
      "u": "4",
      "tags": [
        "pre_dis"
      ]
    },
    {
      "w": "petrol",
      "p": "/'petrəl/",
      "t": "n.",
      "m": "汽油",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "avoid",
      "p": "/ə'vɔɪd/",
      "t": "v.",
      "m": "避免；防止",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "take place",
      "p": "",
      "t": "v. phr.",
      "m": "发生；进行",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "for instance",
      "p": "",
      "t": "phrase",
      "m": "例如；比如",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "large amounts of",
      "p": "",
      "t": "phrase",
      "m": "大量的",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "international trade",
      "p": "",
      "t": "n. phr.",
      "m": "国际贸易",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "depend on",
      "p": "",
      "t": "v. phr.",
      "m": "依靠；依赖",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "without doubt",
      "p": "",
      "t": "phrase",
      "m": "毫无疑问",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "of all time",
      "p": "",
      "t": "phrase",
      "m": "自古以来；有史以来",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "make fun of",
      "p": "",
      "t": "v. phr.",
      "m": "取笑",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "traffic jam",
      "p": "",
      "t": "n. phr.",
      "m": "交通阻塞",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "exchange",
      "p": "/ɪks'tʃeɪndʒ/",
      "t": "v.",
      "m": "交流",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "nervous",
      "p": "/'nɜːvəs/",
      "t": "adj.",
      "m": "焦虑的；担心的",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "grateful",
      "p": "/'ɡreɪtfəl/",
      "t": "adj.",
      "m": "感激的",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "chopstick",
      "p": "/'tʃɒpstɪk/",
      "t": "n.",
      "m": "筷子",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "tour",
      "p": "/tʊə(r)/",
      "t": "n.",
      "m": "旅行；旅游",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "chi",
      "p": "/tʃi/",
      "t": "n.",
      "m": "太极",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "yet",
      "p": "/jet/",
      "t": "adv.",
      "m": "尚 (未)；还",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "independent",
      "p": "/ˌɪndɪ'pendənt/",
      "t": "adj.",
      "m": "自主的",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "content",
      "p": "/'kɒntent/",
      "t": "n.",
      "m": "内容",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "feeling",
      "p": "/'fiːlɪŋ/",
      "t": "n.",
      "m": "感觉；感情",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "foreign",
      "p": "/'fɒrən/",
      "t": "adj.",
      "m": "外国的",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "confused",
      "p": "/kən'fjuːzd/",
      "t": "adj.",
      "m": "糊涂的；迷惑的",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "anxious",
      "p": "/'æŋkʃəs/",
      "t": "adj.",
      "m": "焦虑的；担心的",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "phase",
      "p": "/feɪz/",
      "t": "n.",
      "m": "阶段；时期",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "honeymoon",
      "p": "/'hʌnimuːn/",
      "t": "n.",
      "m": "蜜月",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "unfamiliar",
      "p": "/ʌn'fæmɪliə(r)/",
      "t": "adj.",
      "m": "陌生的；不熟悉的",
      "e": [],
      "u": "5",
      "tags": [
        "pre_un"
      ]
    },
    {
      "w": "homesick",
      "p": "/'həʊmsɪk/",
      "t": "adj.",
      "m": "想家的",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "lonely",
      "p": "/'ləʊnli/",
      "t": "adj.",
      "m": "孤独的",
      "e": [],
      "u": "5",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "deal",
      "p": "/diːl/",
      "t": "v.",
      "m": "对；应付",
      "e": [],
      "u": "5",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "expect",
      "p": "/ɪk'spekt/",
      "t": "v.",
      "m": "预料；预期",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "situation",
      "p": "/sɪtʃu'eɪʃn/",
      "t": "n.",
      "m": "情况",
      "e": [],
      "u": "5",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "accept",
      "p": "/ək'sept/",
      "t": "v.",
      "m": "接受",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "adaptation",
      "p": "/ˌædæp'teɪʃn/",
      "t": "n.",
      "m": "适应",
      "e": [],
      "u": "5",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "Beijing opera",
      "p": "",
      "t": "n. phr.",
      "m": "京剧",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "host family",
      "p": "",
      "t": "n. phr.",
      "m": "寄宿家庭",
      "e": [],
      "u": "5",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "snake its way",
      "p": "",
      "t": "v. phr.",
      "m": "蜿蜒",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "culture shock",
      "p": "",
      "t": "n. phr.",
      "m": "文化冲击",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "deal with",
      "p": "",
      "t": "v. phr.",
      "m": "解决；处理",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "feel at home",
      "p": "",
      "t": "phrase",
      "m": "感到舒适自在",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "author",
      "p": "/'ɔːθə(r)/",
      "t": "n.",
      "m": "作者；作家",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "remains",
      "p": "/rɪ'meɪnz/",
      "t": "n.",
      "m": "遗迹；遗存",
      "e": [],
      "u": "6",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "locate",
      "p": "/ləʊ'keɪt/",
      "t": "v.",
      "m": "把......安置在 (或建造于)",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "soldier",
      "p": "/'səʊldʒə(r)/",
      "t": "n.",
      "m": "士兵",
      "e": [],
      "u": "6",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "captain",
      "p": "/'keptɪn/",
      "t": "n.",
      "m": "首领；领导者",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "empty",
      "p": "/'empti/",
      "t": "adj.",
      "m": "空的",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "victory",
      "p": "/'vɪktəri/",
      "t": "n.",
      "m": "胜利；成功",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "joke",
      "p": "/dʒəʊk/",
      "t": "n.",
      "m": "笑话；玩笑",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "midnight",
      "p": "/'mɪdnaɪt/",
      "t": "n.",
      "m": "午夜",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "except",
      "p": "/ɪk'sept/",
      "t": "prep.",
      "m": "除......之外",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "hide",
      "p": "/haɪd/",
      "t": "v.",
      "m": "隐藏",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "secretly",
      "p": "/'siːkrətli/",
      "t": "adv.",
      "m": "秘密地",
      "e": [],
      "u": "6",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "enter",
      "p": "/'entə(r)/",
      "t": "v.",
      "m": "来；进入",
      "e": [],
      "u": "6",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "succeed",
      "p": "/sək'siːd/",
      "t": "v.",
      "m": "达到目的；成功",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "trick",
      "p": "/trɪk/",
      "t": "n.",
      "m": "骗局",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "fight",
      "p": "/faɪt/",
      "t": "v.",
      "m": "打；战斗",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "beat",
      "p": "/biːt/",
      "t": "v.",
      "m": "打败 (某人)",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "pretend",
      "p": "/prɪ'tend/",
      "t": "v.",
      "m": "假装；伪装",
      "e": [],
      "u": "6",
      "tags": [
        "pre_pre"
      ]
    },
    {
      "w": "enemy",
      "p": "/'enəmi/",
      "t": "n.",
      "m": "敌人",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "fail",
      "p": "/feɪl/",
      "t": "v.",
      "m": "失败；未能 (做到)",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "therefore",
      "p": "/ðeə'fɔː(r)/",
      "t": "adv.",
      "m": "因此；所以",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "within",
      "p": "/wɪ'ðɪn/",
      "t": "prep.",
      "m": "在......之内",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "fill",
      "p": "/fɪl/",
      "t": "v.",
      "m": "(使) 充满；(使) 装满",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "towards",
      "p": "/tə'wɔːdz/",
      "t": "prep.",
      "m": "向；朝着",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "make jokes about",
      "p": "",
      "t": "v. phr.",
      "m": "开......的玩笑",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "succeed in",
      "p": "",
      "t": "v. phr.",
      "m": "在......方面成功",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "be tired of",
      "p": "",
      "t": "phrase",
      "m": "厌烦",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "go on board",
      "p": "",
      "t": "v. phr.",
      "m": "上船",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "be jealous of",
      "p": "",
      "t": "phrase",
      "m": "嫉妒",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "be full of",
      "p": "",
      "t": "phrase",
      "m": "充满",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "regularly",
      "p": "/'reɡjələli/",
      "t": "adv.",
      "m": "有规律地",
      "e": [],
      "u": "7",
      "tags": [
        "suf_ly",
        "pre_re"
      ]
    },
    {
      "w": "repeat",
      "p": "/rɪ'piːt/",
      "t": "v.",
      "m": "重复",
      "e": [],
      "u": "7",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "note",
      "p": "/nəʊt/",
      "t": "n.",
      "m": "笔记；记录",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "visual",
      "p": "/'vɪʒuəl/",
      "t": "adj.",
      "m": "视觉的",
      "e": [],
      "u": "7",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "mentally",
      "p": "/'mentəli/",
      "t": "adv.",
      "m": "精神上",
      "e": [],
      "u": "7",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "link",
      "p": "/lɪŋk/",
      "t": "v.",
      "m": "联系；相连",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "list",
      "p": "/lɪst/",
      "t": "n.",
      "m": "清单",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "especially",
      "p": "/ɪ'speʃəli/",
      "t": "adv.",
      "m": "尤其；特别",
      "e": [],
      "u": "7",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "diet",
      "p": "/'daɪət/",
      "t": "n.",
      "m": "日常饮食",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "maintain",
      "p": "/meɪn'teɪn/",
      "t": "v.",
      "m": "维持；保持",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "nut",
      "p": "/nʌt/",
      "t": "n.",
      "m": "坚果",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "relax",
      "p": "/rɪ'læks/",
      "t": "v.",
      "m": "放松；休息",
      "e": [],
      "u": "7",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "stressed",
      "p": "/strest/",
      "t": "adj.",
      "m": "焦虑的",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "tend",
      "p": "/tend/",
      "t": "v.",
      "m": "往往；趋向",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "normal",
      "p": "/'nɔːml/",
      "t": "adj.",
      "m": "正常的",
      "e": [],
      "u": "7",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "loss",
      "p": "/lɒs/",
      "t": "n.",
      "m": "损失；丢失",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "fork",
      "p": "/fɔːk/",
      "t": "n.",
      "m": "叉子",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "onion",
      "p": "/'ʌnjən/",
      "t": "n.",
      "m": "洋葱",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "ant",
      "p": "/ænt/",
      "t": "n.",
      "m": "蚂蚁",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "image",
      "p": "/'ɪmɪdʒ/",
      "t": "n.",
      "m": "图像",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "sense",
      "p": "/sens/",
      "t": "n.",
      "m": "感觉器官 (如视、听、嗅、味、触五感)",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "summary",
      "p": "/'sʌməri/",
      "t": "n.",
      "m": "总结；概括",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "contain",
      "p": "/kən'teɪn/",
      "t": "v.",
      "m": "包含；含有",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "particular",
      "p": "/pə'tɪkjələ(r)/",
      "t": "adj.",
      "m": "专指的；特有的",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "chemistry",
      "p": "/'kemɪstri/",
      "t": "n.",
      "m": "化学；化学组成 (或性质)",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "flash card",
      "p": "",
      "t": "n. phr.",
      "m": "闪卡",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "make a point of doing something",
      "p": "",
      "t": "phrase",
      "m": "务必做某事 (因重要或必要)",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "last but not least",
      "p": "",
      "t": "phrase",
      "m": "最后但同样重要的",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "natural disaster",
      "p": "",
      "t": "n. phr.",
      "m": "自然灾害",
      "e": [],
      "u": "7",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "figure out",
      "p": "",
      "t": "v. phr.",
      "m": "弄清楚；想出",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "attack",
      "p": "/ə'tæk/",
      "t": "v.",
      "m": "攻击；袭击",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "faithful",
      "p": "/'feɪθfl/",
      "t": "adj.",
      "m": "忠实的；忠诚的",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "hold",
      "p": "/həʊld/",
      "t": "v.",
      "m": "抱着；拿着",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "responsible",
      "p": "/rɪ'spɒnsəbl/",
      "t": "adj.",
      "m": "可信任的；负责的",
      "e": [],
      "u": "8",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "awake",
      "p": "/ə'weɪk/",
      "t": "adj.",
      "m": "醒着的",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "flat",
      "p": "/flæt/",
      "t": "n.",
      "m": "公寓",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "choice",
      "p": "/tʃɔɪs/",
      "t": "n.",
      "m": "选择",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "advise",
      "p": "/əd'vaɪz/",
      "t": "v.",
      "m": "劝告；建议",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "cause",
      "p": "/kɔːz/",
      "t": "v.",
      "m": "引起；导致",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "complain",
      "p": "/kəm'pleɪn/",
      "t": "v.",
      "m": "抱怨；投诉",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "litter",
      "p": "/'lɪtə(r)/",
      "t": "n.",
      "m": "垃圾",
      "e": [],
      "u": "8",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "relieve",
      "p": "/rɪ'liːv/",
      "t": "v.",
      "m": "缓解；减轻",
      "e": [],
      "u": "8",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "unlikely",
      "p": "/ʌn'laɪkli/",
      "t": "adj.",
      "m": "不太可能的",
      "e": [],
      "u": "8",
      "tags": [
        "suf_ly",
        "pre_un"
      ]
    },
    {
      "w": "indoors",
      "p": "/ˌɪn'dɔːz/",
      "t": "adv.",
      "m": "在室内",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "magical",
      "p": "/'mædʒɪkl/",
      "t": "adj.",
      "m": "有魔力的",
      "e": [],
      "u": "8",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "wealth",
      "p": "/welθ/",
      "t": "n.",
      "m": "财富",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "queen",
      "p": "/kwiːn/",
      "t": "n.",
      "m": "女王；王后",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "servant",
      "p": "/'sɜːvənt/",
      "t": "n.",
      "m": "仆人",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "pride",
      "p": "/praɪd/",
      "t": "n.",
      "m": "自豪；骄傲",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "relationship",
      "p": "/rɪ'leɪʃnʃɪp/",
      "t": "n.",
      "m": "关系",
      "e": [],
      "u": "8",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "grow up",
      "p": "",
      "t": "v. phr.",
      "m": "长大；成熟",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "care for",
      "p": "",
      "t": "v. phr.",
      "m": "照料；喜爱",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "in short",
      "p": "",
      "t": "phrase",
      "m": "总之；简而言之",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "have no choice but to",
      "p": "",
      "t": "phrase",
      "m": "别无选择；只能",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "run free",
      "p": "",
      "t": "v. phr.",
      "m": "自由奔跑/生活",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "lie around",
      "p": "",
      "t": "v. phr.",
      "m": "闲逛；躺在周围",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "complain about",
      "p": "",
      "t": "v. phr.",
      "m": "抱怨",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "in addition",
      "p": "",
      "t": "phrase",
      "m": "此外；另外",
      "e": [],
      "u": "8",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "catch the eye of somebody",
      "p": "",
      "t": "v. phr.",
      "m": "引起某人的注意",
      "e": [],
      "u": "8",
      "tags": []
    }
  ],
  "9A": {
    "1": [
      {
        "w": "outstanding",
        "p": "/aʊt'stændɪŋ/",
        "t": "adj.",
        "m": "优秀的; 杰出的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "old-fashioned",
        "p": "/əʊld-'fæʃnd/",
        "t": "adj.",
        "m": "陈旧的; 过时的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "classical",
        "p": "/'klæsɪkl/",
        "t": "adj.",
        "m": "古典的; 经典的",
        "e": [],
        "u": "1",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "ordinary",
        "p": "/'ɔːdnri/",
        "t": "adj.",
        "m": "普通的; 平凡的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "lamp",
        "p": "/læmp/",
        "t": "n.",
        "m": "灯",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "poet",
        "p": "/'pəʊɪt/",
        "t": "n.",
        "m": "诗人",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "convinced",
        "p": "/kən'vɪnst/",
        "t": "adj.",
        "m": "坚信的; 确信的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "philosopher",
        "p": "/fə'lɒsəfə(r)/",
        "t": "n.",
        "m": "哲学家",
        "e": [],
        "u": "1",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "dramatist",
        "p": "/'dræmətɪst/",
        "t": "n.",
        "m": "作家",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "judgement",
        "p": "/'dʒʌdʒmənt/",
        "t": "n.",
        "m": "法则; 评价",
        "e": [],
        "u": "1",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "evaluation",
        "p": "/ɪ,vælju'eɪʃn/",
        "t": "n.",
        "m": "评估",
        "e": [],
        "u": "1",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "consider",
        "p": "/kən'sɪdə(r)/",
        "t": "v.",
        "m": "认为; 觉得",
        "e": [],
        "u": "1",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "background",
        "p": "/'bækɡraʊnd/",
        "t": "n.",
        "m": "背景",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "significant",
        "p": "/sɪɡ'nɪfɪkənt/",
        "t": "adj.",
        "m": "重大的; 显著的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "milestone",
        "p": "/'maɪlstəʊn/",
        "t": "n.",
        "m": "重要事件; 里程碑",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "manage",
        "p": "/'mænɪdʒ/",
        "t": "v.",
        "m": "完成 (困难的事)",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "impact",
        "p": "/'ɪmpækt/",
        "t": "n.",
        "m": "巨大影响; 强大作用",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "truth",
        "p": "/truːθ/",
        "t": "n.",
        "m": "真理",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "concentrated",
        "p": "/'kɒnsntreɪtɪd/",
        "t": "adj.",
        "m": "全神贯注的; 全力以赴的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "belief",
        "p": "/bɪ'liːf/",
        "t": "n.",
        "m": "相信",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "with something in mind",
        "p": "",
        "t": "phrase",
        "m": "考虑到......",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "in addition",
        "p": "",
        "t": "phrase",
        "m": "除......之外",
        "e": [],
        "u": "1",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "be fond of",
        "p": "",
        "t": "phrase",
        "m": "喜爱",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "reach for",
        "p": "",
        "t": "phrase",
        "m": "伸手去拿",
        "e": [],
        "u": "1",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "fight for",
        "p": "",
        "t": "phrase",
        "m": "为......作斗争",
        "e": [],
        "u": "1",
        "tags": []
      }
    ],
    "2": [
      {
        "w": "firm",
        "p": "/fɜːm/",
        "t": "adj.",
        "m": "牢固的；坚硬的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "ignorance",
        "p": "/'ɪɡnərəns/",
        "t": "n.",
        "m": "无知",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "application",
        "p": "/ˌæplɪ'keɪʃn/",
        "t": "n.",
        "m": "(尤指理论、发现等的)应用",
        "e": [],
        "u": "2",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "fancy",
        "p": "/'fænsi/",
        "t": "adj.",
        "m": "精致的；绚丽的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "satisfied",
        "p": "/'sætɪsfaɪd/",
        "t": "adj.",
        "m": "满意的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "prince",
        "p": "/prɪns/",
        "t": "n.",
        "m": "王子",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "displace",
        "p": "/dɪ'spleɪs/",
        "t": "v.",
        "m": "排挤；取代",
        "e": [],
        "u": "2",
        "tags": [
          "pre_dis"
        ]
      },
      {
        "w": "fool",
        "p": "/fuːl/",
        "t": "v.",
        "m": "欺骗；愚弄",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "prison",
        "p": "/'prɪzn/",
        "t": "n.",
        "m": "监狱",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "wire",
        "p": "/'waɪə(r)/",
        "t": "n.",
        "m": "金属丝；电线",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "prove",
        "p": "/pruːv/",
        "t": "v.",
        "m": "证明；证实",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "hardly",
        "p": "/'hɑːdli/",
        "t": "adv.",
        "m": "几乎没有",
        "e": [],
        "u": "2",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "balanced",
        "p": "/'bælənst/",
        "t": "adj.",
        "m": "保持(或显示)平衡的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "reject",
        "p": "/rɪ'dʒekt/",
        "t": "v.",
        "m": "拒绝；不接受",
        "e": [],
        "u": "2",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "thus",
        "p": "/ðʌs/",
        "t": "adv.",
        "m": "因此；如此",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "be satisfied with",
        "p": "",
        "t": "v. phr.",
        "m": "对......感到满意",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "fill ... with",
        "p": "",
        "t": "v. phr.",
        "m": "充满；装满",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "run over",
        "p": "",
        "t": "v. phr.",
        "m": "溢出",
        "e": [],
        "u": "2",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "go straight to",
        "p": "",
        "t": "v. phr.",
        "m": "直奔",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "lightning rod",
        "p": "",
        "t": "n. phr.",
        "m": "避雷针",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "as a whole",
        "p": "",
        "t": "phrase",
        "m": "作为一个整体；总体上",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "get something across",
        "p": "",
        "t": "v. phr.",
        "m": "把某事讲清楚",
        "e": [],
        "u": "2",
        "tags": []
      }
    ],
    "3": [
      {
        "w": "indeed",
        "p": "/ɪn'diːd/",
        "t": "adv.",
        "m": "确实 (强调肯定的陈述或答复)",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "match",
        "p": "/mætʃ/",
        "t": "n.",
        "m": "火柴",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "shame",
        "p": "/ʃeɪm/",
        "t": "n.",
        "m": "令人惋惜的事；令人遗憾的事",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "remove",
        "p": "/rɪ'muːv/",
        "t": "v.",
        "m": "去掉",
        "e": [],
        "u": "3",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "lower",
        "p": "/'ləʊə(r)/",
        "t": "v.",
        "m": "把......降低",
        "e": [],
        "u": "3",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "practice",
        "p": "/'præktɪs/",
        "t": "n.",
        "m": "实践；实际行动",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "decision",
        "p": "/dɪ'sɪʒn/",
        "t": "n.",
        "m": "决定",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "cheat",
        "p": "/tʃiːt/",
        "t": "v.",
        "m": "欺骗；作弊",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "strengthen",
        "p": "/'streŋθn/",
        "t": "v.",
        "m": "加强；增强；巩固",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "*fold",
        "p": "/fəʊld/",
        "t": "v.",
        "m": "折叠",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "repetition",
        "p": "/ˌrepəˈtɪʃn/",
        "t": "n.",
        "m": "重复",
        "e": [],
        "u": "3",
        "tags": [
          "suf_tion",
          "pre_re"
        ]
      },
      {
        "w": "pardon",
        "p": "/'pɑːdn/",
        "t": "exclamation",
        "m": "请再说一遍",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "vacation",
        "p": "/və'keɪʃn/",
        "t": "n.",
        "m": "假期",
        "e": [],
        "u": "3",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "somehow",
        "p": "/'sʌmhaʊ/",
        "t": "adv.",
        "m": "不知为什么",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "awkward",
        "p": "/'ɔːkwəd/",
        "t": "adj.",
        "m": "令人尴尬的",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "laughter",
        "p": "/'lɑːftə(r)/",
        "t": "n.",
        "m": "笑声",
        "e": [],
        "u": "3",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "tear",
        "p": "/tɪə(r)/",
        "t": "n.",
        "m": "眼泪",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "with admiration",
        "p": "",
        "t": "phrase",
        "m": "钦佩地；赞赏地",
        "e": [],
        "u": "3",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "right and wrong",
        "p": "",
        "t": "phrase",
        "m": "是非",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "fresh in one's memory",
        "p": "",
        "t": "phrase",
        "m": "记忆犹新",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "bed sheet",
        "p": "",
        "t": "n. phr.",
        "m": "床单",
        "e": [],
        "u": "3",
        "tags": []
      }
    ],
    "4": [
      {
        "w": "manage",
        "p": "/'mænɪdʒ/",
        "t": "v.",
        "m": "明智地使用（金钱、时间、信息等）",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "pressure",
        "p": "/'preʃə(r)/",
        "t": "n.",
        "m": "心理压力；紧张",
        "e": [],
        "u": "4",
        "tags": [
          "pre_pre"
        ]
      },
      {
        "w": "addicted",
        "p": "/ə'dɪktɪd/",
        "t": "adj.",
        "m": "上瘾的",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "bullying",
        "p": "/'bʊliɪŋ/",
        "t": "n.",
        "m": "霸凌；势必作恶",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "charge",
        "p": "/tʃɑːdʒ/",
        "t": "n.",
        "m": "职责；责任",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "awful",
        "p": "/'ɔ:fl/",
        "t": "adj.",
        "m": "很坏的；极讨厌的",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "focus",
        "p": "/'fəʊkəs/",
        "t": "v.",
        "m": "集中（注意力）",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "affect",
        "p": "/ə'fekt/",
        "t": "v.",
        "m": "影响",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "hate",
        "p": "/hett/",
        "t": "v.",
        "m": "厌恶；讨厌",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "embarrassed",
        "p": "/im'bærəst/",
        "t": "adj.",
        "m": "尴尬的；羞涩的",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "mad",
        "p": "/mæd/",
        "t": "adj.",
        "m": "生气",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "mess",
        "p": "/mes/",
        "t": "v.",
        "m": "弄脏；弄乱",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "radio",
        "p": "/'reidiəʊ/",
        "t": "n.",
        "m": "广播",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "ashamed",
        "p": "/ə'ʃeɪmd/",
        "t": "adj.",
        "m": "惭愧；羞愧",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "respond",
        "p": "/rɪ'spɒnd/",
        "t": "v.",
        "m": "(口头或书面) 回答",
        "e": [],
        "u": "4",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "sympathy",
        "p": "/'sɪmpəθi/",
        "t": "n.",
        "m": "同情",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "section",
        "p": "/sek'ʃn/",
        "t": "n.",
        "m": "部分",
        "e": [],
        "u": "4",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "newspaper",
        "p": "/nju:'zeipə(r)/",
        "t": "n.",
        "m": "报纸",
        "e": [],
        "u": "4",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "operate",
        "p": "/ə'pəreɪt/",
        "t": "v.",
        "m": "运营；管理",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "helpline",
        "p": "/'helpmæn/",
        "t": "n.",
        "m": "服务热线（提供咨询和信息的电话）",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "click",
        "p": "/klɪk/",
        "t": "n.",
        "m": "(对鼠标的) 点击",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "get addicted to",
        "p": "",
        "t": "v. phr.",
        "m": "对……上瘾",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "be in charge of",
        "p": "",
        "t": "v. phr.",
        "m": "负责",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "focus on",
        "p": "",
        "t": "v. phr.",
        "m": "集中",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "drive someone mad",
        "p": "",
        "t": "v. phr.",
        "m": "使某人发狂",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "mess something up",
        "p": "",
        "t": "v. phr.",
        "m": "弄脏；弄乱",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "hit it off with somebody",
        "p": "",
        "t": "idiom",
        "m": "和某人投缘",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "fall out with somebody",
        "p": "",
        "t": "v. phr.",
        "m": "与某人闹翻",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "feel ashamed of",
        "p": "",
        "t": "v. phr.",
        "m": "感到羞愧",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "advice column",
        "p": "",
        "t": "n. phr.",
        "m": "(报纸或杂志的) 问答专栏",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "agony aunt",
        "p": "",
        "t": "n. phr.",
        "m": "(报纸或杂志的) 问答作者",
        "e": [],
        "u": "4",
        "tags": []
      }
    ],
    "5": [
      {
        "w": "various",
        "p": "/'veəriəs/",
        "t": "adj.",
        "m": "各种不同的; 各种各样的",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "lighting operator",
        "p": "",
        "t": "n. phr.",
        "m": "灯光师",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "make-up artist",
        "p": "",
        "t": "n. phr.",
        "m": "化妆师",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "sound operator",
        "p": "",
        "t": "n. phr.",
        "m": "音响师",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "quiz show",
        "p": "",
        "t": "n. phr.",
        "m": "智力竞赛节目",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "reality show",
        "p": "",
        "t": "n. phr.",
        "m": "真人秀节目",
        "e": [],
        "u": "5",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "talk show",
        "p": "",
        "t": "n. phr.",
        "m": "脱口秀",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "travel show",
        "p": "",
        "t": "n. phr.",
        "m": "旅游节目",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "TV drama",
        "p": "",
        "t": "n. phr.",
        "m": "电视连续剧",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "shine upon",
        "p": "",
        "t": "v. phr.",
        "m": "照在身上",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "pass out",
        "p": "",
        "t": "v. phr.",
        "m": "昏迷",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "time zone",
        "p": "",
        "t": "n. phr.",
        "m": "时代/时区",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "hang out",
        "p": "",
        "t": "v. phr.",
        "m": "在某处闲逛",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "film review",
        "p": "",
        "t": "n. phr.",
        "m": "影评",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "board game",
        "p": "",
        "t": "n. phr.",
        "m": "棋类游戏",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "constant",
        "p": "/'kɒnstənt/",
        "t": "n.",
        "m": "比赛者; 竞争者",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "upon",
        "p": "/ə'pɒn/",
        "t": "prep.",
        "m": "(同 on)... 上",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "zone",
        "p": "/'zəʊn/",
        "t": "n.",
        "m": "区域",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "outdoors",
        "p": "/'aʊtdɔːz/",
        "t": "adv.",
        "m": "在户外; 在野外",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "conduct",
        "p": "/kən'dækt/",
        "t": "v.",
        "m": "实施",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "concert",
        "p": "/'kɒnsət/",
        "t": "n.",
        "m": "音乐会",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "gain",
        "p": "/ɡeɪn/",
        "t": "v.",
        "m": "获得",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "recommendation",
        "p": "/ˌrekəmen'deɪʃn/",
        "t": "n.",
        "m": "推荐",
        "e": [],
        "u": "5",
        "tags": [
          "suf_tion",
          "pre_re"
        ]
      },
      {
        "w": "plot",
        "p": "/plɒt/",
        "t": "n.",
        "m": "故事情节",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "calm",
        "p": "/kɑːm/",
        "t": "v.",
        "m": "使平静",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "snowboarding",
        "p": "/'snəʊbɔːrdɪŋ/",
        "t": "n.",
        "m": "滑雪板运动",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "challenging",
        "p": "/'tʃælɪndʒɪn/",
        "t": "adj.",
        "m": "挑战性的; 考验能力的",
        "e": [],
        "u": "5",
        "tags": []
      }
    ],
    "6": [
      {
        "w": "boil",
        "p": "/bɔɪl/",
        "t": "v.",
        "m": "沸水煮 (或洗)",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "biscuit",
        "p": "/ˈbɪskɪt/",
        "t": "n.",
        "m": "饼干",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "score",
        "p": "/skɔː(r)/",
        "t": "n.",
        "m": "分数; 得分",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "tofu",
        "p": "/ˈtɔːfu/",
        "t": "n.",
        "m": "豆腐",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "stuff",
        "p": "/stʌf/",
        "t": "n.",
        "m": "东西",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "overweight",
        "p": "/ˌəʊvəˈweɪt/",
        "t": "adj.",
        "m": "超重的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "sugary",
        "p": "/ˈʃʊɡəri/",
        "t": "adj.",
        "m": "含糖的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "menu",
        "p": "/ˈmenjuː/",
        "t": "n.",
        "m": "菜单",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "cucumber",
        "p": "/kjuːˈkʌmbə(r)/",
        "t": "n.",
        "m": "黄瓜",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "carrot",
        "p": "/ˈkærət/",
        "t": "n.",
        "m": "胡萝卜",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "cabbage",
        "p": "/ˈkæbɪdʒ/",
        "t": "n.",
        "m": "甘蓝; 卷心菜",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "preference",
        "p": "/ˈprefrəns/",
        "t": "n.",
        "m": "偏爱; 爱好",
        "e": [],
        "u": "6",
        "tags": [
          "pre_pre"
        ]
      },
      {
        "w": "waiter",
        "p": "/ˈweɪtə(r)/",
        "t": "n.",
        "m": "服务员",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "sweat",
        "p": "/swet/",
        "t": "n.",
        "m": "汗水",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "soil",
        "p": "/ˈsɔɪl/",
        "t": "n.",
        "m": "土壤",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "welcoming",
        "p": "/ˈwelkəmɪŋ/",
        "t": "adj.",
        "m": "令人感到惬意的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "gentle",
        "p": "/ˈdʒentl/",
        "t": "adj.",
        "m": "温和的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "whisper",
        "p": "/ˈwɪspə(r)/",
        "t": "v.",
        "m": "低语; 私语",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "tend",
        "p": "/tend/",
        "t": "v.",
        "m": "照料; 照管",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "fizzy drink",
        "p": "",
        "t": "n. phr.",
        "m": "起泡饮料",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "fried food",
        "p": "",
        "t": "n. phr.",
        "m": "油炸食品",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "a balanced diet",
        "p": "",
        "t": "n. phr.",
        "m": "均衡的饮食",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "in general",
        "p": "",
        "t": "adv. phr.",
        "m": "通常; 大体上",
        "e": [],
        "u": "6",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "dairy product",
        "p": "",
        "t": "n. phr.",
        "m": "乳制品",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "get through",
        "p": "",
        "t": "v. phr.",
        "m": "度过; 通过",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "would rather",
        "p": "",
        "t": "v. phr.",
        "m": "宁愿; 更喜欢",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "remind somebody of...",
        "p": "",
        "t": "v. phr.",
        "m": "使某人想起 (类似的人、地方、事物等)",
        "e": [],
        "u": "6",
        "tags": [
          "pre_re"
        ]
      }
    ],
    "7": [
      {
        "w": "associate",
        "p": "/ə'səʊsieɪt/",
        "t": "v.",
        "m": "联系",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "humour",
        "p": "/'hju:mə(r)/",
        "t": "n.",
        "m": "幽默；幽默感",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "fence",
        "p": "/fens/",
        "t": "n.",
        "m": "栅栏；篱笆",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "sink",
        "p": "/sɪŋk/",
        "t": "v.",
        "m": "下沉；下降",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "yard",
        "p": "/jɑ:d/",
        "t": "n.",
        "m": "码（长度单位）",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "survey",
        "p": "/'sɜ:veɪ/",
        "t": "v.",
        "m": "查看；视察；调查",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "aloud",
        "p": "/ə'laʊd/",
        "t": "adv.",
        "m": "大声地",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "pity",
        "p": "/'pɪti/",
        "t": "n.",
        "m": "遗憾；可惜",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "gentleman",
        "p": "/'dʒentlmən/",
        "t": "n.",
        "m": "彬彬有礼的人；有教养的人",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "concern",
        "p": "/kən'sɜːn/",
        "t": "n.",
        "m": "担心；忧虑",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "sort",
        "p": "/sɔ:t/",
        "t": "n.",
        "m": "种类；类别",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "spring",
        "p": "/sprɪŋ/",
        "t": "n.",
        "m": "活力；朝气",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "*bet",
        "p": "/bet/",
        "t": "n.",
        "m": "打赌",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "afterwards",
        "p": "/'ɑːftəwədz/",
        "t": "adv.",
        "m": "后来",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "brief",
        "p": "/brɪf/",
        "t": "adj.",
        "m": "简短的；简单的",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "journalist",
        "p": "/'dʒɜ:nəlɪst/",
        "t": "n.",
        "m": "新闻记者",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "pilot",
        "p": "/'paɪlət/",
        "t": "n.",
        "m": "领航员；引水员",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "failure",
        "p": "/'feɪljə(r)/",
        "t": "n.",
        "m": "失败",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "regard",
        "p": "/rɪ'ɡɑ:d/",
        "t": "v.",
        "m": "把......视为；看待",
        "e": [],
        "u": "7",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "first-person point of view",
        "p": "",
        "t": "phrase",
        "m": "第一人称视角",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "pick up",
        "p": "",
        "t": "v. phr.",
        "m": "捡起；重新开始",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "carry on with something",
        "p": "",
        "t": "v. phr.",
        "m": "继续做某事；坚持做某事",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "go on",
        "p": "",
        "t": "v. phr.",
        "m": "继续下去",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "be particular about",
        "p": "",
        "t": "v. phr.",
        "m": "讲究；挑剔",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "get tired with a spring in one's step",
        "p": "",
        "t": "idiom",
        "m": "迈着轻快的步伐 (注：此处合并了原文表达)",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "public speaker",
        "p": "",
        "t": "n. phr.",
        "m": "发言人；演讲者",
        "e": [],
        "u": "7",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "make a fortune",
        "p": "",
        "t": "v. phr.",
        "m": "发财；赚大钱",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "pay ... back",
        "p": "",
        "t": "v. phr.",
        "m": "(向某人)还钱",
        "e": [],
        "u": "7",
        "tags": []
      }
    ],
    "8": [
      {
        "w": "attack",
        "p": "/ə'tæk/",
        "t": "v.",
        "m": "攻击；袭击",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "faithful",
        "p": "/'feɪθfl/",
        "t": "adj.",
        "m": "忠实的；忠诚的",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "hold",
        "p": "/həʊld/",
        "t": "v.",
        "m": "抱着；拿着",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "responsible",
        "p": "/rɪ'spɒnsəbl/",
        "t": "adj.",
        "m": "可信任的；负责的",
        "e": [],
        "u": "8",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "awake",
        "p": "/ə'weɪk/",
        "t": "adj.",
        "m": "醒着的",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "flat",
        "p": "/flæt/",
        "t": "n.",
        "m": "公寓",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "choice",
        "p": "/tʃɔɪs/",
        "t": "n.",
        "m": "选择",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "advise",
        "p": "/əd'vaɪz/",
        "t": "v.",
        "m": "劝告；建议",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "cause",
        "p": "/kɔːz/",
        "t": "v.",
        "m": "引起；导致",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "complain",
        "p": "/kəm'pleɪn/",
        "t": "v.",
        "m": "抱怨；投诉",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "litter",
        "p": "/'lɪtə(r)/",
        "t": "n.",
        "m": "垃圾",
        "e": [],
        "u": "8",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "relieve",
        "p": "/rɪ'liːv/",
        "t": "v.",
        "m": "缓解；减轻",
        "e": [],
        "u": "8",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "unlikely",
        "p": "/ʌn'laɪkli/",
        "t": "adj.",
        "m": "不太可能的",
        "e": [],
        "u": "8",
        "tags": [
          "suf_ly",
          "pre_un"
        ]
      },
      {
        "w": "indoors",
        "p": "/ˌɪn'dɔːz/",
        "t": "adv.",
        "m": "在室内",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "magical",
        "p": "/'mædʒɪkl/",
        "t": "adj.",
        "m": "有魔力的",
        "e": [],
        "u": "8",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "wealth",
        "p": "/welθ/",
        "t": "n.",
        "m": "财富",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "queen",
        "p": "/kwiːn/",
        "t": "n.",
        "m": "女王；王后",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "servant",
        "p": "/'sɜːvənt/",
        "t": "n.",
        "m": "仆人",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "pride",
        "p": "/praɪd/",
        "t": "n.",
        "m": "自豪；骄傲",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "relationship",
        "p": "/rɪ'leɪʃnʃɪp/",
        "t": "n.",
        "m": "关系",
        "e": [],
        "u": "8",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "grow up",
        "p": "",
        "t": "v. phr.",
        "m": "长大；成熟",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "care for",
        "p": "",
        "t": "v. phr.",
        "m": "照料；喜爱",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "in short",
        "p": "",
        "t": "phrase",
        "m": "总之；简而言之",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "have no choice but to",
        "p": "",
        "t": "phrase",
        "m": "别无选择；只能",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "run free",
        "p": "",
        "t": "v. phr.",
        "m": "自由奔跑/生活",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "lie around",
        "p": "",
        "t": "v. phr.",
        "m": "闲逛；躺在周围",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "complain about",
        "p": "",
        "t": "v. phr.",
        "m": "抱怨",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "in addition",
        "p": "",
        "t": "phrase",
        "m": "此外；另外",
        "e": [],
        "u": "8",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "catch the eye of somebody",
        "p": "",
        "t": "v. phr.",
        "m": "引起某人的注意",
        "e": [],
        "u": "8",
        "tags": []
      }
    ]
  },
  "9B": {
    "1": [
      {
        "w": "voyage",
        "p": "/ˈvɔɪədʒ/",
        "t": "n.",
        "m": "航行",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "repetition",
        "p": "/ˌrepəˈtɪʃn/",
        "t": "n.",
        "m": "重复",
        "e": [],
        "u": "1",
        "tags": [
          "suf_tion",
          "pre_re"
        ]
      },
      {
        "w": "American",
        "p": "/əˈmerɪkən/",
        "t": "adj.",
        "m": "美国的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "continent",
        "p": "/ˈkɒntɪnənt/",
        "t": "n.",
        "m": "大陆",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "route",
        "p": "/ruːt/",
        "t": "n.",
        "m": "路线",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "discovery",
        "p": "/dɪˈskʌvəri/",
        "t": "n.",
        "m": "发现",
        "e": [],
        "u": "1",
        "tags": [
          "pre_dis"
        ]
      },
      {
        "w": "rise",
        "p": "/raɪz/",
        "t": "v.",
        "m": "变得成功 (或重要大帝)",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "official",
        "p": "/əˈfɪʃl/",
        "t": "n.",
        "m": "官员",
        "e": [],
        "u": "1",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "develop",
        "p": "/dɪˈveləp/",
        "t": "v.",
        "m": "增强；加强",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "relation",
        "p": "/rɪˈleɪʃn/",
        "t": "n.",
        "m": "关系",
        "e": [],
        "u": "1",
        "tags": [
          "suf_tion",
          "pre_re"
        ]
      },
      {
        "w": "trade",
        "p": "/treɪd/",
        "t": "v.",
        "m": "交易；以物易物",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "foreign",
        "p": "/ˈfɒrən/",
        "t": "adj.",
        "m": "外国的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "fleet",
        "p": "/fliːt/",
        "t": "n.",
        "m": "船队；机群",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "Africa",
        "p": "/ˈæfrɪkə/",
        "t": "n.",
        "m": "非洲",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "nowhere",
        "p": "/ˈnəʊweə(r)/",
        "t": "adv.",
        "m": "无处；哪里都不",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "silk",
        "p": "/sɪlk/",
        "t": "n.",
        "m": "织物；丝绸",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "giraffe",
        "p": "/dʒəˈrɑːf/",
        "t": "n.",
        "m": "长颈鹿",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "besides",
        "p": "/bɪˈsaɪdz/",
        "t": "prep.",
        "m": "除...之外 (也)",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "development",
        "p": "/dɪˈveləpmənt/",
        "t": "n.",
        "m": "发展",
        "e": [],
        "u": "1",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "region",
        "p": "/ˈriːdʒən/",
        "t": "n.",
        "m": "地区",
        "e": [],
        "u": "1",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "pioneer",
        "p": "/ˌpaɪəˈnɪə(r)/",
        "t": "n.",
        "m": "先锋；先驱",
        "e": [],
        "u": "1",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "people",
        "p": "/ˈpiːpl/",
        "t": "n.",
        "m": "民族；种族",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "wealth",
        "p": "/welθ/",
        "t": "n.",
        "m": "财富",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "spread",
        "p": "/spred/",
        "t": "v.",
        "m": "传播",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "open up",
        "p": "",
        "t": "v. phr.",
        "m": "开辟",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "go on a trip",
        "p": "",
        "t": "v. phr.",
        "m": "去旅行",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "set up",
        "p": "",
        "t": "v. phr.",
        "m": "建立；设立",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "set sail",
        "p": "",
        "t": "v. phr.",
        "m": "起航",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "(be) known as",
        "p": "",
        "t": "v. phr.",
        "m": "被称为",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "as well as",
        "p": "",
        "t": "conj.",
        "m": "也；还",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "lead to",
        "p": "",
        "t": "v. phr.",
        "m": "导致",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "compare ... with ...",
        "p": "",
        "t": "v. phr.",
        "m": "把...与...对比",
        "e": [],
        "u": "1",
        "tags": []
      }
    ],
    "2": [
      {
        "w": "culture shock",
        "p": "/ˈkʌltʃə ʃɒk/",
        "t": "n. phr.",
        "m": "文化冲击",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "international",
        "p": "/ˌɪntəˈnæʃnəl/",
        "t": "adj.",
        "m": "国际的",
        "e": [],
        "u": "2",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "national",
        "p": "/ˈnæʃnəl/",
        "t": "adj.",
        "m": "国家的",
        "e": [],
        "u": "2",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "schedule",
        "p": "/ˈʃedjuːl/",
        "t": "n.",
        "m": "时间表",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "set off",
        "p": "/set ɒf/",
        "t": "v. phr.",
        "m": "出发",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "take off",
        "p": "/teɪk ɒf/",
        "t": "v. phr.",
        "m": "起飞；脱掉",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "get used to",
        "p": "/ɡet juːst tu/",
        "t": "v. phr.",
        "m": "习惯于",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "under the weather",
        "p": "/ˈʌndə ðə ˈweðə/",
        "t": "idiom",
        "m": "身体不适",
        "e": [],
        "u": "2",
        "tags": [
          "suf_er",
          "pre_un"
        ]
      }
    ],
    "3": [
      {
        "w": "concern",
        "p": "/kənˈsɜːn/",
        "t": "n./v.",
        "m": "关心；担忧",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "atmosphere",
        "p": "/ˈætməsfɪə(r)/",
        "t": "n.",
        "m": "大气；气氛",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "greenhouse effect",
        "p": "/ˈɡriːnhaʊs ɪˈfekt/",
        "t": "n. phr.",
        "m": "温室效应",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "make a difference",
        "p": "/meɪk ə ˈdɪfrəns/",
        "t": "v. phr.",
        "m": "产生影响",
        "e": [],
        "u": "3",
        "tags": []
      }
    ],
    "4": [
      {
        "w": "asteroid",
        "p": "/ˈæstərɔɪd/",
        "t": "n.",
        "m": "小行星",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "typhoon",
        "p": "/taɪˈfuːn/",
        "t": "n.",
        "m": "台风",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "natural disaster",
        "p": "/ˈnætʃrəl dɪˈzɑːstə/",
        "t": "n. phr.",
        "m": "自然灾害",
        "e": [],
        "u": "4",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "survival kit",
        "p": "/səˈvaɪvl kɪt/",
        "t": "n. phr.",
        "m": "生存工具包",
        "e": [],
        "u": "4",
        "tags": []
      }
    ],
    "5": [
      {
        "w": "announcement",
        "p": "/əˈnaʊnsmənt/",
        "t": "n.",
        "m": "公告；通知",
        "e": [],
        "u": "5",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "passport",
        "p": "/ˈpɑːspɔːt/",
        "t": "n.",
        "m": "护照",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "be dying to",
        "p": "/bi ˈdaɪɪŋ tu/",
        "t": "v. phr.",
        "m": "渴望做...",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "keep one's balance",
        "p": "/kiːp wʌnz ˈbæləns/",
        "t": "v. phr.",
        "m": "保持平衡",
        "e": [],
        "u": "5",
        "tags": []
      }
    ],
    "6": [
      {
        "w": "regular",
        "p": "/'reɡjələ(r)/",
        "t": "adj.",
        "m": "有规律的；定时的",
        "e": [],
        "u": "6",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "cheer",
        "p": "/tʃɪə(r)/",
        "t": "v.",
        "m": "鼓励；鼓舞",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "low",
        "p": "/ləʊ/",
        "t": "adj.",
        "m": "沮丧的；消沉的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "eyesight",
        "p": "/'aɪsaɪt/",
        "t": "n.",
        "m": "视力",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "dentist",
        "p": "/'dentɪst/",
        "t": "n.",
        "m": "牙科医生",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "recovery",
        "p": "/rɪ'kʌvəri/",
        "t": "n.",
        "m": "恢复；愈合",
        "e": [],
        "u": "6",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "deal with",
        "p": "",
        "t": "v. phr.",
        "m": "解决；处理；应对",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "guard against",
        "p": "",
        "t": "v. phr.",
        "m": "防止；提防",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "cancel out",
        "p": "",
        "t": "v. phr.",
        "m": "抵消；消除",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "look on the bright side",
        "p": "",
        "t": "idiom",
        "m": "持乐观的态度",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "take up",
        "p": "",
        "t": "v. phr.",
        "m": "(尤指为消遣)学着做；开始做",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "be (busy) ...ing",
        "p": "",
        "t": "v. phr.",
        "m": "忙于做......",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "leave ... behind",
        "p": "",
        "t": "v. phr.",
        "m": "把......抛在后面",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "cheer up",
        "p": "",
        "t": "v. phr.",
        "m": "(使)振奋起来",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "focus",
        "p": "/'fəʊkəs/",
        "t": "v.",
        "m": "集中 (注意力、精力等)",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "peer",
        "p": "/pɪə(r)/",
        "t": "n.",
        "m": "同龄人；同辈",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "pressure",
        "p": "/'preʃə(r)/",
        "t": "n.",
        "m": "压力",
        "e": [],
        "u": "6",
        "tags": [
          "pre_pre"
        ]
      },
      {
        "w": "whether",
        "p": "/'weðə(r)/",
        "t": "conj.",
        "m": "是否",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "risk",
        "p": "/rɪsk/",
        "t": "n.",
        "m": "风险",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "guard",
        "p": "/ɡɑːd/",
        "t": "v.",
        "m": "守卫；保卫",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "positive",
        "p": "/'pɒzətɪv/",
        "t": "adj.",
        "m": "积极乐观的；自信的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "cancel",
        "p": "/'kænsl/",
        "t": "v.",
        "m": "取消；撤销；终止",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "bright",
        "p": "/braɪt/",
        "t": "adj.",
        "m": "有希望的；明亮的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "force",
        "p": "/fɔːs/",
        "t": "v.",
        "m": "强迫；迫使",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "concert",
        "p": "/'kɒnsət/",
        "t": "n.",
        "m": "音乐会；演奏会",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "private",
        "p": "/'praɪvət/",
        "t": "adj.",
        "m": "私人的；私有的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "silent",
        "p": "/'saɪlənt/",
        "t": "adj.",
        "m": "不说话的；沉默的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "musical instrument",
        "p": "/mju'zɪkl 'ɪnstrəmənt/",
        "t": "n.",
        "m": "乐器",
        "e": [],
        "u": "6",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "enemy",
        "p": "/'enəmi/",
        "t": "n.",
        "m": "危害物；敌人",
        "e": [],
        "u": "6",
        "tags": []
      }
    ]
  },
  "8B": {
    "1": [
      {
        "w": "elderly",
        "p": "/'eldəli/",
        "t": "adj.",
        "m": "上了年纪的",
        "e": [],
        "u": "1",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "offer",
        "p": "/'ɔːfə(r)/",
        "t": "v.",
        "m": "主动提出",
        "e": [],
        "u": "1",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "serious",
        "p": "/'sɪəriəs/",
        "t": "adj.",
        "m": "严重的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "disease",
        "p": "/dɪ'ziːz/",
        "t": "n.",
        "m": "病；疾病",
        "e": [],
        "u": "1",
        "tags": [
          "pre_dis"
        ]
      },
      {
        "w": "cancer",
        "p": "/'kænsə(r)/",
        "t": "n.",
        "m": "癌症",
        "e": [],
        "u": "1",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "spirits",
        "p": "/'spɪrɪts/",
        "t": "[pl.] n.",
        "m": "情绪；心境",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "pain",
        "p": "/peɪn/",
        "t": "n.",
        "m": "痛苦；苦恼",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "belong",
        "p": "/bɪ'lɒŋ/",
        "t": "v.",
        "m": "适应；合得来",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "spare",
        "p": "/'speə(r)/",
        "t": "adj.",
        "m": "空闲的；可余下的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "volunteer",
        "p": "/ˌvɒlən'tɪə(r)/",
        "t": "v.",
        "m": "自愿做",
        "e": [],
        "u": "1",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "joy",
        "p": "/dʒɔɪ/",
        "t": "n.",
        "m": "高兴；愉快；喜悦",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "lose",
        "p": "/luːz/",
        "t": "v.",
        "m": "丧失",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "raise",
        "p": "/reɪz/",
        "t": "v.",
        "m": "筹募",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "pound",
        "p": "/paʊnd/",
        "t": "n.",
        "m": "英镑",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "structure",
        "p": "/'strʌktʃə(r)/",
        "t": "n.",
        "m": "结构",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "permission",
        "p": "/pə'mɪʃn/",
        "t": "n.",
        "m": "准许；许可",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "arrange",
        "p": "/ə'reɪndʒ/",
        "t": "v.",
        "m": "排列；布置",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "donate",
        "p": "/dəʊ'neɪt/",
        "t": "v.",
        "m": "赠送",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "mighty",
        "p": "/'maɪti/",
        "t": "adj.",
        "m": "巨大的；非凡的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "pleasant",
        "p": "/'pleznt/",
        "t": "adj.",
        "m": "宜人的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "ease",
        "p": "/iːz/",
        "t": "v.",
        "m": "减轻；缓解",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "ache",
        "p": "/eɪk/",
        "t": "n.",
        "m": "疼痛；隐痛",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "voluntary work",
        "p": "",
        "t": "phrase",
        "m": "志愿工作",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "disabled people",
        "p": "",
        "t": "phrase",
        "m": "残障人士",
        "e": [],
        "u": "1",
        "tags": [
          "pre_dis"
        ]
      },
      {
        "w": "look after",
        "p": "",
        "t": "v. phr.",
        "m": "照料；照顾",
        "e": [],
        "u": "1",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "suffer from",
        "p": "",
        "t": "v. phr.",
        "m": "遭受；患...",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "cheer somebody up",
        "p": "",
        "t": "v. phr.",
        "m": "(使)高兴起来",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "raise one's spirits",
        "p": "",
        "t": "v. phr.",
        "m": "使某人振奋",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "in low spirits",
        "p": "",
        "t": "phrase",
        "m": "情绪低落",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "in order to",
        "p": "",
        "t": "phrase",
        "m": "目的是；为了",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "in need",
        "p": "",
        "t": "phrase",
        "m": "在危难中；在穷困中的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "flower arranging",
        "p": "",
        "t": "phrase",
        "m": "插花",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "tidy up",
        "p": "",
        "t": "v. phr.",
        "m": "收拾；整理",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "in vain",
        "p": "",
        "t": "phrase",
        "m": "枉费心机；徒劳无益",
        "e": [],
        "u": "1",
        "tags": []
      }
    ],
    "2": [
      {
        "w": "shake",
        "p": "/ʃeɪk/",
        "t": "v.",
        "m": "摇动；抖动",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "clap",
        "p": "/klæp/",
        "t": "v.",
        "m": "鼓掌；拍手",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "madam",
        "p": "/'mædəm/",
        "t": "n.",
        "m": "夫人；女士",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "boss",
        "p": "/bɒs/",
        "t": "n.",
        "m": "老板",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "expression",
        "p": "/ɪk'spreʃn/",
        "t": "n.",
        "m": "表情；神色",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "impression",
        "p": "/ɪm'preʃn/",
        "t": "n.",
        "m": "印象",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "hold",
        "p": "/həʊld/",
        "t": "v.",
        "m": "保持（在某位置）",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "observe",
        "p": "/əb'zɜːv/",
        "t": "v.",
        "m": "观察；注意到",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "remind",
        "p": "/rɪ'maɪnd/",
        "t": "v.",
        "m": "提醒；使想起",
        "e": [],
        "u": "2",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "emphasize",
        "p": "/'emfəsaɪz/",
        "t": "v.",
        "m": "强调",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "nod",
        "p": "/nɒd/",
        "t": "v.",
        "m": "点头",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "meaning",
        "p": "/'miːnɪŋ/",
        "t": "n.",
        "m": "意思；意义",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "immediately",
        "p": "/ɪ'miːdiətli/",
        "t": "adv.",
        "m": "马上；立即",
        "e": [],
        "u": "2",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "shy",
        "p": "/ʃaɪ/",
        "t": "adj.",
        "m": "腼腆的；羞怯的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "case",
        "p": "/keɪs/",
        "t": "n.",
        "m": "情况；事例",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "direct",
        "p": "/də'rekt/",
        "t": "adj.",
        "m": "直接的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "truly",
        "p": "/'truːli/",
        "t": "adv.",
        "m": "真诚地",
        "e": [],
        "u": "2",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "suppose",
        "p": "/sə'pəʊz/",
        "t": "v.",
        "m": "假设；认为",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "behaviour",
        "p": "/bɪ'heɪvjə(r)/",
        "t": "n.",
        "m": "行为；举止",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "uncomfortable",
        "p": "/ʌn'kʌmftəbl/",
        "t": "adj.",
        "m": "不舒服的；局促不安的",
        "e": [],
        "u": "2",
        "tags": [
          "pre_un"
        ]
      },
      {
        "w": "hold something up",
        "p": "",
        "t": "v. phr.",
        "m": "抬起；举起",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "sit up",
        "p": "",
        "t": "v. phr.",
        "m": "坐起来；坐直",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "make a good impression on",
        "p": "",
        "t": "v. phr.",
        "m": "给……留下好印象",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "eye contact",
        "p": "",
        "t": "n. phr.",
        "m": "目光接触",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "pay attention",
        "p": "",
        "t": "v. phr.",
        "m": "注意",
        "e": [],
        "u": "2",
        "tags": [
          "suf_tion"
        ]
      }
    ],
    "3": [
      {
        "w": "performance",
        "p": "/pə'mɑːns/",
        "t": "n.",
        "m": "表演；演出",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "combine",
        "p": "/kəm'baɪn/",
        "t": "v.",
        "m": "组合；联合",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "theme",
        "p": "/θiːm/",
        "t": "n.",
        "m": "主题",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "advanced",
        "p": "/əd'vɑːnst/",
        "t": "adj.",
        "m": "先进的",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "artificial intelligence",
        "p": "/ˌɑːtɪ'fɪʃl ɪn'telɪdʒəns/",
        "t": "n. phr.",
        "m": "人工智能",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "hero",
        "p": "/'hɪərəʊ/",
        "t": "n.",
        "m": "男主角；英雄",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "classic",
        "p": "/'klæsɪk/",
        "t": "adj.",
        "m": "经典的",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "speech",
        "p": "/spiːtʃ/",
        "t": "n.",
        "m": "讲说；演说",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "text",
        "p": "/tekst/",
        "t": "n.",
        "m": "正文；文本",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "warning",
        "p": "/'wɔːnɪŋ/",
        "t": "n.",
        "m": "警告；警示",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "mind",
        "p": "/maɪnd/",
        "t": "n.",
        "m": "心；注意",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "gap",
        "p": "/ɡæp/",
        "t": "n.",
        "m": "裂口；间隙",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "crash",
        "p": "/kræʃ/",
        "t": "v.",
        "m": "碰撞；破裂",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "wave",
        "p": "/weɪv/",
        "t": "n.",
        "m": "波浪",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "folk",
        "p": "/fəʊk/",
        "t": "adj.",
        "m": "民间的；民俗的",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "risk",
        "p": "/rɪsk/",
        "t": "n.",
        "m": "危险；风险",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "leather",
        "p": "/'leðə(r)/",
        "t": "n.",
        "m": "皮革",
        "e": [],
        "u": "3",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "advantage",
        "p": "/əd'vɑːntɪdʒ/",
        "t": "n.",
        "m": "优点；优势",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "breath",
        "p": "/breθ/",
        "t": "n.",
        "m": "呼吸",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "element",
        "p": "/'elɪmənt/",
        "t": "n.",
        "m": "要素；基本部分",
        "e": [],
        "u": "3",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "shadow puppet",
        "p": "",
        "t": "n. phr.",
        "m": "皮影",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "ink-wash painting",
        "p": "",
        "t": "n. phr.",
        "m": "水墨画",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "come to life",
        "p": "",
        "t": "v. phr.",
        "m": "变得生动；活过来",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "with the help of",
        "p": "",
        "t": "phrase",
        "m": "在...的帮助下",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "comic strip",
        "p": "",
        "t": "n. phr.",
        "m": "连环漫画",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "watch out",
        "p": "",
        "t": "v. phr.",
        "m": "小心；留神",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "look out",
        "p": "",
        "t": "v. phr.",
        "m": "小心；当心",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "weather forecast",
        "p": "",
        "t": "n. phr.",
        "m": "天气预报",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "life jacket",
        "p": "",
        "t": "n. phr.",
        "m": "救生衣",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "thank goodness",
        "p": "",
        "t": "phrase",
        "m": "谢天谢地",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "make it",
        "p": "",
        "t": "v. phr.",
        "m": "侥幸做到；渡过难关",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "date back",
        "p": "",
        "t": "v. phr.",
        "m": "追溯到；始于",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "have an influence on",
        "p": "",
        "t": "v. phr.",
        "m": "对...有影响",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "at risk",
        "p": "",
        "t": "phrase",
        "m": "处在危险中",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "in the shape of",
        "p": "",
        "t": "phrase",
        "m": "以...的形状",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "take one's eyes off something",
        "p": "",
        "t": "v. phr.",
        "m": "把视线移开",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "hold one's breath",
        "p": "",
        "t": "v. phr.",
        "m": "屏住呼吸",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "neither...nor...",
        "p": "",
        "t": "conjunction",
        "m": "既不...也不...",
        "e": [],
        "u": "3",
        "tags": []
      }
    ],
    "4": [
      {
        "w": "master",
        "p": "/'mɑːstə(r)/",
        "t": "n.",
        "m": "著名画家；大师",
        "e": [],
        "u": "4",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "masterpiece",
        "p": "/'mɑːstəpiːs/",
        "t": "n.",
        "m": "代表作；杰作",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "whenever",
        "p": "/wen'evə(r)/",
        "t": "conj.",
        "m": "每当；每次",
        "e": [],
        "u": "4",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "alone",
        "p": "/ə'luːn/",
        "t": "adv.",
        "m": "独自",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "copy",
        "p": "/'kɒpi/",
        "t": "v.",
        "m": "仿造；临摹",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "unfortunately",
        "p": "/ʌn'fɔːtʃənətli/",
        "t": "adv.",
        "m": "不幸地；遗憾地",
        "e": [],
        "u": "4",
        "tags": [
          "suf_ly",
          "pre_un"
        ]
      },
      {
        "w": "graduate",
        "p": "/'ɡrædʒueɪt/",
        "t": "v.",
        "m": "毕业",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "limit",
        "p": "/'lɪmɪt/",
        "t": "n./v.",
        "m": "限量；量",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "temperature",
        "p": "/'temprətʃə(r)/",
        "t": "n.",
        "m": "温度",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "preserve",
        "p": "/prɪ'zɜːv/",
        "t": "v.",
        "m": "维持；保存",
        "e": [],
        "u": "4",
        "tags": [
          "pre_pre"
        ]
      },
      {
        "w": "exhibition",
        "p": "/ˌeksɪ'bɪʃn/",
        "t": "/eks'bɪʃn/",
        "m": "展览",
        "e": [],
        "u": "4",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "lifestyle",
        "p": "/'laɪfstaɪl/",
        "t": "n.",
        "m": "生活方式",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "detail",
        "p": "/'diːteɪl/",
        "t": "n.",
        "m": "细节；局部",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "daughter",
        "p": "/'dɔːtə(r)/",
        "t": "n.",
        "m": "女儿",
        "e": [],
        "u": "4",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "priceless",
        "p": "/'praɪsləs/",
        "t": "adj.",
        "m": "无价的",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "sculpture",
        "p": "/'skʌlptʃə(r)/",
        "t": "n.",
        "m": "雕塑",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "in one sitting",
        "p": "",
        "t": "phrase",
        "m": "一口气地",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "once in a while",
        "p": "",
        "t": "phrase",
        "m": "偶尔；间或",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "once in a blue moon",
        "p": "",
        "t": "phrase",
        "m": "罕见地；千载难逢",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "be related to",
        "p": "",
        "t": "v. phr.",
        "m": "与...有关；与...相关",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "be proud of",
        "p": "",
        "t": "v. phr.",
        "m": "为...感到自豪",
        "e": [],
        "u": "4",
        "tags": [
          "phr_be_of"
        ]
      },
      {
        "w": "be full of",
        "p": "",
        "t": "v. phr.",
        "m": "充满...",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "even so",
        "p": "",
        "t": "phrase",
        "m": "尽管如此",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "in addition",
        "p": "",
        "t": "phrase",
        "m": "此外；另外",
        "e": [],
        "u": "4",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "all the time",
        "p": "",
        "t": "phrase",
        "m": "一直；始终",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "make up one's mind",
        "p": "",
        "t": "v. phr.",
        "m": "下定决心",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "take part in",
        "p": "",
        "t": "v. phr.",
        "m": "参加；参与",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "be interested in",
        "p": "",
        "t": "v. phr.",
        "m": "对...感兴趣",
        "e": [],
        "u": "4",
        "tags": [
          "phr_be_in"
        ]
      },
      {
        "w": "the first time",
        "p": "",
        "t": "phrase",
        "m": "第一次",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "at least",
        "p": "",
        "t": "phrase",
        "m": "至少",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "as well as",
        "p": "",
        "t": "phrase",
        "m": "也；以及",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "instead of",
        "p": "",
        "t": "phrase",
        "m": "代替",
        "e": [],
        "u": "4",
        "tags": []
      }
    ],
    "5": [
      {
        "w": "recognize",
        "p": "/rɪ'kɒɡnaɪz/",
        "t": "v.",
        "m": "认出；识别",
        "e": [],
        "u": "5",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "mirror",
        "p": "/'mɪrə(r)/",
        "t": "n.",
        "m": "镜子",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "gun",
        "p": "/ɡʌn/",
        "t": "n.",
        "m": "枪；炮",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "handsome",
        "p": "/'hænsəm/",
        "t": "adj.",
        "m": "英俊的；潇洒的",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "force",
        "p": "/fɔːs/",
        "t": "v.",
        "m": "强迫；迫使",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "corn",
        "p": "/kɔːn/",
        "t": "n.",
        "m": "谷物；玉米",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "direction",
        "p": "/də'rekʃn/",
        "t": "n.",
        "m": "方向；方位",
        "e": [],
        "u": "5",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "progress",
        "p": "/'prəʊɡres/",
        "t": "n.",
        "m": "进步；进展",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "value",
        "p": "/'væljuː/",
        "t": "v.",
        "m": "重视；珍视",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "official",
        "p": "/ə'fɪʃl/",
        "t": "adj.",
        "m": "官方的",
        "e": [],
        "u": "5",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "throw",
        "p": "/θrəʊ/",
        "t": "v.",
        "m": "扔；抛",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "law",
        "p": "/lɔː/",
        "t": "n.",
        "m": "法律；法规",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "punish",
        "p": "/'pʌnɪʃ/",
        "t": "v.",
        "m": "处罚；惩罚",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "blog",
        "p": "/blɒɡ/",
        "t": "n.",
        "m": "博客",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "publish",
        "p": "/'pʌblɪʃ/",
        "t": "v.",
        "m": "发表；公布",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "shoulder",
        "p": "/'ʃəʊldə(r)/",
        "t": "n.",
        "m": "肩膀",
        "e": [],
        "u": "5",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "perhaps",
        "p": "/pə'hæps/",
        "t": "adv.",
        "m": "也许；可能",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "slim",
        "p": "/slɪm/",
        "t": "adj.",
        "m": "苗条的；纤细的",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "alarmed",
        "p": "/ə'lɑːmd/",
        "t": "adj.",
        "m": "惊吓的；受惊的",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "doubt",
        "p": "/daʊt/",
        "t": "v.",
        "m": "怀疑",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "sore",
        "p": "/sɔː(r)/",
        "t": "adj.",
        "m": "疼痛的；酸痛的",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "red-crowned crane",
        "p": "",
        "t": "n. phr.",
        "m": "丹顶鹤",
        "e": [],
        "u": "5",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "polar bear",
        "p": "",
        "t": "n. phr.",
        "m": "北极熊",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "more than",
        "p": "",
        "t": "phrase",
        "m": "多于；超过",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "shoot",
        "p": "",
        "t": "v.",
        "m": "射击；拍摄",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "make use of",
        "p": "",
        "t": "v. phr.",
        "m": "利用",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "without a care in the world",
        "p": "",
        "t": "phrase",
        "m": "无忧无虑",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "ought to",
        "p": "",
        "t": "v. phr.",
        "m": "应该",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "up to",
        "p": "",
        "t": "phrase",
        "m": "达到（某数量、程度等）",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "nature reserve",
        "p": "",
        "t": "n. phr.",
        "m": "自然保护区",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "day and night",
        "p": "",
        "t": "phrase",
        "m": "日以继夜",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "go missing",
        "p": "",
        "t": "v. phr.",
        "m": "失踪；不见",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "look for",
        "p": "",
        "t": "v. phr.",
        "m": "寻找",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "lose one's life",
        "p": "",
        "t": "v. phr.",
        "m": "丧命；失去生命",
        "e": [],
        "u": "5",
        "tags": []
      }
    ],
    "6": [
      {
        "w": "stage",
        "p": "/steɪdʒ/",
        "t": "n.",
        "m": "舞台",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "clue",
        "p": "/kluː/",
        "t": "n.",
        "m": "线索；提示",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "promote",
        "p": "/prə'məʊt/",
        "t": "v.",
        "m": "促进；推动",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "abroad",
        "p": "/ə'brɔːd/",
        "t": "adv.",
        "m": "在国外；到国外",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "costume",
        "p": "/'kɒstjuːm/",
        "t": "n.",
        "m": "戏装；服装",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "opportunity",
        "p": "/ˌɒpə'tjuːnəti/",
        "t": "n.",
        "m": "机会",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "acquire",
        "p": "/ə'kwaɪə(r)/",
        "t": "v.",
        "m": "获得；得到",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "sheet",
        "p": "/ʃiːt/",
        "t": "n.",
        "m": "一张（通常指纸张尺寸）",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "conclusion",
        "p": "/kən'kluːʒn/",
        "t": "n.",
        "m": "结束；结尾",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "poetry",
        "p": "/'pəʊətri/",
        "t": "n.",
        "m": "诗歌",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "concerned",
        "p": "/kən'sɜːnd/",
        "t": "adj.",
        "m": "关切的；注意的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "creative",
        "p": "/kri'eɪtɪv/",
        "t": "adj.",
        "m": "创造（性）的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "pleasure",
        "p": "/'pleʒə(r)/",
        "t": "n.",
        "m": "高兴；快乐",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "rosy",
        "p": "/'rəʊzi/",
        "t": "adj.",
        "m": "粉红色的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "youth",
        "p": "/juːθ/",
        "t": "n.",
        "m": "年轻；青春",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "worth",
        "p": "/wɜːθ/",
        "t": "adj.",
        "m": "有价值的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "Christmas",
        "p": "/'krɪsməs/",
        "t": "n.",
        "m": "圣诞节",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "eve",
        "p": "/iːv/",
        "t": "n.",
        "m": "前夕；前夕",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "spelling bee",
        "p": "",
        "t": "n. phr.",
        "m": "拼写比赛",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "treasure hunt",
        "p": "",
        "t": "n. phr.",
        "m": "寻宝游戏",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "cultural exchange",
        "p": "",
        "t": "n. phr.",
        "m": "文化交流",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "technological development",
        "p": "",
        "t": "n. phr.",
        "m": "技术发展",
        "e": [],
        "u": "6",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "no wonder",
        "p": "",
        "t": "phrase",
        "m": "不难理解；难怪",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "high-speed train",
        "p": "",
        "t": "n. phr.",
        "m": "高铁",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "pass by",
        "p": "",
        "t": "v. phr.",
        "m": "经过；驶过",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "go by",
        "p": "",
        "t": "v. phr.",
        "m": "（时间）流逝；过去",
        "e": [],
        "u": "6",
        "tags": []
      }
    ],
    "7": [
      {
        "w": "beef",
        "p": "/biːf/",
        "t": "n.",
        "m": "牛肉",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "seafood",
        "p": "/'siːfuːd/",
        "t": "n.",
        "m": "海鲜",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "afford",
        "p": "/ə'fɔːd/",
        "t": "v.",
        "m": "承担得起（后果）",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "mistake",
        "p": "/mɪ'steɪk/",
        "t": "n.",
        "m": "错误；失误",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "stick",
        "p": "/stɪk/",
        "t": "v.",
        "m": "粘；贴",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "hit",
        "p": "/hɪt/",
        "t": "v.",
        "m": "碰撞；撞击",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "muscle",
        "p": "/'mʌsl/",
        "t": "n.",
        "m": "肌肉",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "gym",
        "p": "/dʒɪm/",
        "t": "n.",
        "m": "健身房",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "satellite",
        "p": "/'sætəlaɪt/",
        "t": "n.",
        "m": "人造卫星",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "destroy",
        "p": "/dɪ'strɔɪ/",
        "t": "v.",
        "m": "摧毁",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "average",
        "p": "/'ævərɪdʒ/",
        "t": "adj.",
        "m": "平均的",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "probable",
        "p": "/'prɒbəbl/",
        "t": "adj.",
        "m": "很可能发生的（或存在的）",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "select",
        "p": "/sɪ'lekt/",
        "t": "v.",
        "m": "挑选；选拔",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "spaceflight",
        "p": "/'speɪsflaɪt/",
        "t": "n.",
        "m": "宇宙飞行",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "serve",
        "p": "/sɜːv/",
        "t": "v.",
        "m": "为……工作；服务",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "deliver",
        "p": "/dɪ'lɪvə(r)/",
        "t": "v.",
        "m": "传送；运送",
        "e": [],
        "u": "7",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "fuel",
        "p": "/'fjuːəl/",
        "t": "n.",
        "m": "燃料",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "supplies",
        "p": "/sə'plaɪz/",
        "t": "[pl.]",
        "m": "补给；供给",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "point",
        "p": "/pɔɪnt/",
        "t": "n.",
        "m": "时刻；阶段",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "lab",
        "p": "/læb/",
        "t": "n.",
        "m": "实验室",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "take off",
        "p": "",
        "t": "v. phr.",
        "m": "起飞",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "for a while",
        "p": "",
        "t": "phrase",
        "m": "一段时间",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "turn out",
        "p": "",
        "t": "v. phr.",
        "m": "证明是；结果是",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "cannot afford to",
        "p": "",
        "t": "v. phr.",
        "m": "承担不起（后果）",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "by mistake",
        "p": "",
        "t": "phrase",
        "m": "错误地；无意中",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "daily routine",
        "p": "",
        "t": "n. phr.",
        "m": "日常生活",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "space junk",
        "p": "",
        "t": "n. phr.",
        "m": "太空垃圾",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "naked eye",
        "p": "",
        "t": "n. phr.",
        "m": "肉眼",
        "e": [],
        "u": "7",
        "tags": []
      }
    ],
    "8": [
      {
        "w": "mention",
        "p": "/'menʃn/",
        "t": "v.",
        "m": "提到；说到",
        "e": [],
        "u": "8",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "tonight",
        "p": "/tə'naɪt/",
        "t": "adv.",
        "m": "在今夜；在今晚",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "frightened",
        "p": "/'fraɪtnd/",
        "t": "adj.",
        "m": "受惊的；害怕的",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "throat",
        "p": "/θrəʊt/",
        "t": "n.",
        "m": "喉咙",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "cough",
        "p": "/kɒf/",
        "t": "v./n.",
        "m": "咳嗽",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "fear",
        "p": "/fɪə(r)/",
        "t": "n./v.",
        "m": "恐惧；害怕",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "creature",
        "p": "/'kriːtʃə(r)/",
        "t": "n.",
        "m": "生物；动物",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "ugly",
        "p": "/'ʌɡli/",
        "t": "adj.",
        "m": "丑陋的",
        "e": [],
        "u": "8",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "crazy",
        "p": "/'kreɪzi/",
        "t": "adj.",
        "m": "疯狂的；不理智的",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "refuse",
        "p": "/rɪ'fjuːz/",
        "t": "v.",
        "m": "拒绝；回绝",
        "e": [],
        "u": "8",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "emergency",
        "p": "/ɪ'mɜːdʒənsi/",
        "t": "n.",
        "m": "紧急情况",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "camera",
        "p": "/'kæmərə/",
        "t": "n.",
        "m": "照相机",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "tent",
        "p": "/tent/",
        "t": "n.",
        "m": "帐篷",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "unnecessary",
        "p": "/ʌn'nesəsəri/",
        "t": "adj.",
        "m": "不必要的；多余的",
        "e": [],
        "u": "8",
        "tags": [
          "pre_un"
        ]
      },
      {
        "w": "because of",
        "p": "",
        "t": "phrase",
        "m": "因为；由于",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "in fear",
        "p": "",
        "t": "phrase",
        "m": "恐惧地",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "landing site",
        "p": "",
        "t": "n. phr.",
        "m": "着陆地点；降落场",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "agree with",
        "p": "",
        "t": "v. phr.",
        "m": "同意；与...一致",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "disagree with",
        "p": "",
        "t": "v. phr.",
        "m": "不同意",
        "e": [],
        "u": "8",
        "tags": [
          "pre_dis"
        ]
      },
      {
        "w": "move about",
        "p": "",
        "t": "v. phr.",
        "m": "四处走动",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "in all directions",
        "p": "",
        "t": "phrase",
        "m": "向四面八方",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "speed up",
        "p": "",
        "t": "v. phr.",
        "m": "加速",
        "e": [],
        "u": "8",
        "tags": []
      }
    ]
  }
};
const PATTERNS = [
  {
    "id": "suf_tion",
    "type": "suffix",
    "pattern": "tion",
    "title": "名词后缀 -tion",
    "desc": "表示动作或状态，常由动词演变而来。"
  },
  {
    "id": "suf_er",
    "type": "suffix",
    "pattern": "er",
    "title": "人/物后缀 -er",
    "desc": "表示动作的执行者或者是某种工具/物品。"
  },
  {
    "id": "suf_ly",
    "type": "suffix",
    "pattern": "ly",
    "title": "副词后缀 -ly",
    "desc": "表示动作的方式、程度或频率。"
  },
  {
    "id": "suf_al",
    "type": "suffix",
    "pattern": "al",
    "title": "形容词后缀 -al",
    "desc": "表示性质或相关的特征。"
  },
  {
    "id": "suf_ment",
    "type": "suffix",
    "pattern": "ment",
    "title": "名词后缀 -ment",
    "desc": "表示动作的结果、状态或手段。"
  },
  {
    "id": "pre_re",
    "type": "prefix",
    "pattern": "re",
    "title": "前缀 re-",
    "desc": "表示“再次”、“重新”或“向后”。"
  },
  {
    "id": "pre_un",
    "type": "prefix",
    "pattern": "un",
    "title": "前缀 un-",
    "desc": "表示“不”、“相反”或“除去”。"
  },
  {
    "id": "pre_dis",
    "type": "prefix",
    "pattern": "dis",
    "title": "前缀 dis-",
    "desc": "表示“不”、“分离”或“离开”。"
  },
  {
    "id": "pre_pre",
    "type": "prefix",
    "pattern": "pre",
    "title": "前缀 pre-",
    "desc": "表示“预先”、“在之前”。"
  },
  {
    "id": "phr_be_in",
    "type": "exact",
    "pattern": "be interested in",
    "title": "固定搭配：be interested in",
    "desc": "对...感兴趣"
  },
  {
    "id": "phr_be_of",
    "type": "exact",
    "pattern": "be proud of",
    "title": "固定搭配：be proud of",
    "desc": "为...感到自豪"
  },
  {
    "id": "phr_be_at",
    "type": "exact",
    "pattern": "be good at",
    "title": "固定搭配：be good at",
    "desc": "擅长..."
  }
];
const GRAMMAR = {
  "一般现在时": {
    "title": "一般现在时 (Simple Present Tense)",
    "desc": "表示经常性、习惯性的动作或客观事实。",
    "rules": [
      "第三人称单数 (he/she/it) 的动词加 -s 或 -es。",
      "be 动词用 am/is/are。",
      "否定形式：don't / doesn't + 动词原形。",
      "疑问形式：Do / Does + 主语 + 动词原形？",
      "常见时间状语：always, usually, often, every day, sometimes 等。"
    ],
    "example": [
      "She goes to school every day.",
      "He doesn't like pears.",
      "Do you play football?"
    ],
    "note": "第三人称单数动词变形规则：一般加 -s，如 works；以 s/x/ch/sh/o 结尾加 -es，如 watches, goes；辅音+y 结尾变 y 为 i 再加 -es，如 studies。"
  },
  "be动词": {
    "title": "be 动词 (am / is / are / was / were)",
    "desc": "表示主语的身份、特征、状态或地点。",
    "rules": [
      "I → am；He/She/It → is；You/We/They → are。",
      "否定形式: am not / isn't / aren't。",
      "一般疑问句: 把 be 提到主语前面 (Is he...? Are they...?)。",
      "过去式: was (第一、三人称单数) / were (第二人称及复数)。",
      "There be 句型: There is / There are 表示‘有’。"
    ],
    "example": [
      "I am a student.",
      "She is my friend.",
      "They are not at home.",
      "There is a cat in the room."
    ],
    "note": "be 动词不可单独作实义动词使用；表存在时用 There be，而非 have。"
  },
  "情态动词": {
    "title": "情态动词 can / should / must",
    "desc": "can 表示能力或可能性，should 表示建议或义务，must 表示必须。",
    "rules": [
      "情态动词后接动词原形，不能单独作谓语。",
      "没有人称和数的变化 (He can, not He cans)。",
      "can 的否定: can't；疑问: Can ...?",
      "should 的否定: shouldn't；表示‘应该’。",
      "must 的否定: mustn't (禁止)；must 表示‘必须’。",
      "情态动词没有时态变化，过去意义常借助 could / should have done。"
    ],
    "example": [
      "I can swim.",
      "She can't speak Japanese.",
      "You should do your homework.",
      "He shouldn't eat too much.",
      "You mustn't park here."
    ],
    "note": "情态动词有时可表示推测：He must be tired. (他肯定累了。) / It may rain. (可能下雨。)"
  },
  "There_be": {
    "title": "There be 句型",
    "desc": "表示‘某处有某物/某人’，强调存在。",
    "rules": [
      "There is + 单数或不可数名词；There are + 复数名词。",
      "就近原则: There is a book and two pencils on the desk. (is 跟第一个名词一致)",
      "否定: There isn't / There aren't。",
      "疑问: Is there...? / Are there...?",
      "常与 some, any, no, many, much, a lot of 连用。"
    ],
    "example": [
      "There is a cat in the room.",
      "There are many flowers in spring.",
      "Is there any milk in the fridge?",
      "There isn't a bus stop near here."
    ],
    "note": "There be 与 have/has 的区别: There be 表示‘存在’，have 表示‘拥有'。"
  },
  "一般疑问句": {
    "title": "一般疑问句与特殊疑问句",
    "desc": "一般疑问句(boolean question) 期待 Yes/No 回答；特殊疑问句以疑问词开头。",
    "rules": [
      "一般疑问句: be 动词或助动词 (do/does/did/will/shall) 提前。",
      "回答必须用 Yes/No + 主语 + 对应形式。",
      "特殊疑问词: what, where, who, when, why, how, which。",
      "how 常与形容词/副词搭配: how old, how many, how much, how often, how long。",
      "回答不能只用 Yes/No，必须给出具体信息。"
    ],
    "example": [
      "Are you a student? — Yes, I am.",
      "Where do you live? — I live in Guangzhou.",
      "How old are you? — I'm thirteen.",
      "What is your favourite colour? — I like red."
    ],
    "note": "‘Who is that boy?’ 回答可以是 ‘He is my brother.’ 或简略 ‘My brother.’"
  },
  "主谓一致": {
    "title": "主谓一致 (Subject-Verb Agreement)",
    "desc": "谓语动词在人称和数上必须与主语保持一致。",
    "rules": [
      "单数第三人称现在时动词加 -s/-es。",
      "and 连接的并列主语通常看作复数 (Tom and Jerry are... )。",
      "each, every, no one, nobody, everybody 等作主语时，谓语用单数。",
      "以 -s 结尾的名词(news, maths, physics)作主语时，谓语用单数。",
      "there be 句型中，be 跟靠近它的主语一致(就近原则)。"
    ],
    "example": [
      "He likes music. / They like music.",
      "Every student has a book.",
      "Maths is my favourite subject.",
      "There is a book and two pens on the desk."
    ],
    "note": "以 ex, class, family, team 等集体名词作主语，看作整体时用单数，看作个体时用复数。"
  },
  "名词的数": {
    "title": "名词的单复数 (Singular & Plural)",
    "desc": "可数名词有单复数形式，不可数名词没有复数形式。",
    "rules": [
      "一般情况下加 -s: book → books, dog → dogs。",
      "以 s, x, ch, sh 结尾加 -es: bus → buses, box → boxes, watch → watches。",
      "以辅音字母 + y 结尾的，变 y 为 i 加 -es: city → cities, baby → babies。",
      "以 f/fe 结尾的，常变 f/fe 为 v 加 -es: leaf → leaves, knife → knives (个别仍加 -s: roof → roofs)。",
      "不规则变化: child → children, man → men, woman → women, foot → feet, tooth → teeth。",
      "不可数名词(milk, rice, water, homework) 无复数，谓语用单数。"
    ],
    "example": [
      "I have two apples.",
      "There are many children in the park.",
      "The leaves turn yellow in autumn.",
      "Some water is in the bottle."
    ],
    "note": "表示‘一些’时可用 some 修饰可数/不可数，any 常用于否定和疑问。"
  },
  "冠词": {
    "title": "冠词 (a, an, the)",
    "desc": "不定冠词 a/an 表示‘一个’泛指；定冠词 the 表示特指。",
    "rules": [
      "a 用在辅音音素前: a book, a dog. an 用在元音音素前: an apple, an hour。",
      "第一次提到的单数可数名词用 a/an。",
      "第二次提到或说话双方都明确的，用 the。",
      "表示类别时可用零冠词(复数或不可数): Cats are cute. / Water is important。",
      "某些固定结构中不用冠词: by bus, at school, go home。"
    ],
    "example": [
      "I have a book. The book is new.",
      "She is an English teacher.",
      "Cats are lovely animals.",
      "He goes to school by bus."
    ],
    "note": "元音音素原则: an hour (h 不发音), a university (u 读作辅音 /j/)。"
  },
  "形容词和副词": {
    "title": "形容词和副词的比较级/最高级",
    "desc": "形容词修饰名词，副词修饰动词/形容词/副词；多数可比较。",
    "rules": [
      "一般词尾加 -er / -est: tall → taller → tallest。",
      "重叠或多音节词用 more / most: beautiful → more beautiful → most beautiful。",
      "不规则变化: good/well → better → best；bad → worse → worst。",
      "副词比较级: fast → faster → fastest；slowly → more slowly。",
      "比较级常用 than，最高级常用 the。"
    ],
    "example": [
      "Tom is taller than Mike.",
      "She is the most beautiful girl in my class.",
      "He runs faster than me.",
      "This is the best book I've ever read."
    ],
    "note": "more/most 也可用于某些双音节词: more careful, most careful。"
  },
  "现在进行时": {
    "title": "现在进行时 (Present Continuous Tense)",
    "desc": "表示正在进行的动作。",
    "rules": [
      "构成: be (am/is/are) + 动词的现在分词 (v.-ing)。",
      "否定: am/is/are not + v.-ing。",
      "疑问: 把 be 提前。",
      "常与 now, at the moment, look!, listen! 等连用。",
      "有些动词(如 like, want, know, believe)一般不用于进行时。"
    ],
    "example": [
      "I am doing my homework now.",
      "She is reading a book.",
      "They are not playing football.",
      "Are you watching TV?"
    ],
    "note": "现在进行时还可表示将来安排好的动作: We are leaving tomorrow."
  },
  "一般过去时": {
    "title": "一般过去时 (Simple Past Tense)",
    "desc": "表示过去某个时间发生的动作或状态。",
    "rules": [
      "规则动词加 -ed: walk → walked, play → played。",
      "不规则动词须记忆: go → went, have → had, be → was/were, do → did。",
      "否定: didn't + 动词原形。",
      "疑问: Did + 主语 + 动词原形？",
      "常见时间状语: yesterday, last week, ... ago, in 2020, just now。"
    ],
    "example": [
      "I went to school yesterday.",
      "She didn't do her homework.",
      "Did you see the film last night?",
      "I was at home an hour ago."
    ],
    "note": "was/were 是 be 动词的过去式，不由 -ed 构成。"
  },
  "祈使句": {
    "title": "祈使句 (Imperative Sentences)",
    "desc": "表示命令、请求、建议、警告等。",
    "rules": [
      "以动词原形开头: Open the door. Sit down。",
      "否定: Don't + 动词原形: Don't run. Don't eat in class。",
      "主语 you 常省略，有时加 please 表示礼貌。",
      "Let's + 动词原形 表示建议: Let's go. Let's play a game。",
      "Let me + 动词原形 表示请求: Let me help you."
    ],
    "example": [
      "Open the window, please.",
      "Don't be late.",
      "Let's start the class.",
      "Let me carry the bag for you."
    ],
    "note": "祈使句感叹号结尾较强语气，句号可表示较温和的请求。"
  },
  "一般将来时": {
    "title": "一般将来时 (Simple Future Tense)",
    "desc": "表示将来要做的事情或将要发生的动作。",
    "rules": [
      "will + 动词原形：表示意愿、决定、预测等。",
      "be going to + 动词原形：表示计划、安排、迹象明显的将来。",
      "表示将来常用时间状语：tomorrow, next week, in two days 等。"
    ],
    "example": [
      "I will help you.",
      "He is going to visit his grandparents next week.",
      "It's going to rain; look at those clouds!"
    ],
    "note": "will 通常用于临时决定，be going to 通常用于事先计划好的安排。"
  },
  "过去进行时": {
    "title": "过去进行时 (Past Continuous Tense)",
    "desc": "表示在过去某一时刻正在进行的动作。",
    "rules": [
      "构成：was / were + 动词的现在分词 (v-ing)。",
      "was 用于 I, he, she, it 及单数名词；were 用于 you, we, they 及复数名词。",
      "常与过去某一时间点连用，如: at 8:00 yesterday, when..."
    ],
    "example": [
      "I was sleeping at 10:00 last night.",
      "They were playing football when it started to rain.",
      "What were you doing at that time?"
    ],
    "note": "若动作是瞬间完成的（如 start, stop），通常不用进行时。"
  },
  "现在完成时": {
    "title": "现在完成时 (Present Perfect Tense)",
    "desc": "表示动作从过去持续到现在，或对现在造成的影响。",
    "rules": [
      "构成：have / has + 过去分词 (done)。",
      "have 用于 I, you, we, they；has 用于 he, she, it。",
      "常用时间状语：already, yet, ever, never, just, so far, since + 时间点, for + 时间段。",
      "否定句常用 yet (放在句末)，疑问句常用 ever。"
    ],
    "example": [
      "I have already finished my homework.",
      "She has lived in Guangzhou for five years.",
      "Have you ever been to Shanghai?",
      "He hasn't eaten lunch yet."
    ],
    "note": "since + 时间点 (since 2010)；for + 时间段 (for three years)。"
  },
  "被动语态": {
    "title": "被动语态 (Passive Voice)",
    "desc": "当动作的承受者是句子的主语时使用。",
    "rules": [
      "构成：be + 过去分词 (done)。",
      "be 动词的时态取决于句子的基本时态（am/is/are, was/were, will be 等）。",
      "动作的发出者通常用 by + 某人 来表示。"
    ],
    "example": [
      "The book was written by Lu Xun.",
      "Rice is grown in China.",
      "The window was broken yesterday."
    ],
    "note": "如果动作的发出者不重要或不知道是谁，可以省略 by 部分。"
  },
  "定语从句": {
    "title": "定语从句 (Attributive Clauses)",
    "desc": "在名词后面起修饰作用的从句。",
    "rules": [
      "使用关系代词 (that, which, who) 引导。",
      "who/that 引导先行词是人的从句；which/that 引导先行词是物的从句。",
      "注意先行词在从句中是主语还是宾语。"
    ],
    "example": [
      "The man who is standing there is my teacher.",
      "I like the book that you bought me.",
      "This is the dog which lives in my house."
    ],
    "note": "当先行词是人时，通常用 who 或 that；当先行词是物时，通常用 which 或 that。"
  },
  "宾语从句": {
    "title": "宾语从句 (Object Clauses)",
    "desc": "在动词后面作宾语的从句。",
    "rules": [
      "引导词可以是 that, whether, if 或疑问词。",
      "核心规则：从句必须使用【陈述句语序】（主语在前，谓语在后），不能用疑问句语序。",
      "如果主句是现在时，从句可以用任何时态；如果主句是过去时，从句通常要进行时态回退。"
    ],
    "example": [
      "I think that he is a good boy.",
      "Do you know where he lives?",
      "He asked me if I liked English."
    ],
    "note": "在宾语从句中，不要说 'Do you know where does he live?'，要说 'Do you know where he lives?'。"
  },
  "条件状语从句": {
    "title": "条件状语从句 (Conditional Clauses)",
    "desc": "由 if 引导，表示在某种条件下会发生的事情。",
    "rules": [
      "主将从现原则：在 if 引导的条件状语从句中，用一般现在时表示将来，主句用一般将来时。",
      "unless = if...not (除非)。"
    ],
    "example": [
      "If it rains tomorrow, we will stay at home.",
      "If you study hard, you will pass the exam.",
      "Unless you hurry, you will miss the bus."
    ],
    "note": "从句中使用的是现在时，但在逻辑意义上表达的是将来。"
  }
};

/* --- HARDCODED DATA INJECTION --- */
 圈形的东西",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "collection",
      "p": "/kəˈlekʃn/",
      "t": "n.",
      "m": "(常指同类的) 收集物",
      "e": [],
      "u": "8",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "own",
      "p": "/əʊn/",
      "t": "v.",
      "m": "拥有; 有",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "valuable",
      "p": "/ˈvæljuəbl/",
      "t": "adj.",
      "m": "贵重的",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "handle",
      "p": "/ˈhændl/",
      "t": "v.",
      "m": "(用手) 接触",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "glove",
      "p": "/ɡlʌv/",
      "t": "n.",
      "m": "(分手指的) 手套",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "add",
      "p": "/æd/",
      "t": "v.",
      "m": "增加; 添加",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "envelope",
      "p": "/ˈenvələʊp/",
      "t": "n.",
      "m": "信封",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "absolutely",
      "p": "/ˈæbsəluːtli/",
      "t": "adv.",
      "m": "极其",
      "e": [],
      "u": "8",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "seem",
      "p": "/siːm/",
      "t": "v.",
      "m": "好像; 似乎",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "impossible",
      "p": "/ɪmˈpɒsəbl/",
      "t": "adj.",
      "m": "不可能的",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "single",
      "p": "/ˈsɪŋɡl/",
      "t": "adj.",
      "m": "单个的",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "nail",
      "p": "/neɪl/",
      "t": "n.",
      "m": "钉子",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "explain",
      "p": "/ɪkˈspleɪn/",
      "t": "v.",
      "m": "解释; 说明",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "bit",
      "p": "/bɪt/",
      "t": "n.",
      "m": "一部分",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "similar",
      "p": "/ˈsɪmələ(r)/",
      "t": "adj.",
      "m": "相似的; 类似的",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "coin",
      "p": "/kɔɪn/",
      "t": "n.",
      "m": "(一枚) 硬币",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "soft",
      "p": "/sɒft/",
      "t": "adj.",
      "m": "软的",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "sticker",
      "p": "/ˈstɪkə(r)/",
      "t": "n.",
      "m": "贴纸",
      "e": [],
      "u": "8",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "waste",
      "p": "/weɪst/",
      "t": "n.",
      "m": "白费; 浪费",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "item",
      "p": "/ˈaɪtəm/",
      "t": "n.",
      "m": "一件商品 (或物品)",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "result",
      "p": "/rɪˈzʌlt/",
      "t": "n.",
      "m": "后果; 结果",
      "e": [],
      "u": "8",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "wrapper",
      "p": "/ˈræpə(r)/",
      "t": "n.",
      "m": "包装纸",
      "e": [],
      "u": "8",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "bar",
      "p": "/bɑː(r)/",
      "t": "n.",
      "m": "(长方形) 条",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "include",
      "p": "/ɪnˈkluːd/",
      "t": "v.",
      "m": "包括; 包含",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "size",
      "p": "/saɪz/",
      "t": "n.",
      "m": "大小",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "pattern",
      "p": "/ˈpætn/",
      "t": "n.",
      "m": "图案; 花样",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "produce",
      "p": "/prəˈdjuːs/",
      "t": "v.",
      "m": "生产; 製造",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "attract",
      "p": "/əˈtrækt/",
      "t": "v.",
      "m": "吸引; 招引",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "insect",
      "p": "/ˈɪnsekt/",
      "t": "n.",
      "m": "昆虫",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "object",
      "p": "/ˈɒbdʒɪkt/",
      "t": "n.",
      "m": "物体; 物品",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "flat",
      "p": "/flæt/",
      "t": "adj.",
      "m": "平滑的",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "language",
      "p": "/ˈlæŋɡwɪdʒ/",
      "t": "n.",
      "m": "语言",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "treasure",
      "p": "/ˈtreʒə(r)/",
      "t": "n.",
      "m": "珍宝; 宝物",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "unlock",
      "p": "/ʌnˈlɒk/",
      "t": "v.",
      "m": "发现; 揭示",
      "e": [],
      "u": "8",
      "tags": [
        "pre_un"
      ]
    },
    {
      "w": "real",
      "p": "/ˈriːəl/",
      "t": "adj.",
      "m": "真的",
      "e": [],
      "u": "8",
      "tags": [
        "suf_al",
        "pre_re"
      ]
    },
    {
      "w": "add to...",
      "p": "",
      "t": "v. phr.",
      "m": "把......添加到...",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "soft drink",
      "p": "",
      "t": "n. phr.",
      "m": "饮料 (不含酒精)",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "a waste of time",
      "p": "",
      "t": "n. phr.",
      "m": "浪费时间",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "pocket money",
      "p": "",
      "t": "n. phr.",
      "m": "零花钱 (父母给孩子的)",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "all over the world",
      "p": "",
      "t": "phrase",
      "m": "全世界",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "literature",
      "p": "/'lɪtrətʃə(r)/",
      "t": "n.",
      "m": "文学; 文学作品",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "act",
      "p": "/ækt/",
      "t": "v.",
      "m": "扮演 (戏剧、电影中的角色)",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "appreciate",
      "p": "/ə'priːʃieɪt/",
      "t": "v.",
      "m": "欣赏; 赏识",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "join",
      "p": "/dʒɔɪn/",
      "t": "v.",
      "m": "成为......的一员; 参加",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "definitely",
      "p": "/'defɪnətli/",
      "t": "adv.",
      "m": "肯定; 没问题; 当然; 确实",
      "e": [],
      "u": "7",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "fair",
      "p": "/feə(r)/",
      "t": "n.",
      "m": "集市",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "rocket",
      "p": "/'rɒkɪt/",
      "t": "n.",
      "m": "火箭",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "launch",
      "p": "/lɔːntʃ/",
      "t": "v.",
      "m": "发射",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "shout",
      "p": "/ʃaʊt/",
      "t": "v.",
      "m": "大声; 叫喊",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "power",
      "p": "/'paʊə(r)/",
      "t": "n.",
      "m": "力量; 能量",
      "e": [],
      "u": "7",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "moment",
      "p": "/'məʊmənt/",
      "t": "n.",
      "m": "片刻; 瞬间",
      "e": [],
      "u": "7",
      "tags": [
        "suf_ment"
      ]
    },
    {
      "w": "member",
      "p": "/'membə(r)/",
      "t": "n.",
      "m": "成员",
      "e": [],
      "u": "7",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "shape",
      "p": "/ʃeɪp/",
      "t": "n.",
      "m": "形状; 外形",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "hike",
      "p": "/haɪk/",
      "t": "n.",
      "m": "远足; 徒步旅行",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "recently",
      "p": "/'riːsntli/",
      "t": "adv.",
      "m": "不久前; 最近",
      "e": [],
      "u": "7",
      "tags": [
        "suf_ly",
        "pre_re"
      ]
    },
    {
      "w": "attend",
      "p": "/ə'tend/",
      "t": "v.",
      "m": "出席; 参加",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "charity",
      "p": "/'tʃærəti/",
      "t": "n.",
      "m": "慈善机构 (或组织)",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "sale",
      "p": "/seɪl/",
      "t": "n.",
      "m": "出售; 销售",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "snack",
      "p": "/snæk/",
      "t": "n.",
      "m": "点心; 小吃",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "decorate",
      "p": "/'dekəreɪt/",
      "t": "v.",
      "m": "装饰; 装潢",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "difference",
      "p": "/'dɪfrəns/",
      "t": "n.",
      "m": "差别; 差异",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "compete",
      "p": "/kəm'piːt/",
      "t": "v.",
      "m": "参加比赛 (或竞赛)",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "event",
      "p": "/ɪ'vent/",
      "t": "n.",
      "m": "公开活动; 社交场合",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "spook",
      "p": "/spuːk/",
      "t": "v.",
      "m": "吓; 使惊恐",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "annual",
      "p": "/'ænjuəl/",
      "t": "adj.",
      "m": "每年的; 一年一度的",
      "e": [],
      "u": "7",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "fantastic",
      "p": "/fæn'tæstɪk/",
      "t": "adj.",
      "m": "极好的",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "dollar",
      "p": "/'dɒlə(r)/",
      "t": "n.",
      "m": "元 (美国、加拿大、澳大利亚等国的货币单位)",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "pie",
      "p": "/paɪ/",
      "t": "n.",
      "m": "馅饼; 派",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "equipment",
      "p": "/ɪ'kwɪpmənt/",
      "t": "n.",
      "m": "设备; 器材",
      "e": [],
      "u": "7",
      "tags": [
        "suf_ment"
      ]
    },
    {
      "w": "clubs fair",
      "p": "/'klʌbz feə(r)/",
      "t": "n. phr.",
      "m": "社团招募展",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "solar power",
      "p": "/'səʊlə ˈpaʊə(r)/",
      "t": "n. phr.",
      "m": "太阳能",
      "e": [],
      "u": "7",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "of course",
      "p": "/əv ˈkɔːs/",
      "t": "phr.",
      "m": "当然",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "remote control",
      "p": "/rɪ'məʊt kən'trəʊl/",
      "t": "n. phr.",
      "m": "遥控器",
      "e": [],
      "u": "7",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "turn around",
      "p": "/tɜːn ə'raʊnd/",
      "t": "v. phr.",
      "m": "转身",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "make a difference",
      "p": "/meɪk ə ˈdɪfrəns/",
      "t": "v. phr.",
      "m": "有作用; 有影响",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "primary school",
      "p": "/'praɪməri skuːl/",
      "t": "n. phr.",
      "m": "小学",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "fall off",
      "p": "/fɔːl ɒf/",
      "t": "v. phr.",
      "m": "掉落",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "guide",
      "p": "/ɡaɪd/",
      "t": "n.",
      "m": "指南；手册",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "east",
      "p": "/iːst/",
      "t": "n./adv.",
      "m": "东；东方 (adv: 朝东)",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "west",
      "p": "/west/",
      "t": "n./adv.",
      "m": "西；西方 (adv: 朝西)",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "mix",
      "p": "/mɪks/",
      "t": "v.",
      "m": "混合；结合",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "western",
      "p": "/'westən/",
      "t": "adj.",
      "m": "西方的；西方的国家",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "style",
      "p": "/staɪl/",
      "t": "n.",
      "m": "风格",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "popular",
      "p": "/'pɒpjələ(r)/",
      "t": "adj.",
      "m": "大众喜爱的；受欢迎的",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "site",
      "p": "/saɪt/",
      "t": "n.",
      "m": "(建筑物、城镇等的)地点",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "across",
      "p": "/ə'krɒs/",
      "t": "prep.",
      "m": "在......对面; 在......对过",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "adult",
      "p": "/'ædʌlt/",
      "t": "n.",
      "m": "成年人",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "senior",
      "p": "/'siːniə(r)/",
      "t": "n.",
      "m": "老年人；长者",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "customer",
      "p": "/'kʌstəmə(r)/",
      "t": "n.",
      "m": "顾客；主顾",
      "e": [],
      "u": "6",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "design",
      "p": "/dɪ'zaɪn/",
      "t": "v.",
      "m": "设计",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "else",
      "p": "/els/",
      "t": "adv.",
      "m": "其他的；别的",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "suggestion",
      "p": "/sə'dʒestʃən/",
      "t": "n.",
      "m": "建议；提议",
      "e": [],
      "u": "6",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "tip",
      "p": "/tɪp/",
      "t": "n.",
      "m": "点拨；实用的提示",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "historic",
      "p": "/hɪ'stɒrɪk/",
      "t": "adj.",
      "m": "历史著名的 (或重要的); 可名垂青史的",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "list",
      "p": "/lɪst/",
      "t": "n.",
      "m": "名单；目录",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "landscape",
      "p": "/'lændskeɪp/",
      "t": "n.",
      "m": "风景；景色",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "memory",
      "p": "/'meməri/",
      "t": "n.",
      "m": "记忆",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "general",
      "p": "/'dʒenərəl/",
      "t": "adj.",
      "m": "将军",
      "e": [],
      "u": "6",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "sight",
      "p": "/saɪt/",
      "t": "n.",
      "m": "看见",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "natural",
      "p": "/'nætʃrəl/",
      "t": "adj.",
      "m": "自然的；天然的",
      "e": [],
      "u": "6",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "almost",
      "p": "/'ɔːlməʊst/",
      "t": "adv.",
      "m": "几乎；差不多",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "become",
      "p": "/bɪ'kʌm/",
      "t": "v.",
      "m": "开始变得；变成",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "gateway",
      "p": "/'ɡeɪtweɪ/",
      "t": "n.",
      "m": "(通往其他地区的)门户",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "influence",
      "p": "/'ɪnfluəns/",
      "t": "v.",
      "m": "影响；对......起作用",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "artwork",
      "p": "/'ɑːtwɜːk/",
      "t": "n.",
      "m": "艺术作品",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "painting",
      "p": "/'peɪntɪŋ/",
      "t": "n.",
      "m": "绘画",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "scene",
      "p": "/siːn/",
      "t": "n.",
      "m": "场面；情景",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "wife",
      "p": "/waɪf/",
      "t": "n.",
      "m": "妻子",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "stone",
      "p": "/stəʊn/",
      "t": "n.",
      "m": "石头；石料",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "reflection",
      "p": "/rɪ'fle'(ʃ)n/",
      "t": "n.",
      "m": "映像；图像",
      "e": [],
      "u": "6",
      "tags": [
        "suf_tion",
        "pre_re"
      ]
    },
    {
      "w": "tourist spot",
      "p": "",
      "t": "n. phr.",
      "m": "旅游景点",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "light up",
      "p": "",
      "t": "v. phr.",
      "m": "照亮",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "in the centre",
      "p": "",
      "t": "phrase",
      "m": "在......的中心",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "be interested in",
      "p": "",
      "t": "v. phr.",
      "m": "对......有兴趣",
      "e": [],
      "u": "6",
      "tags": [
        "phr_be_in"
      ]
    },
    {
      "w": "historic site",
      "p": "",
      "t": "n. phr.",
      "m": "古迹",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "in memory of",
      "p": "",
      "t": "phrase",
      "m": "作为......的纪念",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "spacesuit",
      "p": "/spetsuːt/",
      "t": "n.",
      "m": "航天; 宇宙航行",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "collect",
      "p": "/kə'lekt/",
      "t": "v.",
      "m": "收集; 采集",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "leave",
      "p": "/liːv/",
      "t": "v.",
      "m": "离开",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "kilometre",
      "p": "/kɪlə'miːtə(r)/",
      "t": "n.",
      "m": "千米; 公里",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "weak",
      "p": "/wiːk/",
      "t": "adj.",
      "m": "虚弱的; 无力的",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "tie",
      "p": "/taɪ/",
      "t": "v.",
      "m": "(用线、绳等) 拴",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "low",
      "p": "/ləʊ/",
      "t": "adj.",
      "m": "低于(通常或平均)的数量(或水平)的",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "weight",
      "p": "/weɪ/",
      "t": "n.",
      "m": "有......重; 重",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "breathe",
      "p": "/briːð/",
      "t": "v.",
      "m": "呼吸",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "facility",
      "p": "/fə'sɪləti/",
      "t": "n.",
      "m": "设施; 设备",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "adventure",
      "p": "/əd'ventʃə(r)/",
      "t": "n.",
      "m": "冒险经历; 奇遇",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "pass",
      "p": "/pɑːs/",
      "t": "v.",
      "m": "向某方向前进; 向某方向移动",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "experiment",
      "p": "/ɪk'sperɪmənt/",
      "t": "n.",
      "m": "实验; 试验",
      "e": [],
      "u": "5",
      "tags": [
        "suf_ment"
      ]
    },
    {
      "w": "prepare",
      "p": "/pri'peə(r)/",
      "t": "v.",
      "m": "做好准备; 把......准备好",
      "e": [],
      "u": "5",
      "tags": [
        "pre_pre"
      ]
    },
    {
      "w": "dream",
      "p": "/driːm/",
      "t": "n./v.",
      "m": "梦想; 理想 / 做梦; 梦见",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "chance",
      "p": "/tʃɑːns/",
      "t": "n.",
      "m": "机会; 机遇",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "send",
      "p": "/send/",
      "t": "v.",
      "m": "邮寄; 发送",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "introduction",
      "p": "/ˌɪntrə'dʌkʃn/",
      "t": "n.",
      "m": "序言; 引言",
      "e": [],
      "u": "5",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "view",
      "p": "/vjuː/",
      "t": "n.",
      "m": "景色; 风景",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "someday",
      "p": "/'sʌmdeɪ/",
      "t": "adv.",
      "m": "总有一天",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "experience",
      "p": "/ɪk'spɪəriəns/",
      "t": "n.",
      "m": "经历",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "lander",
      "p": "/'lændə(r)/",
      "t": "n.",
      "m": "着陆器",
      "e": [],
      "u": "5",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "island",
      "p": "/'aɪlənd/",
      "t": "n.",
      "m": "岛",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "ancient",
      "p": "/'eɪnʃənt/",
      "t": "adj.",
      "m": "古老的; 很老的",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "poem",
      "p": "/'pəʊm/",
      "t": "n.",
      "m": "诗",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "express",
      "p": "/ɪk'spres/",
      "t": "v.",
      "m": "表示; 表达; 表露",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "determination",
      "p": "/dɪˌtɜːmɪ'neɪʃn/",
      "t": "n.",
      "m": "决心; 坚定",
      "e": [],
      "u": "5",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "universe",
      "p": "/juː'nɪvɜːs/",
      "t": "n.",
      "m": "宇宙",
      "e": [],
      "u": "5",
      "tags": [
        "pre_un"
      ]
    },
    {
      "w": "already",
      "p": "/ɔːl'redi/",
      "t": "adv.",
      "m": "已经; 完毕",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "century",
      "p": "/'sentʃəri/",
      "t": "n.",
      "m": "世纪",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "neighbour",
      "p": "/'neɪbə(r)/",
      "t": "n.",
      "m": "邻居",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "circle",
      "p": "/'sɜːkl/",
      "t": "v.",
      "m": "环行",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "surface",
      "p": "/'sɜːfɪs/",
      "t": "n.",
      "m": "表面; 层面",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "release",
      "p": "/ri'liːs/",
      "t": "v.",
      "m": "放; 松开; (使......)释放",
      "e": [],
      "u": "5",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "kilogram",
      "p": "/'kɪləɡræm/",
      "t": "n.",
      "m": "千克; 公斤",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "programme",
      "p": "/'prəʊɡræm/",
      "t": "n.",
      "m": "计划; 方案",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "space station",
      "p": "",
      "t": "n. phr.",
      "m": "空间站",
      "e": [],
      "u": "5",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "do exercise",
      "p": "",
      "t": "v. phr.",
      "m": "锻炼; 运动",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "take photos",
      "p": "",
      "t": "v. phr.",
      "m": "拍照",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "get into orbit",
      "p": "",
      "t": "v. phr.",
      "m": "进入轨道",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "thousands of",
      "p": "",
      "t": "phrase",
      "m": "数以千计; 成千上万",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "system",
      "p": "/'sɪstəm/",
      "t": "n.",
      "m": "系统",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "file",
      "p": "/faɪl/",
      "t": "n.",
      "m": "档案",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "amazing",
      "p": "/ə'meɪzɪŋ/",
      "t": "adj.",
      "m": "令人大为惊奇的",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "planet",
      "p": "/'plænɪt/",
      "t": "n.",
      "m": "行星",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "billion",
      "p": "/'bɪljən/",
      "t": "n.",
      "m": "十亿",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "cover",
      "p": "/'kʌvə(r)/",
      "t": "v.",
      "m": "覆盖",
      "e": [],
      "u": "4",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "support",
      "p": "/sə'pɔːt/",
      "t": "v.",
      "m": "支持",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "area",
      "p": "/'eəriə/",
      "t": "n.",
      "m": "地区",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "freezing",
      "p": "/'friːzɪŋ/",
      "t": "adj.",
      "m": "冰冻的",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "north",
      "p": "/nɔːθ/",
      "t": "n.",
      "m": "北",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "pole",
      "p": "/pəʊl/",
      "t": "n.",
      "m": "(行星的)极地",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "desert",
      "p": "/'dezət/",
      "t": "n.",
      "m": "沙漠",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "reach",
      "p": "/riːtʃ/",
      "t": "v.",
      "m": "足够达到",
      "e": [],
      "u": "4",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "grain",
      "p": "/ɡreɪn/",
      "t": "n.",
      "m": "谷粒",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "wide",
      "p": "/waɪd/",
      "t": "adj.",
      "m": "宽的",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "whale",
      "p": "/weɪl/",
      "t": "n.",
      "m": "鲸",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "butterfly",
      "p": "/'bʌtəflaɪ/",
      "t": "n.",
      "m": "蝴蝶",
      "e": [],
      "u": "4",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "provide",
      "p": "/prə'vaɪd/",
      "t": "v.",
      "m": "提供",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "explore",
      "p": "/ɪk'splɔː(r)/",
      "t": "v.",
      "m": "探索",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "disappear",
      "p": "/ˌdɪsə'pɪə(r)/",
      "t": "v.",
      "m": "消失；不见",
      "e": [],
      "u": "4",
      "tags": [
        "pre_dis"
      ]
    },
    {
      "w": "plastic",
      "p": "/'plæstɪk/",
      "t": "n.",
      "m": "塑料",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "pollution",
      "p": "/pə'luːʃn/",
      "t": "n.",
      "m": "污染",
      "e": [],
      "u": "4",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "protect",
      "p": "/prə'tekt/",
      "t": "v.",
      "m": "保护",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "chemical",
      "p": "/'kemɪkl/",
      "t": "n.",
      "m": "化学品",
      "e": [],
      "u": "4",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "emperor",
      "p": "/'empərə(r)/",
      "t": "n.",
      "m": "皇帝",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "hunt",
      "p": "/hʌnt/",
      "t": "v.",
      "m": "打猎",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "war",
      "p": "/wɔː(r)/",
      "t": "n.",
      "m": "战争",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "sandstorm",
      "p": "/'sændstɔːm/",
      "t": "n.",
      "m": "沙暴",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "solution",
      "p": "/sə'luːʃn/",
      "t": "n.",
      "m": "解决方法",
      "e": [],
      "u": "4",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "blow",
      "p": "/bləʊ/",
      "t": "v.",
      "m": "吹",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "generation",
      "p": "/ˌdʒenə'reɪʃn/",
      "t": "n.",
      "m": "(统称)一代人",
      "e": [],
      "u": "4",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "groundwater",
      "p": "/'ɡraʊndwɔːtə(r)/",
      "t": "n.",
      "m": "地下水",
      "e": [],
      "u": "4",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "burn",
      "p": "/bɜːn/",
      "t": "v.",
      "m": "燃烧",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "oil",
      "p": "/ɔɪl/",
      "t": "n.",
      "m": "石油；原油",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "gas",
      "p": "/ɡæs/",
      "t": "n.",
      "m": "天然气",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "harmful",
      "p": "/'hɑːmfʊl/",
      "t": "adj.",
      "m": "有害的",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "website",
      "p": "/'websaɪt/",
      "t": "n.",
      "m": "网站",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "own",
      "p": "/əʊn/",
      "t": "adj.",
      "m": "自己的；本人的",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "however",
      "p": "/haʊ'evə(r)/",
      "t": "adv.",
      "m": "然而；不过",
      "e": [],
      "u": "4",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "solar system",
      "p": "",
      "t": "n. phr.",
      "m": "太阳系",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "be covered by",
      "p": "",
      "t": "v. phr.",
      "m": "被......覆盖",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "as far as we know",
      "p": "",
      "t": "phrase",
      "m": "据我们所知",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "sea level",
      "p": "",
      "t": "n. phr.",
      "m": "海平面",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "what's more",
      "p": "",
      "t": "phrase",
      "m": "更重要的是",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "provide with",
      "p": "",
      "t": "v. phr.",
      "m": "提供；给予",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "take care of",
      "p": "",
      "t": "v. phr.",
      "m": "照料",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "plastic pollution",
      "p": "",
      "t": "n. phr.",
      "m": "塑料污染",
      "e": [],
      "u": "4",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "hundreds of",
      "p": "",
      "t": "phrase",
      "m": "数百；以百计",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "blow away",
      "p": "",
      "t": "v. phr.",
      "m": "吹走",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "footprint",
      "p": "/'fʊtprɪnt/",
      "t": "n.",
      "m": "脚印；足迹",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "wet",
      "p": "/wet/",
      "t": "adj.",
      "m": "潮湿的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "deep",
      "p": "/diːp/",
      "t": "adj.",
      "m": "深的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "sandy",
      "p": "/'sændi/",
      "t": "adj.",
      "m": "铺满沙子的；含沙的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "kick",
      "p": "/kɪk/",
      "t": "v.",
      "m": "踢；踹",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "town",
      "p": "/taʊn/",
      "t": "n.",
      "m": "镇；市镇",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "feature",
      "p": "/'fiːtʃə(r)/",
      "t": "n.",
      "m": "特色；特征",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "south",
      "p": "/saʊθ/",
      "t": "n.",
      "m": "南；南方",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "clear",
      "p": "/klɪə(r)/",
      "t": "adj.",
      "m": "晴朗的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "loudly",
      "p": "/'laʊdli/",
      "t": "adv.",
      "m": "响地；响亮地",
      "e": [],
      "u": "3",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "thunder",
      "p": "/'θʌndə(r)/",
      "t": "n.",
      "m": "雷",
      "e": [],
      "u": "3",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "lightning",
      "p": "/'laɪtnɪŋ/",
      "t": "n.",
      "m": "闪电",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "fresh",
      "p": "/freʃ/",
      "t": "adj.",
      "m": "新鲜的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "gather",
      "p": "/'ɡæðə(r)/",
      "t": "v.",
      "m": "采集；聚集；水果等",
      "e": [],
      "u": "3",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "peaceful",
      "p": "/'piːsfl/",
      "t": "adj.",
      "m": "安静的；平静的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "snake",
      "p": "/sneɪk/",
      "t": "n.",
      "m": "蛇",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "through",
      "p": "/θruː/",
      "t": "prep.",
      "m": "自始至；从...穿过",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "from head to toe",
      "p": "",
      "t": "phrase",
      "m": "从头到脚",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "part",
      "p": "/pɑːt/",
      "t": "n.",
      "m": "区域；部分",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "article",
      "p": "/'ɑːtɪkl/",
      "t": "n.",
      "m": "文章",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "reason",
      "p": "/'riːzn/",
      "t": "n.",
      "m": "原因",
      "e": [],
      "u": "3",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "alive",
      "p": "/ə'laɪv/",
      "t": "adj.",
      "m": "有活力的；活着的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "sandcastle",
      "p": "/'sændkɑːsl/",
      "t": "n.",
      "m": "沙堡",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "crop",
      "p": "/krɒp/",
      "t": "n.",
      "m": "庄稼；作物",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "culture",
      "p": "/'kʌltʃə(r)/",
      "t": "n.",
      "m": "文化",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "local",
      "p": "/'ləʊkl/",
      "t": "adj.",
      "m": "地方的；当地的",
      "e": [],
      "u": "3",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "tradition",
      "p": "/trə'dɪʃn/",
      "t": "n.",
      "m": "传统",
      "e": [],
      "u": "3",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "continue",
      "p": "/kən'tɪnjuː/",
      "t": "v.",
      "m": "持续；继续",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "have picnics",
      "p": "",
      "t": "v. phr.",
      "m": "野餐",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "work one's land",
      "p": "",
      "t": "v. phr.",
      "m": "耕种土地",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "take a trip",
      "p": "",
      "t": "v. phr.",
      "m": "去旅行",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "solar term",
      "p": "",
      "t": "n. phr.",
      "m": "节气",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "in fact",
      "p": "",
      "t": "phrase",
      "m": "事实上",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "be divided into",
      "p": "",
      "t": "v. phr.",
      "m": "被分成...",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "be based on",
      "p": "",
      "t": "v. phr.",
      "m": "基于...",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "play an important part in",
      "p": "",
      "t": "v. phr.",
      "m": "在...发挥重要作用",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "geography",
      "p": "/dʒi'ɒɡrəfi/",
      "t": "n.",
      "m": "地理",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "corner",
      "p": "/'kɔːnə(r)/",
      "t": "n.",
      "m": "角；角落",
      "e": [],
      "u": "2",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "activity",
      "p": "/æk'tɪvəti/",
      "t": "n.",
      "m": "活动",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "club",
      "p": "/klʌb/",
      "t": "n.",
      "m": "社团",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "practice",
      "p": "/'præktɪs/",
      "t": "v.",
      "m": "练习",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "solve",
      "p": "/sɒlv/",
      "t": "v.",
      "m": "解答；解决",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "develop",
      "p": "/dɪ'veləp/",
      "t": "v.",
      "m": "培养；发展",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "skill",
      "p": "/skɪl/",
      "t": "n.",
      "m": "技术；技能",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "teen",
      "p": "/tiːn/",
      "t": "n.",
      "m": "青少年",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "magazine",
      "p": "/ˌmæɡə'ziːn/",
      "t": "n.",
      "m": "杂志",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "teenager",
      "p": "/'tiːneɪdʒə(r)/",
      "t": "n.",
      "m": "(13至19岁)的少年",
      "e": [],
      "u": "2",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "greeting",
      "p": "/'ɡriːtɪŋ/",
      "t": "n.",
      "m": "问候；招呼",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "great",
      "p": "/ɡreɪt/",
      "t": "adj.",
      "m": "伟大的；极好的",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "energy",
      "p": "/'enədʒi/",
      "t": "n.",
      "m": "精力；活力",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "drama",
      "p": "/'drɑːmə/",
      "t": "n.",
      "m": "戏；戏剧",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "sincerely",
      "p": "/'sɪnsəli/",
      "t": "adv.",
      "m": "诚地；真诚地",
      "e": [],
      "u": "2",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "diary",
      "p": "/'daɪəri/",
      "t": "n.",
      "m": "日记；日记本",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "project",
      "p": "/'prɒdʒekt/",
      "t": "n.",
      "m": "课题",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "realize",
      "p": "/'rɪəlaɪz/",
      "t": "v.",
      "m": "意识到",
      "e": [],
      "u": "2",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "luckily",
      "p": "/'lʌkɪli/",
      "t": "adv.",
      "m": "幸运地",
      "e": [],
      "u": "2",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "just",
      "p": "/dʒʌst/",
      "t": "adv.",
      "m": "恰好",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "presentation",
      "p": "/ˌprezn'teɪʃn/",
      "t": "n.",
      "m": "演示",
      "e": [],
      "u": "2",
      "tags": [
        "suf_tion",
        "pre_pre"
      ]
    },
    {
      "w": "mood",
      "p": "/muːd/",
      "t": "n.",
      "m": "情绪；心情",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "mind",
      "p": "/maɪnd/",
      "t": "n.",
      "m": "意介",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "main",
      "p": "/meɪn/",
      "t": "adj.",
      "m": "主要的",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "pack",
      "p": "/pæk/",
      "t": "v.",
      "m": "(把......)打包",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "celebrate",
      "p": "/'selɪbreɪt/",
      "t": "v.",
      "m": "庆祝",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "success",
      "p": "/sək'ses/",
      "t": "n.",
      "m": "成功",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "rocky",
      "p": "/'rɒki/",
      "t": "adj.",
      "m": "困难的",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "do the dishes",
      "p": "",
      "t": "v. phr.",
      "m": "洗碗",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "junior high school",
      "p": "",
      "t": "n. phr.",
      "m": "初级中学",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "take part in",
      "p": "",
      "t": "v. phr.",
      "m": "参与",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "look forward to",
      "p": "",
      "t": "v. phr.",
      "m": "期待；盼望",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "daily life",
      "p": "",
      "t": "n. phr.",
      "m": "日常生活",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "go to bed",
      "p": "",
      "t": "v. phr.",
      "m": "去睡觉",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "get up",
      "p": "",
      "t": "v. phr.",
      "m": "起床",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "on foot",
      "p": "",
      "t": "phrase",
      "m": "步行",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "full of energy",
      "p": "",
      "t": "phrase",
      "m": "充满活力",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "get ready for",
      "p": "",
      "t": "v. phr.",
      "m": "准备好......",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "put on",
      "p": "",
      "t": "v. phr.",
      "m": "穿上；上演",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "clean up",
      "p": "",
      "t": "v. phr.",
      "m": "打扫",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "pick up",
      "p": "",
      "t": "v. phr.",
      "m": "捡起；取(某物)",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "guitar",
      "p": "/ɡɪ'tɑː(r)/",
      "t": "n.",
      "m": "吉他",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "honest",
      "p": "/'ɒnɪst/",
      "t": "adj.",
      "m": "诚实的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "patient",
      "p": "/'peɪʃnt/",
      "t": "adj.",
      "m": "有耐心的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "improve",
      "p": "/ɪm'pruːv/",
      "t": "v.",
      "m": "改进；改善",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "convenient",
      "p": "/kən'viːniənt/",
      "t": "adj.",
      "m": "方便的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "confident",
      "p": "/'kɒnfɪdənt/",
      "t": "adj.",
      "m": "自信的；有自信心的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "courage",
      "p": "/'kʌrɪdʒ/",
      "t": "n.",
      "m": "勇气；胆量",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "friendship",
      "p": "/'frendʃɪp/",
      "t": "n.",
      "m": "友谊；朋友关系",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "trust",
      "p": "/trʌst/",
      "t": "n.",
      "m": "信任；信赖",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "survey",
      "p": "/'sɜːveɪ/",
      "t": "n.",
      "m": "调查",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "personal",
      "p": "/'pɜːsənl/",
      "t": "adj.",
      "m": "个性的；个人的",
      "e": [],
      "u": "1",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "quality",
      "p": "/'kwɒləti/",
      "t": "n.",
      "m": "品质；品德",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "caring",
      "p": "/'keərɪŋ/",
      "t": "adj.",
      "m": "关心他人的；体贴的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "describe",
      "p": "/dɪ'skraɪb/",
      "t": "v.",
      "m": "描述；形容",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "appearance",
      "p": "/ə'pɪərəns/",
      "t": "n.",
      "m": "外貌",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "straight",
      "p": "/streɪt/",
      "t": "adj.",
      "m": "直的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "dark",
      "p": "/dɑːk/",
      "t": "adj.",
      "m": "褐色的；乌色的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "same",
      "p": "/seɪm/",
      "t": "adj.",
      "m": "同的；相同的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "basic",
      "p": "/'beɪsɪk/",
      "t": "adj.",
      "m": "基本的；基础的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "thought",
      "p": "/θɔːt/",
      "t": "n.",
      "m": "想法",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "glad",
      "p": "/ɡlæd/",
      "t": "adj.",
      "m": "高兴的；愉快的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "rise",
      "p": "/raɪz/",
      "t": "v.",
      "m": "上升",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "end",
      "p": "/end/",
      "t": "n.",
      "m": "结局",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "heart",
      "p": "/hɑːt/",
      "t": "n.",
      "m": "心",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "be good at",
      "p": "",
      "t": "v. phr.",
      "m": "擅长......",
      "e": [],
      "u": "1",
      "tags": [
        "phr_be_at"
      ]
    },
    {
      "w": "turn to somebody for help",
      "p": "",
      "t": "v. phr.",
      "m": "求助于某人",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "after school",
      "p": "",
      "t": "phrase",
      "m": "放学后",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "be willing to do something",
      "p": "",
      "t": "v. phr.",
      "m": "愿意做某事",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "medium height",
      "p": "",
      "t": "n. phr.",
      "m": "中等身高",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "modern dance",
      "p": "",
      "t": "n. phr.",
      "m": "现代舞",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "watch over",
      "p": "",
      "t": "v. phr.",
      "m": "照看；监督",
      "e": [],
      "u": "1",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "rise into",
      "p": "",
      "t": "v. phr.",
      "m": "上升到",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "cry out",
      "p": "",
      "t": "v. phr.",
      "m": "大叫；叫喊",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "cry oneself to sleep",
      "p": "",
      "t": "v. phr.",
      "m": "哭着睡着",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "wake up",
      "p": "",
      "t": "v. phr.",
      "m": "醒来",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "come and go",
      "p": "",
      "t": "v. phr.",
      "m": "来去",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "take somebody's place",
      "p": "",
      "t": "v. phr.",
      "m": "替代某人",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "come along",
      "p": "",
      "t": "v. phr.",
      "m": "出现",
      "e": [],
      "u": "1",
      "tags": []
    }
  ],
  "7B": {
    "1": [
      {
        "w": "advice",
        "p": "/əd'vaɪs/",
        "t": "n.",
        "m": "建议",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "encourage",
        "p": "/ɪn'kʌrɪdʒ/",
        "t": "v.",
        "m": "鼓励",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "retire",
        "p": "/rɪ'taɪə(r)/",
        "t": "v.",
        "m": "(使) 退休",
        "e": [],
        "u": "1",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "cheerful",
        "p": "/'tʃɪəfl/",
        "t": "adj.",
        "m": "快乐的; 高兴的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "community",
        "p": "/kə'mjuːnəti/",
        "t": "n.",
        "m": "社区",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "medical",
        "p": "/'medɪkl/",
        "t": "adj.",
        "m": "医学的",
        "e": [],
        "u": "1",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "fried",
        "p": "/fraɪd/",
        "t": "adj.",
        "m": "油腻的; 油炸的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "wherever",
        "p": "/weə'evə(r)/",
        "t": "conj.",
        "m": "各处; 处处",
        "e": [],
        "u": "1",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "future",
        "p": "/fjuːtʃə(r)/",
        "t": "n.",
        "m": "将来; 未来",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "soon",
        "p": "/suːn/",
        "t": "adv.",
        "m": "很快; 马上; 不久",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "smart",
        "p": "/smɑːt/",
        "t": "adj.",
        "m": "聪明的; 机敏的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "attention",
        "p": "/ə'tenʃn/",
        "t": "n.",
        "m": "专心; 注意力",
        "e": [],
        "u": "1",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "seldom",
        "p": "/'seldəm/",
        "t": "adv.",
        "m": "不常; 很少",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "bored",
        "p": "/bɔːrd/",
        "t": "adj.",
        "m": "(对某人/事物) 厌烦的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "strict",
        "p": "/strɪkt/",
        "t": "adj.",
        "m": "要求严格的; 严厉的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "relative",
        "p": "/'relətɪv/",
        "t": "n.",
        "m": "亲戚; 亲属",
        "e": [],
        "u": "1",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "uniform",
        "p": "/'juːnɪfɔːm/",
        "t": "n.",
        "m": "制服",
        "e": [],
        "u": "1",
        "tags": [
          "pre_un"
        ]
      },
      {
        "w": "personality",
        "p": "/ˌpɜːsə'næləti/",
        "t": "n.",
        "m": "性格; 个性",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "characteristic",
        "p": "/ˌkærəktə'rɪstɪk/",
        "t": "n.",
        "m": "特点; 品质",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "topic",
        "p": "/'tɒpɪk/",
        "t": "n.",
        "m": "话题; 标题",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "active",
        "p": "/'æktɪv/",
        "t": "adj.",
        "m": "忙碌的; 活跃的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "smartphone",
        "p": "/'smɑːtfəʊn/",
        "t": "n.",
        "m": "智能手机",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "give up",
        "p": "/ɡɪv ʌp/",
        "t": "v. phr.",
        "m": "放弃",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "used to",
        "p": "/juːst tu/",
        "t": "v.",
        "m": "曾经",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "be strict about",
        "p": "/bi strɪkt ə'baʊt/",
        "t": "v. phr.",
        "m": "要求严格",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "be worried about",
        "p": "/bi 'wʌrid ə'baʊt/",
        "t": "v. phr.",
        "m": "担心",
        "e": [],
        "u": "1",
        "tags": []
      }
    ],
    "2": [
      {
        "w": "wine",
        "p": "/waɪn/",
        "t": "n.",
        "m": "葡萄酒",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "match",
        "p": "/mætʃ/",
        "t": "v.",
        "m": "对比",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "rich",
        "p": "/rɪtʃ/",
        "t": "adj.",
        "m": "丰富的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "lie",
        "p": "/laɪ/",
        "t": "v.",
        "m": "坐着",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "cafe",
        "p": "/'kæfeɪ/",
        "t": "n.",
        "m": "咖啡馆; 小咖啡馆",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "excellent",
        "p": "/'eksələnt/",
        "t": "adj.",
        "m": "优秀的; 很好的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "sea",
        "p": "/siː/",
        "t": "n.",
        "m": "海",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "coast",
        "p": "/kəʊst/",
        "t": "n.",
        "m": "海岸",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "perfect",
        "p": "/'pɜːfɪkt/",
        "t": "adj.",
        "m": "完美的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "mostly",
        "p": "/'məʊstli/",
        "t": "adv.",
        "m": "主要; 通常",
        "e": [],
        "u": "2",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "receive",
        "p": "/rɪ'siːv/",
        "t": "v.",
        "m": "接收; 收到",
        "e": [],
        "u": "2",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "key",
        "p": "/kiː/",
        "t": "adj.",
        "m": "主要的; 关键的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "remain",
        "p": "/rɪ'meɪn/",
        "t": "v.",
        "m": "仍然是; 保持不变",
        "e": [],
        "u": "2",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "lift",
        "p": "/lɪft/",
        "t": "n.",
        "m": "电梯",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "step",
        "p": "/step/",
        "t": "n.",
        "m": "台阶",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "stair",
        "p": "/steə(r)/",
        "t": "n.",
        "m": "楼梯",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "motorcycle",
        "p": "/'məʊtəsaɪkl/",
        "t": "n.",
        "m": "摩托车",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "sightseeing",
        "p": "/'saɪtˌsiːɪŋ/",
        "t": "n.",
        "m": "观光; 游览",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "imagine",
        "p": "/ɪ'mædʒɪn/",
        "t": "v.",
        "m": "想象; 设想",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "destination",
        "p": "/ˌdestɪ'neɪʃn/",
        "t": "n.",
        "m": "目的地; 终点",
        "e": [],
        "u": "2",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "address",
        "p": "/ə'dres/",
        "t": "n.",
        "m": "地址",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "government",
        "p": "/'ɡʌvənmənt/",
        "t": "n.",
        "m": "政府",
        "e": [],
        "u": "2",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "unique",
        "p": "/ju'niːk/",
        "t": "adj.",
        "m": "特有的; 独特的",
        "e": [],
        "u": "2",
        "tags": [
          "pre_un"
        ]
      },
      {
        "w": "endangered",
        "p": "/ɪn'deɪndʒəd/",
        "t": "adj.",
        "m": "濒危的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "stretch",
        "p": "/streʧ/",
        "t": "v.",
        "m": "延伸",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "huge",
        "p": "/hjuːdʒ/",
        "t": "adj.",
        "m": "巨大的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "wild",
        "p": "/waɪld/",
        "t": "adj.",
        "m": "野的; 野生的; 自然环境",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "discover",
        "p": "/dɪ'skʌvə(r)/",
        "t": "v.",
        "m": "了解到; 查明",
        "e": [],
        "u": "2",
        "tags": [
          "suf_er",
          "pre_dis"
        ]
      },
      {
        "w": "volcano",
        "p": "/vɒl'keɪnəʊ/",
        "t": "n.",
        "m": "火山",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "reindeer",
        "p": "/'reɪndɪə(r)/",
        "t": "n.",
        "m": "驯鹿",
        "e": [],
        "u": "2",
        "tags": [
          "suf_er",
          "pre_re"
        ]
      },
      {
        "w": "snowmobile",
        "p": "/'snəʊməbaɪl/",
        "t": "n.",
        "m": "雪地摩托车",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "wolf",
        "p": "/wʊlf/",
        "t": "n.",
        "m": "狼",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "department store",
        "p": "/dɪ'pɑːtmənt stɔː(r)/",
        "t": "n. phr.",
        "m": "百货商店",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "prefer to",
        "p": "/prɪ'fɜː(r) tu/",
        "t": "v. phr.",
        "m": "更喜欢",
        "e": [],
        "u": "2",
        "tags": [
          "pre_pre"
        ]
      },
      {
        "w": "by hand",
        "p": "/baɪ hænd/",
        "t": "phrase",
        "m": "用手工",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "go sightseeing",
        "p": "/ɡəʊ 'saɪtˌsiːɪŋ/",
        "t": "v. phr.",
        "m": "去观光",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "go on a trip",
        "p": "/ɡəʊ ɒn ə trɪp/",
        "t": "v. phr.",
        "m": "去旅行",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "set up",
        "p": "/set ʌp/",
        "t": "v. phr.",
        "m": "建立",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "giant panda",
        "p": "/'dʒaɪənt ˈpændə/",
        "t": "n. phr.",
        "m": "大熊猫",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "in the wild",
        "p": "/ɪn ðə waɪld/",
        "t": "phrase",
        "m": "在野外",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "all year round",
        "p": "/ɔːl jɪə(r) raʊnd/",
        "t": "phrase",
        "m": "全年",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "golden monkey",
        "p": "/'ɡəʊldən ˈmʌŋki/",
        "t": "n. phr.",
        "m": "金丝猴",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "go hiking",
        "p": "/ɡəʊ ˈhaɪkɪŋ/",
        "t": "v. phr.",
        "m": "去远足",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "hot spring",
        "p": "/hɒt sprɪŋ/",
        "t": "n. phr.",
        "m": "温泉",
        "e": [],
        "u": "2",
        "tags": []
      }
    ],
    "3": [
      {
        "w": "branch",
        "p": "/brɑːntʃ/",
        "t": "n.",
        "m": "树枝",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "root",
        "p": "/ruːt/",
        "t": "n.",
        "m": "根; 根茎",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "silent",
        "p": "/'saɪlənt/",
        "t": "adj.",
        "m": "不说话的; 沉默的",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "overlook",
        "p": "/ˌəʊvə'lʊk/",
        "t": "v.",
        "m": "忽略; 未注意到",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "human",
        "p": "/'hjuːmən/",
        "t": "n.",
        "m": "人",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "oxygen",
        "p": "/'ɒksɪdʒən/",
        "t": "n.",
        "m": "氧气",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "create",
        "p": "/kri'eɪt/",
        "t": "v.",
        "m": "创造",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "environment",
        "p": "/ɪn'vaɪrənmənt/",
        "t": "n.",
        "m": "自然环境",
        "e": [],
        "u": "3",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "convenient",
        "p": "/kən'viːniənt/",
        "t": "adj.",
        "m": "便利的; 方便的",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "furniture",
        "p": "/'fɜːrnɪtʃə(r)/",
        "t": "n.",
        "m": "(可移动的) 家具",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "wood",
        "p": "/wʊd/",
        "t": "n.",
        "m": "木; 木头",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "treat",
        "p": "/triːt/",
        "t": "v.",
        "m": "以......态度对待; 以......方式对待",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "communicate",
        "p": "/kə'mjuːnɪkeɪt/",
        "t": "v.",
        "m": "交流; 沟通",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "species",
        "p": "/'spiːʃiːz/",
        "t": "n.",
        "m": "种; 物种",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "product",
        "p": "/'prɒdʌkt/",
        "t": "n.",
        "m": "产品; 制品",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "side",
        "p": "/saɪd/",
        "t": "n.",
        "m": "一面",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "borrow",
        "p": "/'bɒrəʊ/",
        "t": "v.",
        "m": "借; 借用",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "dig",
        "p": "/dɪɡ/",
        "t": "v.",
        "m": "掘; 挖 (地); 凿 (洞)",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "hole",
        "p": "/həʊl/",
        "t": "n.",
        "m": "洞; 孔; 坑",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "stick",
        "p": "/stɪk/",
        "t": "n.",
        "m": "条; 杆",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "accident",
        "p": "/'æksɪdənt/",
        "t": "n.",
        "m": "意外; 偶然的事",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "knowledge",
        "p": "/'nɒlɪdʒ/",
        "t": "n.",
        "m": "知识; 学问",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "character",
        "p": "/'kærəktə(r)/",
        "t": "n.",
        "m": "文字",
        "e": [],
        "u": "3",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "spread",
        "p": "/spred/",
        "t": "v.",
        "m": "传播",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "translation",
        "p": "/træn'sleɪʃn/",
        "t": "n.",
        "m": "译本; 译文",
        "e": [],
        "u": "3",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "take in",
        "p": "",
        "t": "v. phr.",
        "m": "吸收",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "greenhouse gas",
        "p": "",
        "t": "n. phr.",
        "m": "温室气体",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "begin with",
        "p": "",
        "t": "v. phr.",
        "m": "首先; 第一点",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "come from",
        "p": "",
        "t": "v. phr.",
        "m": "来自",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "look around",
        "p": "",
        "t": "v. phr.",
        "m": "环视; 四下察看",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "be made of",
        "p": "",
        "t": "v. phr.",
        "m": "由......制成",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "for example",
        "p": "",
        "t": "phrase",
        "m": "例如; 譬如",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "communicate with",
        "p": "",
        "t": "v. phr.",
        "m": "与......沟通",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "call on",
        "p": "",
        "t": "v. phr.",
        "m": "号召; 动员; 要求",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "according to",
        "p": "",
        "t": "phrase",
        "m": "按 (......所说)",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "by accident",
        "p": "",
        "t": "phrase",
        "m": "偶然; 意外地",
        "e": [],
        "u": "3",
        "tags": []
      }
    ],
    "4": [
      {
        "w": "dolphin",
        "p": "/'dɒlfɪn/",
        "t": "n.",
        "m": "海豚",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "hen",
        "p": "/hen/",
        "t": "n.",
        "m": "母鸡",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "blind",
        "p": "/blaɪnd/",
        "t": "adj.",
        "m": "adj. 瞎的; 失明的",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "wool",
        "p": "/wʊl/",
        "t": "n.",
        "m": "羊毛",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "sir",
        "p": "/sɜː(r)/",
        "t": "n.",
        "m": "先生",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "receptionist",
        "p": "/rɪ'sepʃənɪst/",
        "t": "n.",
        "m": "接待员",
        "e": [],
        "u": "4",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "allow",
        "p": "/ə'laʊ/",
        "t": "v.",
        "m": "允许进入 (或 出去、通过)",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "asleep",
        "p": "/ə'sliːp/",
        "t": "adj.",
        "m": "睡着的",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "smoke",
        "p": "/sməʊk/",
        "t": "n.",
        "m": "烟",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "fireman",
        "p": "/'faɪəmən/",
        "t": "n.",
        "m": "消防队员",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "type",
        "p": "/taɪp/",
        "t": "n.",
        "m": "类型",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "search-and-rescue",
        "p": "/sɜːtʃ ænd 'rezkjuː/",
        "t": "n.",
        "m": "搜索救援",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "search",
        "p": "/sɜːtʃ/",
        "t": "n.",
        "m": "搜索",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "service",
        "p": "/'sɜːvɪs/",
        "t": "n.",
        "m": "服务",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "disaster",
        "p": "/dɪ'zɑːstə(r)/",
        "t": "n.",
        "m": "灾难; 灾害",
        "e": [],
        "u": "4",
        "tags": [
          "suf_er",
          "pre_dis"
        ]
      },
      {
        "w": "guest",
        "p": "/ɡest/",
        "t": "n.",
        "m": "旅客; 房客",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "team",
        "p": "/tiːm/",
        "t": "n.",
        "m": "组; 团队",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "transport",
        "p": "/træn'spɔːt/",
        "t": "n.",
        "m": "运输; 交通",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "guard",
        "p": "/ɡɑːd/",
        "t": "n.",
        "m": "守卫; 保卫",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "honey",
        "p": "/'hʌni/",
        "t": "n.",
        "m": "蜂蜜",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "material",
        "p": "/mə'tɪəriəl/",
        "t": "n.",
        "m": "材料; 原料",
        "e": [],
        "u": "4",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "either",
        "p": "/'aɪðə(r)/",
        "t": "adj.",
        "m": "也",
        "e": [],
        "u": "4",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "shark",
        "p": "/ʃɑːk/",
        "t": "n.",
        "m": "鲨鱼",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "scared",
        "p": "/skeəd/",
        "t": "adj.",
        "m": "害怕的; 恐惧的",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "grey",
        "p": "/ɡreɪ/",
        "t": "adj.",
        "m": "灰色的",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "somewhere",
        "p": "/'sʌmweə(r)/",
        "t": "adv.",
        "m": "在某处; 到某处",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "probably",
        "p": "/'prɒbəbli/",
        "t": "adv.",
        "m": "很可能; 大概",
        "e": [],
        "u": "4",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "source",
        "p": "/sɔːs/",
        "t": "n.",
        "m": "来源; 出处",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "sometime",
        "p": "/'sʌmtaɪm/",
        "t": "adv.",
        "m": "在某时",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "extinct",
        "p": "/ɪk'stɪŋkt/",
        "t": "adj.",
        "m": "已灭绝的",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "effort",
        "p": "/'efət/",
        "t": "n.",
        "m": "努力; 试图",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "lead (somebody)",
        "p": "",
        "t": "v. phr.",
        "m": "带着(某人)",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "fall asleep",
        "p": "",
        "t": "v. phr.",
        "m": "睡着",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "get down",
        "p": "",
        "t": "v. phr.",
        "m": "俯身; 趴下",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "fire engine",
        "p": "",
        "t": "n. phr.",
        "m": "消防车",
        "e": [],
        "u": "4",
        "tags": []
      }
    ],
    "5": [
      {
        "w": "everyday",
        "p": "/'evrideɪ/",
        "t": "adj.",
        "m": "每天的; 日常的",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "journey",
        "p": "/'dʒɜːni/",
        "t": "n.",
        "m": "(尤指长途) 旅行",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "drop",
        "p": "/drɒp/",
        "t": "v.",
        "m": "滴; 洒; 掉落",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "tap",
        "p": "/tæp/",
        "t": "n.",
        "m": "水龙头; 敲击声",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "voice",
        "p": "/vɔɪs/",
        "t": "n.",
        "m": "说话声; 声音",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "eventually",
        "p": "/ɪ'ventʃuəli/",
        "t": "adv.",
        "m": "最后; 终于",
        "e": [],
        "u": "5",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "pipe",
        "p": "/paɪp/",
        "t": "n.",
        "m": "管子; 管道",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "return",
        "p": "/rɪ'tɜːn/",
        "t": "v.",
        "m": "回去; 返回",
        "e": [],
        "u": "5",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "rush",
        "p": "/rʌʃ/",
        "t": "v.",
        "m": "迅速移动; 冲",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "bath",
        "p": "/bɑːθ/",
        "t": "n.",
        "m": "洗澡; 沐浴",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "salt",
        "p": "/sɔːlt/",
        "t": "n.",
        "m": "盐; 含盐的",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "bream",
        "p": "/briːm/",
        "t": "n.",
        "m": "鲻鱼",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "fix",
        "p": "/fɪks/",
        "t": "v.",
        "m": "修理",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "public",
        "p": "/'pʌblɪk/",
        "t": "adj.",
        "m": "公共的; 公开的",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "population",
        "p": "/ˌpɒpju'leɪʃn/",
        "t": "n.",
        "m": "人口",
        "e": [],
        "u": "5",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "agriculture",
        "p": "/'æɡrɪkʌltʃə(r)/",
        "t": "n.",
        "m": "农业",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "trade",
        "p": "/treɪd/",
        "t": "n.",
        "m": "贸易; 交易",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "industry",
        "p": "/'ɪndəstri/",
        "t": "n.",
        "m": "工业; 产业",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "goods",
        "p": "/ɡʊdz/",
        "t": "n.",
        "m": "商品; 货物",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "overseas",
        "p": "/ˌəʊvə'siːz/",
        "t": "adv.",
        "m": "在国外; 向海外",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "global",
        "p": "/'ɡləʊbl/",
        "t": "adj.",
        "m": "全球的; 全世界的",
        "e": [],
        "u": "5",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "income",
        "p": "/'ɪnkʌm/",
        "t": "n.",
        "m": "收入; 收益",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "nearly",
        "p": "/'nɪəli/",
        "t": "adv.",
        "m": "几乎; 差不多; 将近",
        "e": [],
        "u": "5",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "business",
        "p": "/'bɪznəs/",
        "t": "n.",
        "m": "买卖; 生意",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "leisure",
        "p": "/'leʒə(r)/",
        "t": "n.",
        "m": "闲暇; 休闲",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "throughout",
        "p": "/θruː'aʊt/",
        "t": "prep.",
        "m": "自始至终; 贯穿",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "duty",
        "p": "/'djuːti/",
        "t": "n.",
        "m": "责任; 义务",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "a bit",
        "p": "",
        "t": "phrase",
        "m": "有点儿; 稍微",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "at once",
        "p": "",
        "t": "phrase",
        "m": "立即; 马上",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "drinking water",
        "p": "",
        "t": "n. phr.",
        "m": "饮用水",
        "e": [],
        "u": "5",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "play a role in",
        "p": "",
        "t": "v. phr.",
        "m": "在......中起作用",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "steam engine",
        "p": "",
        "t": "n. phr.",
        "m": "蒸汽机",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "as a result",
        "p": "",
        "t": "phrase",
        "m": "作为结果; 因此",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "make sure",
        "p": "",
        "t": "v. phr.",
        "m": "确保; 保证",
        "e": [],
        "u": "5",
        "tags": []
      }
    ],
    "6": [
      {
        "w": "battery",
        "p": "/'bætəri/",
        "t": "n.",
        "m": "电池",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "electricity",
        "p": "/ɪ'lektrɪsəti/",
        "t": "n.",
        "m": "电; 电能",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "switch off",
        "p": "/swɪtʃ ɒf/",
        "t": "v. phr.",
        "m": "关闭 (电灯、机器等)",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "task",
        "p": "/tɑːsk/",
        "t": "n.",
        "m": "任务; 活动",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "while",
        "p": "/waɪl/",
        "t": "conj.",
        "m": "在......期间; 一会儿",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "tablet",
        "p": "/'tæblət/",
        "t": "n.",
        "m": "平板电脑",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "fridge",
        "p": "/frɪdʒ/",
        "t": "n.",
        "m": "冰箱",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "yogurt",
        "p": "/'jɒɡət/",
        "t": "n.",
        "m": "酸奶",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "apartment",
        "p": "/ə'pɑːtmənt/",
        "t": "n.",
        "m": "公寓; 套房",
        "e": [],
        "u": "6",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "household",
        "p": "/'haʊshəʊld/",
        "t": "n.",
        "m": "家庭; 家政",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "against",
        "p": "/ə'ɡenst/",
        "t": "prep.",
        "m": "反对; 抵抗; 紧靠",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "speed",
        "p": "/spiːd/",
        "t": "n.",
        "m": "速度; 速率",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "safety",
        "p": "/'seɪfti/",
        "t": "n.",
        "m": "安全; 安全性",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "instruction",
        "p": "/ɪn'strʌkʃn/",
        "t": "n.",
        "m": "用语说明; 指示",
        "e": [],
        "u": "6",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "connect",
        "p": "/kə'nekt/",
        "t": "v.",
        "m": "(使) 连接",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "rule",
        "p": "/ruːl/",
        "t": "n.",
        "m": "规则; 条例",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "climate",
        "p": "/'klaɪmət/",
        "t": "n.",
        "m": "气候",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "amount",
        "p": "/ə'maʊnt/",
        "t": "n.",
        "m": "数量",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "power",
        "p": "/'paʊə(r)/",
        "t": "v.",
        "m": "推动 (机器或车辆)",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "television",
        "p": "/'telɪvɪʒn/",
        "t": "n.",
        "m": "电视",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "landmark",
        "p": "/'lændmɑːk/",
        "t": "n.",
        "m": "地标; 特征",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "have something in common",
        "p": "",
        "t": "v. phr.",
        "m": "有相同的特征 (或特点)",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "light bulb",
        "p": "",
        "t": "n. phr.",
        "m": "电灯泡",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "air conditioner",
        "p": "",
        "t": "n. phr.",
        "m": "空调; 空调设备",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "video game",
        "p": "",
        "t": "n. phr.",
        "m": "电子游戏",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "run out of",
        "p": "",
        "t": "v. phr.",
        "m": "用完; 耗尽",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "cover",
        "p": "/'kʌvə(r)/",
        "t": "v.",
        "m": "覆盖",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "average",
        "p": "/'ævərɪdʒ/",
        "t": "n.",
        "m": "平均; 平均数",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "culture",
        "p": "/'kʌltʃə(r)/",
        "t": "n.",
        "m": "文化",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "global",
        "p": "/'ɡləʊbl/",
        "t": "adj.",
        "m": "全球的; 全世界的",
        "e": [],
        "u": "6",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "history",
        "p": "/'hɪstri/",
        "t": "n.",
        "m": "历史",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "together",
        "p": "/tə'ɡeðə(r)/",
        "t": "adv.",
        "m": "一起; 共同",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "language",
        "p": "/'læŋɡwɪdʒ/",
        "t": "n.",
        "m": "语言",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "science",
        "p": "/'saɪəns/",
        "t": "n.",
        "m": "科学",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "fact",
        "p": "/fækt/",
        "t": "n.",
        "m": "事实",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "study",
        "p": "/'stʌdi/",
        "t": "v.",
        "m": "学习; 研究",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "special",
        "p": "/'speʃl/",
        "t": "adj.",
        "m": "特别的; 特殊的",
        "e": [],
        "u": "6",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "useful",
        "p": "/'juːsfl/",
        "t": "adj.",
        "m": "有用的",
        "e": [],
        "u": "6",
        "tags": []
      }
    ],
    "7": [
      {
        "w": "contribution",
        "p": "/ˌkɒntrɪ'bjuːʃn/",
        "t": "n.",
        "m": "贡献",
        "e": [],
        "u": "7",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "hero",
        "p": "/'hɪərəʊ/",
        "t": "n.",
        "m": "英雄",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "pioneer",
        "p": "/ˌpaɪə'niːə(r)/",
        "t": "n.",
        "m": "先锋; 先驱",
        "e": [],
        "u": "7",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "technology",
        "p": "/tek'nɒlədʒi/",
        "t": "n.",
        "m": "科技",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "receive",
        "p": "/rɪ'siːv/",
        "t": "v.",
        "m": "拿到; 收到",
        "e": [],
        "u": "7",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "award",
        "p": "/ə'wɔːd/",
        "t": "n.",
        "m": "奖; 奖项",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "engineering",
        "p": "/ˌendʒɪ'nɪərɪŋ/",
        "t": "n.",
        "m": "工程学",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "education",
        "p": "/ˌedju'keɪʃn/",
        "t": "n.",
        "m": "教育",
        "e": [],
        "u": "7",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "spend",
        "p": "/spend/",
        "t": "v.",
        "m": "花 (时间); 度过",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "earn",
        "p": "/ɜːn/",
        "t": "v.",
        "m": "赚 (钱); 赢得",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "common",
        "p": "/'kɒmən/",
        "t": "adj.",
        "m": "共同的; 普通的",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "difference",
        "p": "/'dɪfrəns/",
        "t": "n.",
        "m": "差别; 差异",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "habit",
        "p": "/'hæbɪt/",
        "t": "n.",
        "m": "习惯",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "useful",
        "p": "/'juːsfl/",
        "t": "adj.",
        "m": "有用的",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "environment",
        "p": "/ɪn'vaɪrənmənt/",
        "t": "n.",
        "m": "环境",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "improve",
        "p": "/ɪm'pruːv/",
        "t": "v.",
        "m": "改善; 提高",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "success",
        "p": "/sək'ses/",
        "t": "n.",
        "m": "成功",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "change",
        "p": "/tʃeɪndʒ/",
        "t": "v.",
        "m": "改变",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "life",
        "p": "/laɪf/",
        "t": "n.",
        "m": "生活; 生命",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "important",
        "p": "/ɪm'pɔːtənt/",
        "t": "adj.",
        "m": "重要的",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "goal",
        "p": "/ɡəʊl/",
        "t": "n.",
        "m": "目标",
        "e": [],
        "u": "7",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "achieve",
        "p": "/ə'tʃiːv/",
        "t": "v.",
        "m": "达到; 实现",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "decision",
        "p": "/dɪ'sɪʒn/",
        "t": "n.",
        "m": "决定",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "/plan/",
        "p": "/plæn/",
        "t": "n.",
        "m": "计划",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "hard",
        "p": "/hɑːd/",
        "t": "adj.",
        "m": "努力的; 困难的",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "way",
        "p": "/weɪ/",
        "t": "n.",
        "m": "方式; 方法",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "focus",
        "p": "/'fəʊkəs/",
        "t": "v.",
        "m": "集中; 专注",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "effort",
        "p": "/'efət/",
        "t": "n.",
        "m": "努力",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "result",
        "p": "/rɪ'zʌlt/",
        "t": "n.",
        "m": "结果",
        "e": [],
        "u": "7",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "succeed",
        "p": "/sək'siːd/",
        "t": "v.",
        "m": "成功",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "build",
        "p": "/bɪld/",
        "t": "v.",
        "m": "建立; 建设",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "together",
        "p": "/tə'ɡeðə(r)/",
        "t": "adv.",
        "m": "一起",
        "e": [],
        "u": "7",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "experience",
        "p": "/ɪk'spɪəriəns/",
        "t": "n.",
        "m": "经验; 经历",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "understand",
        "p": "/ˌʌndə'stænd/",
        "t": "v.",
        "m": "理解; 明白",
        "e": [],
        "u": "7",
        "tags": [
          "pre_un"
        ]
      },
      {
        "w": "knowledge",
        "p": "/'nɒlɪdʒ/",
        "t": "n.",
        "m": "知识",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "ability",
        "p": "/ə'bɪləti/",
        "t": "n.",
        "m": "能力",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "grow",
        "p": "/ɡrəʊ/",
        "t": "v.",
        "m": "增长; 成长",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "development",
        "p": "/dɪ'veləpmənt/",
        "t": "n.",
        "m": "发展",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "progress",
        "p": "/'prəʊɡres/",
        "t": "n.",
        "m": "进步",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "future",
        "p": "/fjuːtʃə(r)/",
        "t": "n.",
        "m": "未来",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "society",
        "p": "/sə'saɪəti/",
        "t": "n.",
        "m": "社会",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "individual",
        "p": "/ˌɪndɪ'vɪdʒuəl/",
        "t": "n.",
        "m": "个人",
        "e": [],
        "u": "7",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "well-respected",
        "p": "/wel rɪ'spektɪd/",
        "t": "adj.",
        "m": "受人尊敬的",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "found",
        "p": "/faʊnd/",
        "t": "v.",
        "m": "建立; 创立",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "mission",
        "p": "/'mɪʃn/",
        "t": "n.",
        "m": "使命; 任务",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "honour",
        "p": "/'ɒnə(r)/",
        "t": "n.",
        "m": "荣誉; 尊敬",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "approval",
        "p": "/ə'pruːvl/",
        "t": "n.",
        "m": "赞成; 认可",
        "e": [],
        "u": "7",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "prize",
        "p": "/praɪz/",
        "t": "n.",
        "m": "奖品; 奖金",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "beauty",
        "p": "/'bjuːti/",
        "t": "n.",
        "m": "美丽; 美",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "female",
        "p": "/'fiːmeɪl/",
        "t": "adj.",
        "m": "女性的",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "admire",
        "p": "/əd'maɪə(r)/",
        "t": "v.",
        "m": "钦佩; 羡慕",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "inspire",
        "p": "/ɪn'spaɪə(r)/",
        "t": "v.",
        "m": "鼓舞; 激发",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "regular",
        "p": "/'reɡjələ(r)/",
        "t": "adj.",
        "m": "规则的; 定期的",
        "e": [],
        "u": "7",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "normal",
        "p": "/'nɔːml/",
        "t": "adj.",
        "m": "正常的; 普通的",
        "e": [],
        "u": "7",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "simple",
        "p": "/'sɪmpl/",
        "t": "adj.",
        "m": "简单的",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "easy",
        "p": "/'iːzi/",
        "t": "adj.",
        "m": "容易的",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "often",
        "p": "/'ɒfn/",
        "t": "adv.",
        "m": "经常",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "usually",
        "p": "/'juːʒuəli/",
        "t": "adv.",
        "m": "通常",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "sometimes",
        "p": "/'sʌmtaɪmz/",
        "t": "adv.",
        "m": "有时",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "always",
        "p": "/'ɔːlweɪz/",
        "t": "adv.",
        "m": "总是",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "never",
        "p": "/'nevə(r)/",
        "t": "adv.",
        "m": "从不",
        "e": [],
        "u": "7",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "maybe",
        "p": "/'meibi/",
        "t": "adv.",
        "m": "也许",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "perhaps",
        "p": "/pə'hæps/",
        "t": "adv.",
        "m": "也许",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "really",
        "p": "/'riːəli/",
        "t": "adv.",
        "m": "真正地",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly",
          "pre_re"
        ]
      },
      {
        "w": "indeed",
        "p": "/ɪn'diːd/",
        "t": "adv.",
        "m": "的确; 确实",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "certainly",
        "p": "/'sɜːtnli/",
        "t": "adv.",
        "m": "当然; 肯定",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "surely",
        "p": "/'ʃʊəli/",
        "t": "adv.",
        "m": "无疑地; 肯定地",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "definitely",
        "p": "/'defɪnətli/",
        "t": "adv.",
        "m": "肯定地; 明确地",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "probably",
        "p": "/'prɒbəbli/",
        "t": "adv.",
        "m": "很可能",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "possibly",
        "p": "/'pɒsəbli/",
        "t": "adv.",
        "m": "可能地",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "extremely",
        "p": "/ɪk'striːmli/",
        "t": "adv.",
        "m": "极端地; 非常",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "very",
        "p": "/'veri/",
        "t": "adv.",
        "m": "非常",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "quite",
        "p": "/kwaɪt/",
        "t": "adv.",
        "m": "相当; 相当地",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "a bit",
        "p": "",
        "t": "phrase",
        "m": "一点点",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "slightly",
        "p": "/'slaɪtli/",
        "t": "adv.",
        "m": "轻微地",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "moderately",
        "p": "/'mɒdərətli/",
        "t": "adv.",
        "m": "适度地",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "somewhat",
        "p": "/'sʌmwɒt/",
        "t": "adv.",
        "m": "稍微; 有点",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "rather",
        "p": "/'rɑːðə(r)/",
        "t": "adv.",
        "m": "相当; 颇",
        "e": [],
        "u": "7",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "fairly",
        "p": "/'feəli/",
        "t": "adv.",
        "m": "相当地",
        "e": [],
        "u": "7",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "pretty",
        "p": "/'prɪti/",
        "t": "adv.",
        "m": "相当; 很",
        "e": [],
        "u": "7",
        "tags": [
          "pre_pre"
        ]
      },
      {
        "w": "role model",
        "p": "/rəʊl 'mɒdl/",
        "t": "n. phr.",
        "m": "榜样; 模范",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "in the field of",
        "p": "",
        "t": "phrase",
        "m": "在......领域",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "devote oneself to",
        "p": "",
        "t": "v. phr.",
        "m": "献身于; 致力于",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "college entrance examination",
        "p": "",
        "t": "n. phr.",
        "m": "大学入学考试",
        "e": [],
        "u": "7",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "look up to",
        "p": "",
        "t": "v. phr.",
        "m": "敬仰; 钦佩",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "sugar pill",
        "p": "",
        "t": "n. phr.",
        "m": "糖丸",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "chief engineer",
        "p": "",
        "t": "n. phr.",
        "m": "总工程师",
        "e": [],
        "u": "7",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "put out",
        "p": "",
        "t": "v. phr.",
        "m": "熄灭; 扑灭",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "be able to",
        "p": "",
        "t": "v. phr.",
        "m": "能够",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "be proud of",
        "p": "",
        "t": "v. phr.",
        "m": "为......而自豪",
        "e": [],
        "u": "7",
        "tags": [
          "phr_be_of"
        ]
      }
    ],
    "8": [
      {
        "w": "possible",
        "p": "/'pɒsəbl/",
        "t": "adj.",
        "m": "可能的",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "athlete",
        "p": "/'æθliːt/",
        "t": "n.",
        "m": "运动员",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "biologist",
        "p": "/baɪ'ɒlədʒɪst/",
        "t": "n.",
        "m": "生物学家",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "instrument",
        "p": "/'ɪnstrəmənt/",
        "t": "n.",
        "m": "乐器; 仪器",
        "e": [],
        "u": "8",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "interest",
        "p": "/'ɪntrəst/",
        "t": "n.",
        "m": "兴趣; 爱好",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "career",
        "p": "/kə'rɪə(r)/",
        "t": "n.",
        "m": "职业",
        "e": [],
        "u": "8",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "lifetime",
        "p": "/'laɪftaɪm/",
        "t": "n.",
        "m": "终身; 一生",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "diamond",
        "p": "/'daɪəmənd/",
        "t": "n.",
        "m": "钻石",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "belt",
        "p": "/belt/",
        "t": "n.",
        "m": "腰带",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "shoot",
        "p": "/ʃuːt/",
        "t": "v.",
        "m": "冲; 飞驰",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "extremely",
        "p": "/ɪk'striːmli/",
        "t": "adv.",
        "m": "极其; 非常",
        "e": [],
        "u": "8",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "curious",
        "p": "/'kjʊəriəs/",
        "t": "adj.",
        "m": "求知欲强的; 好奇的",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "increase",
        "p": "/ɪn'kriːs/",
        "t": "v.",
        "m": "增长; 增多; 增加",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "host",
        "p": "/həʊst/",
        "t": "v.",
        "m": "主持",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "beyond",
        "p": "/bɪ'jɒnd/",
        "t": "prep.",
        "m": "在......另一边; (在)更远处",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "audience",
        "p": "/'ɔːdiəns/",
        "t": "n.",
        "m": "观众; 听众",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "actuality",
        "p": "/ˌæktʃu'æləti/",
        "t": "n.",
        "m": "事实",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "sail",
        "p": "/seɪl/",
        "t": "v.",
        "m": "驾驶 (或乘坐) 帆船航行",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "ability",
        "p": "/ə'bɪləti/",
        "t": "n.",
        "m": "才能; 本领",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "review",
        "p": "/rɪ'vjuː/",
        "t": "n.",
        "m": "评论",
        "e": [],
        "u": "8",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "perform",
        "p": "/pə'fɔːm/",
        "t": "v.",
        "m": "演出; 表演",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "kiss",
        "p": "/kɪs/",
        "t": "n.",
        "m": "吻",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "hug",
        "p": "/hʌɡ/",
        "t": "v.",
        "m": "拥抱; 搂抱",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "method",
        "p": "/'meθəd/",
        "t": "n.",
        "m": "方法; 办法",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "compare",
        "p": "/kəm'peə(r)/",
        "t": "v.",
        "m": "将......比作",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "lecture",
        "p": "/'lektʃə(r)/",
        "t": "n.",
        "m": "讲座; 讲课; 演讲",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "go outside",
        "p": "",
        "t": "v. phr.",
        "m": "外出",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "more and more",
        "p": "",
        "t": "phrase",
        "m": "越来越多",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "body language",
        "p": "",
        "t": "n. phr.",
        "m": "肢体语言",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "dream of",
        "p": "",
        "t": "v. phr.",
        "m": "梦想",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "come true",
        "p": "",
        "t": "v. phr.",
        "m": "实现; 成为现实",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "nowadays",
        "p": "/'naʊədeɪz/",
        "t": "adv.",
        "m": "现今; 目前",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "human being",
        "p": "/'hjuːmən ˌbiːɪŋ/",
        "t": "n.",
        "m": "人",
        "e": [],
        "u": "8",
        "tags": []
      }
    ]
  },
  "8A": [
    {
      "w": "dinosaur",
      "p": "/'daɪnəsɔː(r)/",
      "t": "n.",
      "m": "恐龙",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "intelligent",
      "p": "/ɪn'telɪdʒənt/",
      "t": "adj.",
      "m": "有才智的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "talented",
      "p": "/'tæləntɪd/",
      "t": "adj.",
      "m": "有才能的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "artistic",
      "p": "/ɑː'tɪstɪk/",
      "t": "(尤指)有艺术天赋的",
      "m": "有艺术天赋的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "perhaps",
      "p": "/pə'hæps/",
      "t": "adv.",
      "m": "可能；大概；也许",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "notebook",
      "p": "/'nəʊtbʊk/",
      "t": "n.",
      "m": "笔记本",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "vehicle",
      "p": "/'viːɪkl/",
      "t": "n.",
      "m": "交通工具",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "prehistoric",
      "p": "/pri:'hɪstɒrɪk/",
      "t": "adj.",
      "m": "史前的",
      "e": [],
      "u": "1",
      "tags": [
        "pre_pre"
      ]
    },
    {
      "w": "completely",
      "p": "/kəm'pliːtli/",
      "t": "adv.",
      "m": "完全地；彻底地",
      "e": [],
      "u": "1",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "original",
      "p": "/ə'rɪdʒənl/",
      "t": "adj.",
      "m": "原来的；起初的",
      "e": [],
      "u": "1",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "birth",
      "p": "/bɜːθ/",
      "t": "n.",
      "m": "出生",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "suffering",
      "p": "/'sʌfərɪŋ/",
      "t": "n.",
      "m": "苦难；疼痛",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "artist",
      "p": "/'ɑːtɪst/",
      "t": "n.",
      "m": "艺术家；(尤指)画家",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "death",
      "p": "/deθ/",
      "t": "n.",
      "m": "死亡",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "whole",
      "p": "/həʊl/",
      "t": "adj.",
      "m": "全部的；所有的",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "piece",
      "p": "/piːs/",
      "t": "n.",
      "m": "一首，一篇 (作品)",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "editor",
      "p": "/'edɪtə(r)/",
      "t": "n.",
      "m": "(书籍的) 编辑",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "organize",
      "p": "/'ɔːɡənaɪz/",
      "t": "v.",
      "m": "安排；组织",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "record",
      "p": "/rɪ'kɔːd/",
      "t": "v.",
      "m": "记录",
      "e": [],
      "u": "1",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "die out",
      "p": "",
      "t": "v. phr.",
      "m": "灭绝",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "a type of",
      "p": "",
      "t": "phrase",
      "m": "一种",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "be related to",
      "p": "",
      "t": "phrase",
      "m": "与……属于同一种类",
      "e": [],
      "u": "1",
      "tags": []
    },
    {
      "w": "flight",
      "p": "/flaɪt/",
      "t": "n.",
      "m": "旅程 (尤指飞机的)",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "schedule",
      "p": "/'ʃedjuːl/",
      "t": "n.",
      "m": "日程安排",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "everywhere",
      "p": "/'evriweə(r)/",
      "t": "adv.",
      "m": "到处; 各个地方",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "challenge",
      "p": "/'tʃælɪndʒ/",
      "t": "v.",
      "m": "向(某人)挑战",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "prize",
      "p": "/praɪz/",
      "t": "n.",
      "m": "奖；奖励",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "promise",
      "p": "/'prɒmɪs/",
      "t": "v.",
      "m": "承诺；保证",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "chessboard",
      "p": "/'tʃesbɔːd/",
      "t": "n.",
      "m": "国际象棋棋盘",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "silver",
      "p": "/'sɪlvə(r)/",
      "t": "n.",
      "m": "银；回复",
      "e": [],
      "u": "2",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "reply",
      "p": "/rɪ'plaɪ/",
      "t": "v.",
      "m": "回答；答复",
      "e": [],
      "u": "2",
      "tags": [
        "suf_ly",
        "pre_re"
      ]
    },
    {
      "w": "hesitation",
      "p": "/ˌhezɪ'teɪʃn/",
      "t": "n.",
      "m": "犹豫",
      "e": [],
      "u": "2",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "wonder",
      "p": "/'wʌndə(r)/",
      "t": "v.",
      "m": "想知道；琢磨",
      "e": [],
      "u": "2",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "agree",
      "p": "/ə'ɡriː/",
      "t": "v.",
      "m": "同意；赞成",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "per cent",
      "p": "/pə 'sent/",
      "t": "n.",
      "m": "百分之...",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "currently",
      "p": "/'kʌrəntli/",
      "t": "adv.",
      "m": "当前；目前",
      "e": [],
      "u": "2",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "check",
      "p": "/tʃek/",
      "t": "v.",
      "m": "检查；查验",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "budget",
      "p": "/'bʌdʒɪt/",
      "t": "n.",
      "m": "预算",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "province",
      "p": "/'prɒvɪns/",
      "t": "n.",
      "m": "省；省份",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "sharply",
      "p": "/'ʃɑːpli/",
      "t": "adv.",
      "m": "急剧地；突然地",
      "e": [],
      "u": "2",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "count",
      "p": "/kaʊnt/",
      "t": "v.",
      "m": "计算 (或清点) 总数",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "system",
      "p": "/'sɪstəm/",
      "t": "n.",
      "m": "系统",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "symbol",
      "p": "/'sɪmbl/",
      "t": "n.",
      "m": "符号；记号",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "represent",
      "p": "/ˌreprɪ'zent/",
      "t": "v.",
      "m": "代表",
      "e": [],
      "u": "2",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "exactly",
      "p": "/ɪɡ'zæktli/",
      "t": "adv.",
      "m": "准确地；确切地",
      "e": [],
      "u": "2",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "flight schedule",
      "p": "",
      "t": "n. phr.",
      "m": "航班时刻表",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "price tag",
      "p": "",
      "t": "n. phr.",
      "m": "价格标签",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "for a moment",
      "p": "",
      "t": "phrase",
      "m": "片刻；一会儿",
      "e": [],
      "u": "2",
      "tags": [
        "suf_ment"
      ]
    },
    {
      "w": "without hesitation",
      "p": "",
      "t": "phrase",
      "m": "毫不犹豫",
      "e": [],
      "u": "2",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "go up",
      "p": "",
      "t": "v. phr.",
      "m": "上升",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "go down",
      "p": "",
      "t": "v. phr.",
      "m": "下降",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "write down",
      "p": "",
      "t": "v. phr.",
      "m": "写下；记下",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "instead of",
      "p": "",
      "t": "prep.",
      "m": "代替；作为......的替换",
      "e": [],
      "u": "2",
      "tags": []
    },
    {
      "w": "network",
      "p": "/'netwɜːk/",
      "t": "n.",
      "m": "网络",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "flood",
      "p": "/flʌd/",
      "t": "n.",
      "m": "洪水",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "multimedia",
      "p": "/ˌmʌlti'miːdiə/",
      "t": "n.",
      "m": "多媒体",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "expert",
      "p": "/'ekspɜːt/",
      "t": "n.",
      "m": "专家",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "mobile",
      "p": "/'məʊbaɪl/",
      "t": "adj.",
      "m": "可移动的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "payment",
      "p": "/'peɪmənt/",
      "t": "n.",
      "m": "付款；支付",
      "e": [],
      "u": "3",
      "tags": [
        "suf_ment"
      ]
    },
    {
      "w": "warm",
      "p": "/wɔːm/",
      "t": "v.",
      "m": "提醒；注意；警告",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "treatment",
      "p": "/'triːtmənt/",
      "t": "n.",
      "m": "疗法",
      "e": [],
      "u": "3",
      "tags": [
        "suf_ment"
      ]
    },
    {
      "w": "data",
      "p": "/'deɪtə/",
      "t": "n.",
      "m": "数据",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "company",
      "p": "/'kʌmpəni/",
      "t": "n.",
      "m": "公司",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "traffic",
      "p": "/'træfɪk/",
      "t": "n.",
      "m": "路上行驶的车辆；交通",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "flow",
      "p": "/fləʊ/",
      "t": "v.",
      "m": "流动；流",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "smoothly",
      "p": "/'smuːðli/",
      "t": "adv.",
      "m": "平稳地；连续而流畅地",
      "e": [],
      "u": "3",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "laptop",
      "p": "/'læptɒp/",
      "t": "n.",
      "m": "笔记本电脑",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "screen",
      "p": "/skriːn/",
      "t": "n.",
      "m": "屏幕",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "weight",
      "p": "/weɪt/",
      "t": "n.",
      "m": "重量",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "digital",
      "p": "/'dɪdʒɪtl/",
      "t": "adj.",
      "m": "数码的",
      "e": [],
      "u": "3",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "social",
      "p": "/'səʊʃl/",
      "t": "adj.",
      "m": "社交的",
      "e": [],
      "u": "3",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "message",
      "p": "/'mesɪdʒ/",
      "t": "n.",
      "m": "(书面或口头的) 信息",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "interview",
      "p": "/'ɪntəvjuː/",
      "t": "v.",
      "m": "采访",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "positive",
      "p": "/'pɒzətɪv/",
      "t": "adj.",
      "m": "积极的；正面的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "negative",
      "p": "/'neɡətɪv/",
      "t": "adj.",
      "m": "消极的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "effect",
      "p": "/ɪ'fekt/",
      "t": "n.",
      "m": "影响",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "opinion",
      "p": "/ə'pɪnjən/",
      "t": "n.",
      "m": "意见；看法",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "novel",
      "p": "/'nɒvl/",
      "t": "n.",
      "m": "(长篇) 小说",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "comment",
      "p": "/'kɒment/",
      "t": "n.",
      "m": "评论",
      "e": [],
      "u": "3",
      "tags": [
        "suf_ment"
      ]
    },
    {
      "w": "basis",
      "p": "/'beɪsɪs/",
      "t": "n.",
      "m": "基础",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "microprocessor",
      "p": "/ˌmaɪkrəʊ'prəʊsesə(r)/",
      "t": "n.",
      "m": "微处理器",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "microchip",
      "p": "/'maɪkrəʊtʃɪp/",
      "t": "n.",
      "m": "微芯片；芯片",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "major",
      "p": "/'meɪdʒə(r)/",
      "t": "adj.",
      "m": "主要的；重要的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "breakthrough",
      "p": "/'breɪkθruː/",
      "t": "n.",
      "m": "重大进展",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "electronic",
      "p": "/ɪˌlek'trɒnɪk/",
      "t": "adj.",
      "m": "电子的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "software",
      "p": "/'sɒftweə(r)/",
      "t": "n.",
      "m": "软件；应用程序",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "era",
      "p": "/'ɪərə/",
      "t": "n.",
      "m": "时代；纪元",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "download",
      "p": "/ˌdaʊn'ləʊd/",
      "t": "v.",
      "m": "下载",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "tiny",
      "p": "/'taɪni/",
      "t": "adj.",
      "m": "微小的",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "connect to",
      "p": "",
      "t": "v. phr.",
      "m": "连接",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "bring big changes to",
      "p": "",
      "t": "v. phr.",
      "m": "带来重大变化",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "mobile payment",
      "p": "",
      "t": "n. phr.",
      "m": "移动支付",
      "e": [],
      "u": "3",
      "tags": [
        "suf_ment"
      ]
    },
    {
      "w": "take ... for example",
      "p": "",
      "t": "phrase",
      "m": "以......为例",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "rubbish bin",
      "p": "",
      "t": "n. phr.",
      "m": "垃圾箱",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "social media",
      "p": "",
      "t": "n. phr.",
      "m": "社交媒体",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "in person",
      "p": "",
      "t": "adv. phr.",
      "m": "亲自；亲身",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "the general public",
      "p": "",
      "t": "n. phr.",
      "m": "公众；大众",
      "e": [],
      "u": "3",
      "tags": []
    },
    {
      "w": "balloon",
      "p": "/bə'luːn/",
      "t": "n.",
      "m": "气球",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "wheel",
      "p": "/wiːl/",
      "t": "n.",
      "m": "轮；轮车",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "central",
      "p": "/'sentrəl/",
      "t": "adj.",
      "m": "中心的",
      "e": [],
      "u": "4",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "although",
      "p": "/ɔːl'ðəʊ/",
      "t": "conj.",
      "m": "虽然；尽管；即使",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "attach",
      "p": "/ə'tætʃ/",
      "t": "v.",
      "m": "固定；附上 (在...上)",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "pull",
      "p": "/pʊl/",
      "t": "v.",
      "m": "拉；拖；扯",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "international",
      "p": "/ˌɪntə'næʃnəl/",
      "t": "adj.",
      "m": "国际的",
      "e": [],
      "u": "4",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "path",
      "p": "/pɑːθ/",
      "t": "n.",
      "m": "小路；小径",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "technique",
      "p": "/tek'niːk/",
      "t": "n.",
      "m": "技巧；艺技",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "depend",
      "p": "/dɪ'pend/",
      "t": "v.",
      "m": "需要；依靠",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "doubt",
      "p": "/daʊt/",
      "t": "n.",
      "m": "疑惑",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "personally",
      "p": "/'pɜːsənəli/",
      "t": "adv.",
      "m": "个人；就个人而言",
      "e": [],
      "u": "4",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "prediction",
      "p": "/prɪ'dɪkʃn/",
      "t": "n.",
      "m": "预测；预言",
      "e": [],
      "u": "4",
      "tags": [
        "suf_tion",
        "pre_pre"
      ]
    },
    {
      "w": "statement",
      "p": "/'steɪtmənt/",
      "t": "n.",
      "m": "说明",
      "e": [],
      "u": "4",
      "tags": [
        "suf_ment"
      ]
    },
    {
      "w": "benefit",
      "p": "/'benɪfɪt/",
      "t": "n.",
      "m": "益处；优势",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "wing",
      "p": "/wɪŋ/",
      "t": "n.",
      "m": "(飞航器的) 翅膀；机翼",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "distance",
      "p": "/'dɪstəns/",
      "t": "n.",
      "m": "距离",
      "e": [],
      "u": "4",
      "tags": [
        "pre_dis"
      ]
    },
    {
      "w": "petrol",
      "p": "/'petrəl/",
      "t": "n.",
      "m": "汽油",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "avoid",
      "p": "/ə'vɔɪd/",
      "t": "v.",
      "m": "避免；防止",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "take place",
      "p": "",
      "t": "v. phr.",
      "m": "发生；进行",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "for instance",
      "p": "",
      "t": "phrase",
      "m": "例如；比如",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "large amounts of",
      "p": "",
      "t": "phrase",
      "m": "大量的",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "international trade",
      "p": "",
      "t": "n. phr.",
      "m": "国际贸易",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "depend on",
      "p": "",
      "t": "v. phr.",
      "m": "依靠；依赖",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "without doubt",
      "p": "",
      "t": "phrase",
      "m": "毫无疑问",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "of all time",
      "p": "",
      "t": "phrase",
      "m": "自古以来；有史以来",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "make fun of",
      "p": "",
      "t": "v. phr.",
      "m": "取笑",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "traffic jam",
      "p": "",
      "t": "n. phr.",
      "m": "交通阻塞",
      "e": [],
      "u": "4",
      "tags": []
    },
    {
      "w": "exchange",
      "p": "/ɪks'tʃeɪndʒ/",
      "t": "v.",
      "m": "交流",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "nervous",
      "p": "/'nɜːvəs/",
      "t": "adj.",
      "m": "焦虑的；担心的",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "grateful",
      "p": "/'ɡreɪtfəl/",
      "t": "adj.",
      "m": "感激的",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "chopstick",
      "p": "/'tʃɒpstɪk/",
      "t": "n.",
      "m": "筷子",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "tour",
      "p": "/tʊə(r)/",
      "t": "n.",
      "m": "旅行；旅游",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "chi",
      "p": "/tʃi/",
      "t": "n.",
      "m": "太极",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "yet",
      "p": "/jet/",
      "t": "adv.",
      "m": "尚 (未)；还",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "independent",
      "p": "/ˌɪndɪ'pendənt/",
      "t": "adj.",
      "m": "自主的",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "content",
      "p": "/'kɒntent/",
      "t": "n.",
      "m": "内容",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "feeling",
      "p": "/'fiːlɪŋ/",
      "t": "n.",
      "m": "感觉；感情",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "foreign",
      "p": "/'fɒrən/",
      "t": "adj.",
      "m": "外国的",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "confused",
      "p": "/kən'fjuːzd/",
      "t": "adj.",
      "m": "糊涂的；迷惑的",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "anxious",
      "p": "/'æŋkʃəs/",
      "t": "adj.",
      "m": "焦虑的；担心的",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "phase",
      "p": "/feɪz/",
      "t": "n.",
      "m": "阶段；时期",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "honeymoon",
      "p": "/'hʌnimuːn/",
      "t": "n.",
      "m": "蜜月",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "unfamiliar",
      "p": "/ʌn'fæmɪliə(r)/",
      "t": "adj.",
      "m": "陌生的；不熟悉的",
      "e": [],
      "u": "5",
      "tags": [
        "pre_un"
      ]
    },
    {
      "w": "homesick",
      "p": "/'həʊmsɪk/",
      "t": "adj.",
      "m": "想家的",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "lonely",
      "p": "/'ləʊnli/",
      "t": "adj.",
      "m": "孤独的",
      "e": [],
      "u": "5",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "deal",
      "p": "/diːl/",
      "t": "v.",
      "m": "对；应付",
      "e": [],
      "u": "5",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "expect",
      "p": "/ɪk'spekt/",
      "t": "v.",
      "m": "预料；预期",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "situation",
      "p": "/sɪtʃu'eɪʃn/",
      "t": "n.",
      "m": "情况",
      "e": [],
      "u": "5",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "accept",
      "p": "/ək'sept/",
      "t": "v.",
      "m": "接受",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "adaptation",
      "p": "/ˌædæp'teɪʃn/",
      "t": "n.",
      "m": "适应",
      "e": [],
      "u": "5",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "Beijing opera",
      "p": "",
      "t": "n. phr.",
      "m": "京剧",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "host family",
      "p": "",
      "t": "n. phr.",
      "m": "寄宿家庭",
      "e": [],
      "u": "5",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "snake its way",
      "p": "",
      "t": "v. phr.",
      "m": "蜿蜒",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "culture shock",
      "p": "",
      "t": "n. phr.",
      "m": "文化冲击",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "deal with",
      "p": "",
      "t": "v. phr.",
      "m": "解决；处理",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "feel at home",
      "p": "",
      "t": "phrase",
      "m": "感到舒适自在",
      "e": [],
      "u": "5",
      "tags": []
    },
    {
      "w": "author",
      "p": "/'ɔːθə(r)/",
      "t": "n.",
      "m": "作者；作家",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "remains",
      "p": "/rɪ'meɪnz/",
      "t": "n.",
      "m": "遗迹；遗存",
      "e": [],
      "u": "6",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "locate",
      "p": "/ləʊ'keɪt/",
      "t": "v.",
      "m": "把......安置在 (或建造于)",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "soldier",
      "p": "/'səʊldʒə(r)/",
      "t": "n.",
      "m": "士兵",
      "e": [],
      "u": "6",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "captain",
      "p": "/'keptɪn/",
      "t": "n.",
      "m": "首领；领导者",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "empty",
      "p": "/'empti/",
      "t": "adj.",
      "m": "空的",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "victory",
      "p": "/'vɪktəri/",
      "t": "n.",
      "m": "胜利；成功",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "joke",
      "p": "/dʒəʊk/",
      "t": "n.",
      "m": "笑话；玩笑",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "midnight",
      "p": "/'mɪdnaɪt/",
      "t": "n.",
      "m": "午夜",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "except",
      "p": "/ɪk'sept/",
      "t": "prep.",
      "m": "除......之外",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "hide",
      "p": "/haɪd/",
      "t": "v.",
      "m": "隐藏",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "secretly",
      "p": "/'siːkrətli/",
      "t": "adv.",
      "m": "秘密地",
      "e": [],
      "u": "6",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "enter",
      "p": "/'entə(r)/",
      "t": "v.",
      "m": "来；进入",
      "e": [],
      "u": "6",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "succeed",
      "p": "/sək'siːd/",
      "t": "v.",
      "m": "达到目的；成功",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "trick",
      "p": "/trɪk/",
      "t": "n.",
      "m": "骗局",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "fight",
      "p": "/faɪt/",
      "t": "v.",
      "m": "打；战斗",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "beat",
      "p": "/biːt/",
      "t": "v.",
      "m": "打败 (某人)",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "pretend",
      "p": "/prɪ'tend/",
      "t": "v.",
      "m": "假装；伪装",
      "e": [],
      "u": "6",
      "tags": [
        "pre_pre"
      ]
    },
    {
      "w": "enemy",
      "p": "/'enəmi/",
      "t": "n.",
      "m": "敌人",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "fail",
      "p": "/feɪl/",
      "t": "v.",
      "m": "失败；未能 (做到)",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "therefore",
      "p": "/ðeə'fɔː(r)/",
      "t": "adv.",
      "m": "因此；所以",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "within",
      "p": "/wɪ'ðɪn/",
      "t": "prep.",
      "m": "在......之内",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "fill",
      "p": "/fɪl/",
      "t": "v.",
      "m": "(使) 充满；(使) 装满",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "towards",
      "p": "/tə'wɔːdz/",
      "t": "prep.",
      "m": "向；朝着",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "make jokes about",
      "p": "",
      "t": "v. phr.",
      "m": "开......的玩笑",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "succeed in",
      "p": "",
      "t": "v. phr.",
      "m": "在......方面成功",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "be tired of",
      "p": "",
      "t": "phrase",
      "m": "厌烦",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "go on board",
      "p": "",
      "t": "v. phr.",
      "m": "上船",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "be jealous of",
      "p": "",
      "t": "phrase",
      "m": "嫉妒",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "be full of",
      "p": "",
      "t": "phrase",
      "m": "充满",
      "e": [],
      "u": "6",
      "tags": []
    },
    {
      "w": "regularly",
      "p": "/'reɡjələli/",
      "t": "adv.",
      "m": "有规律地",
      "e": [],
      "u": "7",
      "tags": [
        "suf_ly",
        "pre_re"
      ]
    },
    {
      "w": "repeat",
      "p": "/rɪ'piːt/",
      "t": "v.",
      "m": "重复",
      "e": [],
      "u": "7",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "note",
      "p": "/nəʊt/",
      "t": "n.",
      "m": "笔记；记录",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "visual",
      "p": "/'vɪʒuəl/",
      "t": "adj.",
      "m": "视觉的",
      "e": [],
      "u": "7",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "mentally",
      "p": "/'mentəli/",
      "t": "adv.",
      "m": "精神上",
      "e": [],
      "u": "7",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "link",
      "p": "/lɪŋk/",
      "t": "v.",
      "m": "联系；相连",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "list",
      "p": "/lɪst/",
      "t": "n.",
      "m": "清单",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "especially",
      "p": "/ɪ'speʃəli/",
      "t": "adv.",
      "m": "尤其；特别",
      "e": [],
      "u": "7",
      "tags": [
        "suf_ly"
      ]
    },
    {
      "w": "diet",
      "p": "/'daɪət/",
      "t": "n.",
      "m": "日常饮食",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "maintain",
      "p": "/meɪn'teɪn/",
      "t": "v.",
      "m": "维持；保持",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "nut",
      "p": "/nʌt/",
      "t": "n.",
      "m": "坚果",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "relax",
      "p": "/rɪ'læks/",
      "t": "v.",
      "m": "放松；休息",
      "e": [],
      "u": "7",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "stressed",
      "p": "/strest/",
      "t": "adj.",
      "m": "焦虑的",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "tend",
      "p": "/tend/",
      "t": "v.",
      "m": "往往；趋向",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "normal",
      "p": "/'nɔːml/",
      "t": "adj.",
      "m": "正常的",
      "e": [],
      "u": "7",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "loss",
      "p": "/lɒs/",
      "t": "n.",
      "m": "损失；丢失",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "fork",
      "p": "/fɔːk/",
      "t": "n.",
      "m": "叉子",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "onion",
      "p": "/'ʌnjən/",
      "t": "n.",
      "m": "洋葱",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "ant",
      "p": "/ænt/",
      "t": "n.",
      "m": "蚂蚁",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "image",
      "p": "/'ɪmɪdʒ/",
      "t": "n.",
      "m": "图像",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "sense",
      "p": "/sens/",
      "t": "n.",
      "m": "感觉器官 (如视、听、嗅、味、触五感)",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "summary",
      "p": "/'sʌməri/",
      "t": "n.",
      "m": "总结；概括",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "contain",
      "p": "/kən'teɪn/",
      "t": "v.",
      "m": "包含；含有",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "particular",
      "p": "/pə'tɪkjələ(r)/",
      "t": "adj.",
      "m": "专指的；特有的",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "chemistry",
      "p": "/'kemɪstri/",
      "t": "n.",
      "m": "化学；化学组成 (或性质)",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "flash card",
      "p": "",
      "t": "n. phr.",
      "m": "闪卡",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "make a point of doing something",
      "p": "",
      "t": "phrase",
      "m": "务必做某事 (因重要或必要)",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "last but not least",
      "p": "",
      "t": "phrase",
      "m": "最后但同样重要的",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "natural disaster",
      "p": "",
      "t": "n. phr.",
      "m": "自然灾害",
      "e": [],
      "u": "7",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "figure out",
      "p": "",
      "t": "v. phr.",
      "m": "弄清楚；想出",
      "e": [],
      "u": "7",
      "tags": []
    },
    {
      "w": "attack",
      "p": "/ə'tæk/",
      "t": "v.",
      "m": "攻击；袭击",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "faithful",
      "p": "/'feɪθfl/",
      "t": "adj.",
      "m": "忠实的；忠诚的",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "hold",
      "p": "/həʊld/",
      "t": "v.",
      "m": "抱着；拿着",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "responsible",
      "p": "/rɪ'spɒnsəbl/",
      "t": "adj.",
      "m": "可信任的；负责的",
      "e": [],
      "u": "8",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "awake",
      "p": "/ə'weɪk/",
      "t": "adj.",
      "m": "醒着的",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "flat",
      "p": "/flæt/",
      "t": "n.",
      "m": "公寓",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "choice",
      "p": "/tʃɔɪs/",
      "t": "n.",
      "m": "选择",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "advise",
      "p": "/əd'vaɪz/",
      "t": "v.",
      "m": "劝告；建议",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "cause",
      "p": "/kɔːz/",
      "t": "v.",
      "m": "引起；导致",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "complain",
      "p": "/kəm'pleɪn/",
      "t": "v.",
      "m": "抱怨；投诉",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "litter",
      "p": "/'lɪtə(r)/",
      "t": "n.",
      "m": "垃圾",
      "e": [],
      "u": "8",
      "tags": [
        "suf_er"
      ]
    },
    {
      "w": "relieve",
      "p": "/rɪ'liːv/",
      "t": "v.",
      "m": "缓解；减轻",
      "e": [],
      "u": "8",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "unlikely",
      "p": "/ʌn'laɪkli/",
      "t": "adj.",
      "m": "不太可能的",
      "e": [],
      "u": "8",
      "tags": [
        "suf_ly",
        "pre_un"
      ]
    },
    {
      "w": "indoors",
      "p": "/ˌɪn'dɔːz/",
      "t": "adv.",
      "m": "在室内",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "magical",
      "p": "/'mædʒɪkl/",
      "t": "adj.",
      "m": "有魔力的",
      "e": [],
      "u": "8",
      "tags": [
        "suf_al"
      ]
    },
    {
      "w": "wealth",
      "p": "/welθ/",
      "t": "n.",
      "m": "财富",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "queen",
      "p": "/kwiːn/",
      "t": "n.",
      "m": "女王；王后",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "servant",
      "p": "/'sɜːvənt/",
      "t": "n.",
      "m": "仆人",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "pride",
      "p": "/praɪd/",
      "t": "n.",
      "m": "自豪；骄傲",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "relationship",
      "p": "/rɪ'leɪʃnʃɪp/",
      "t": "n.",
      "m": "关系",
      "e": [],
      "u": "8",
      "tags": [
        "pre_re"
      ]
    },
    {
      "w": "grow up",
      "p": "",
      "t": "v. phr.",
      "m": "长大；成熟",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "care for",
      "p": "",
      "t": "v. phr.",
      "m": "照料；喜爱",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "in short",
      "p": "",
      "t": "phrase",
      "m": "总之；简而言之",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "have no choice but to",
      "p": "",
      "t": "phrase",
      "m": "别无选择；只能",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "run free",
      "p": "",
      "t": "v. phr.",
      "m": "自由奔跑/生活",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "lie around",
      "p": "",
      "t": "v. phr.",
      "m": "闲逛；躺在周围",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "complain about",
      "p": "",
      "t": "v. phr.",
      "m": "抱怨",
      "e": [],
      "u": "8",
      "tags": []
    },
    {
      "w": "in addition",
      "p": "",
      "t": "phrase",
      "m": "此外；另外",
      "e": [],
      "u": "8",
      "tags": [
        "suf_tion"
      ]
    },
    {
      "w": "catch the eye of somebody",
      "p": "",
      "t": "v. phr.",
      "m": "引起某人的注意",
      "e": [],
      "u": "8",
      "tags": []
    }
  ],
  "9A": {
    "1": [
      {
        "w": "outstanding",
        "p": "/aʊt'stændɪŋ/",
        "t": "adj.",
        "m": "优秀的; 杰出的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "old-fashioned",
        "p": "/əʊld-'fæʃnd/",
        "t": "adj.",
        "m": "陈旧的; 过时的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "classical",
        "p": "/'klæsɪkl/",
        "t": "adj.",
        "m": "古典的; 经典的",
        "e": [],
        "u": "1",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "ordinary",
        "p": "/'ɔːdnri/",
        "t": "adj.",
        "m": "普通的; 平凡的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "lamp",
        "p": "/læmp/",
        "t": "n.",
        "m": "灯",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "poet",
        "p": "/'pəʊɪt/",
        "t": "n.",
        "m": "诗人",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "convinced",
        "p": "/kən'vɪnst/",
        "t": "adj.",
        "m": "坚信的; 确信的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "philosopher",
        "p": "/fə'lɒsəfə(r)/",
        "t": "n.",
        "m": "哲学家",
        "e": [],
        "u": "1",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "dramatist",
        "p": "/'dræmətɪst/",
        "t": "n.",
        "m": "作家",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "judgement",
        "p": "/'dʒʌdʒmənt/",
        "t": "n.",
        "m": "法则; 评价",
        "e": [],
        "u": "1",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "evaluation",
        "p": "/ɪ,vælju'eɪʃn/",
        "t": "n.",
        "m": "评估",
        "e": [],
        "u": "1",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "consider",
        "p": "/kən'sɪdə(r)/",
        "t": "v.",
        "m": "认为; 觉得",
        "e": [],
        "u": "1",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "background",
        "p": "/'bækɡraʊnd/",
        "t": "n.",
        "m": "背景",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "significant",
        "p": "/sɪɡ'nɪfɪkənt/",
        "t": "adj.",
        "m": "重大的; 显著的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "milestone",
        "p": "/'maɪlstəʊn/",
        "t": "n.",
        "m": "重要事件; 里程碑",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "manage",
        "p": "/'mænɪdʒ/",
        "t": "v.",
        "m": "完成 (困难的事)",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "impact",
        "p": "/'ɪmpækt/",
        "t": "n.",
        "m": "巨大影响; 强大作用",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "truth",
        "p": "/truːθ/",
        "t": "n.",
        "m": "真理",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "concentrated",
        "p": "/'kɒnsntreɪtɪd/",
        "t": "adj.",
        "m": "全神贯注的; 全力以赴的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "belief",
        "p": "/bɪ'liːf/",
        "t": "n.",
        "m": "相信",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "with something in mind",
        "p": "",
        "t": "phrase",
        "m": "考虑到......",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "in addition",
        "p": "",
        "t": "phrase",
        "m": "除......之外",
        "e": [],
        "u": "1",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "be fond of",
        "p": "",
        "t": "phrase",
        "m": "喜爱",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "reach for",
        "p": "",
        "t": "phrase",
        "m": "伸手去拿",
        "e": [],
        "u": "1",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "fight for",
        "p": "",
        "t": "phrase",
        "m": "为......作斗争",
        "e": [],
        "u": "1",
        "tags": []
      }
    ],
    "2": [
      {
        "w": "firm",
        "p": "/fɜːm/",
        "t": "adj.",
        "m": "牢固的；坚硬的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "ignorance",
        "p": "/'ɪɡnərəns/",
        "t": "n.",
        "m": "无知",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "application",
        "p": "/ˌæplɪ'keɪʃn/",
        "t": "n.",
        "m": "(尤指理论、发现等的)应用",
        "e": [],
        "u": "2",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "fancy",
        "p": "/'fænsi/",
        "t": "adj.",
        "m": "精致的；绚丽的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "satisfied",
        "p": "/'sætɪsfaɪd/",
        "t": "adj.",
        "m": "满意的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "prince",
        "p": "/prɪns/",
        "t": "n.",
        "m": "王子",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "displace",
        "p": "/dɪ'spleɪs/",
        "t": "v.",
        "m": "排挤；取代",
        "e": [],
        "u": "2",
        "tags": [
          "pre_dis"
        ]
      },
      {
        "w": "fool",
        "p": "/fuːl/",
        "t": "v.",
        "m": "欺骗；愚弄",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "prison",
        "p": "/'prɪzn/",
        "t": "n.",
        "m": "监狱",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "wire",
        "p": "/'waɪə(r)/",
        "t": "n.",
        "m": "金属丝；电线",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "prove",
        "p": "/pruːv/",
        "t": "v.",
        "m": "证明；证实",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "hardly",
        "p": "/'hɑːdli/",
        "t": "adv.",
        "m": "几乎没有",
        "e": [],
        "u": "2",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "balanced",
        "p": "/'bælənst/",
        "t": "adj.",
        "m": "保持(或显示)平衡的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "reject",
        "p": "/rɪ'dʒekt/",
        "t": "v.",
        "m": "拒绝；不接受",
        "e": [],
        "u": "2",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "thus",
        "p": "/ðʌs/",
        "t": "adv.",
        "m": "因此；如此",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "be satisfied with",
        "p": "",
        "t": "v. phr.",
        "m": "对......感到满意",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "fill ... with",
        "p": "",
        "t": "v. phr.",
        "m": "充满；装满",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "run over",
        "p": "",
        "t": "v. phr.",
        "m": "溢出",
        "e": [],
        "u": "2",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "go straight to",
        "p": "",
        "t": "v. phr.",
        "m": "直奔",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "lightning rod",
        "p": "",
        "t": "n. phr.",
        "m": "避雷针",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "as a whole",
        "p": "",
        "t": "phrase",
        "m": "作为一个整体；总体上",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "get something across",
        "p": "",
        "t": "v. phr.",
        "m": "把某事讲清楚",
        "e": [],
        "u": "2",
        "tags": []
      }
    ],
    "3": [
      {
        "w": "indeed",
        "p": "/ɪn'diːd/",
        "t": "adv.",
        "m": "确实 (强调肯定的陈述或答复)",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "match",
        "p": "/mætʃ/",
        "t": "n.",
        "m": "火柴",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "shame",
        "p": "/ʃeɪm/",
        "t": "n.",
        "m": "令人惋惜的事；令人遗憾的事",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "remove",
        "p": "/rɪ'muːv/",
        "t": "v.",
        "m": "去掉",
        "e": [],
        "u": "3",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "lower",
        "p": "/'ləʊə(r)/",
        "t": "v.",
        "m": "把......降低",
        "e": [],
        "u": "3",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "practice",
        "p": "/'præktɪs/",
        "t": "n.",
        "m": "实践；实际行动",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "decision",
        "p": "/dɪ'sɪʒn/",
        "t": "n.",
        "m": "决定",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "cheat",
        "p": "/tʃiːt/",
        "t": "v.",
        "m": "欺骗；作弊",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "strengthen",
        "p": "/'streŋθn/",
        "t": "v.",
        "m": "加强；增强；巩固",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "*fold",
        "p": "/fəʊld/",
        "t": "v.",
        "m": "折叠",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "repetition",
        "p": "/ˌrepəˈtɪʃn/",
        "t": "n.",
        "m": "重复",
        "e": [],
        "u": "3",
        "tags": [
          "suf_tion",
          "pre_re"
        ]
      },
      {
        "w": "pardon",
        "p": "/'pɑːdn/",
        "t": "exclamation",
        "m": "请再说一遍",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "vacation",
        "p": "/və'keɪʃn/",
        "t": "n.",
        "m": "假期",
        "e": [],
        "u": "3",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "somehow",
        "p": "/'sʌmhaʊ/",
        "t": "adv.",
        "m": "不知为什么",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "awkward",
        "p": "/'ɔːkwəd/",
        "t": "adj.",
        "m": "令人尴尬的",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "laughter",
        "p": "/'lɑːftə(r)/",
        "t": "n.",
        "m": "笑声",
        "e": [],
        "u": "3",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "tear",
        "p": "/tɪə(r)/",
        "t": "n.",
        "m": "眼泪",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "with admiration",
        "p": "",
        "t": "phrase",
        "m": "钦佩地；赞赏地",
        "e": [],
        "u": "3",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "right and wrong",
        "p": "",
        "t": "phrase",
        "m": "是非",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "fresh in one's memory",
        "p": "",
        "t": "phrase",
        "m": "记忆犹新",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "bed sheet",
        "p": "",
        "t": "n. phr.",
        "m": "床单",
        "e": [],
        "u": "3",
        "tags": []
      }
    ],
    "4": [
      {
        "w": "manage",
        "p": "/'mænɪdʒ/",
        "t": "v.",
        "m": "明智地使用（金钱、时间、信息等）",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "pressure",
        "p": "/'preʃə(r)/",
        "t": "n.",
        "m": "心理压力；紧张",
        "e": [],
        "u": "4",
        "tags": [
          "pre_pre"
        ]
      },
      {
        "w": "addicted",
        "p": "/ə'dɪktɪd/",
        "t": "adj.",
        "m": "上瘾的",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "bullying",
        "p": "/'bʊliɪŋ/",
        "t": "n.",
        "m": "霸凌；势必作恶",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "charge",
        "p": "/tʃɑːdʒ/",
        "t": "n.",
        "m": "职责；责任",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "awful",
        "p": "/'ɔ:fl/",
        "t": "adj.",
        "m": "很坏的；极讨厌的",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "focus",
        "p": "/'fəʊkəs/",
        "t": "v.",
        "m": "集中（注意力）",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "affect",
        "p": "/ə'fekt/",
        "t": "v.",
        "m": "影响",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "hate",
        "p": "/hett/",
        "t": "v.",
        "m": "厌恶；讨厌",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "embarrassed",
        "p": "/im'bærəst/",
        "t": "adj.",
        "m": "尴尬的；羞涩的",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "mad",
        "p": "/mæd/",
        "t": "adj.",
        "m": "生气",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "mess",
        "p": "/mes/",
        "t": "v.",
        "m": "弄脏；弄乱",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "radio",
        "p": "/'reidiəʊ/",
        "t": "n.",
        "m": "广播",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "ashamed",
        "p": "/ə'ʃeɪmd/",
        "t": "adj.",
        "m": "惭愧；羞愧",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "respond",
        "p": "/rɪ'spɒnd/",
        "t": "v.",
        "m": "(口头或书面) 回答",
        "e": [],
        "u": "4",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "sympathy",
        "p": "/'sɪmpəθi/",
        "t": "n.",
        "m": "同情",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "section",
        "p": "/sek'ʃn/",
        "t": "n.",
        "m": "部分",
        "e": [],
        "u": "4",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "newspaper",
        "p": "/nju:'zeipə(r)/",
        "t": "n.",
        "m": "报纸",
        "e": [],
        "u": "4",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "operate",
        "p": "/ə'pəreɪt/",
        "t": "v.",
        "m": "运营；管理",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "helpline",
        "p": "/'helpmæn/",
        "t": "n.",
        "m": "服务热线（提供咨询和信息的电话）",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "click",
        "p": "/klɪk/",
        "t": "n.",
        "m": "(对鼠标的) 点击",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "get addicted to",
        "p": "",
        "t": "v. phr.",
        "m": "对……上瘾",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "be in charge of",
        "p": "",
        "t": "v. phr.",
        "m": "负责",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "focus on",
        "p": "",
        "t": "v. phr.",
        "m": "集中",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "drive someone mad",
        "p": "",
        "t": "v. phr.",
        "m": "使某人发狂",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "mess something up",
        "p": "",
        "t": "v. phr.",
        "m": "弄脏；弄乱",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "hit it off with somebody",
        "p": "",
        "t": "idiom",
        "m": "和某人投缘",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "fall out with somebody",
        "p": "",
        "t": "v. phr.",
        "m": "与某人闹翻",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "feel ashamed of",
        "p": "",
        "t": "v. phr.",
        "m": "感到羞愧",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "advice column",
        "p": "",
        "t": "n. phr.",
        "m": "(报纸或杂志的) 问答专栏",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "agony aunt",
        "p": "",
        "t": "n. phr.",
        "m": "(报纸或杂志的) 问答作者",
        "e": [],
        "u": "4",
        "tags": []
      }
    ],
    "5": [
      {
        "w": "various",
        "p": "/'veəriəs/",
        "t": "adj.",
        "m": "各种不同的; 各种各样的",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "lighting operator",
        "p": "",
        "t": "n. phr.",
        "m": "灯光师",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "make-up artist",
        "p": "",
        "t": "n. phr.",
        "m": "化妆师",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "sound operator",
        "p": "",
        "t": "n. phr.",
        "m": "音响师",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "quiz show",
        "p": "",
        "t": "n. phr.",
        "m": "智力竞赛节目",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "reality show",
        "p": "",
        "t": "n. phr.",
        "m": "真人秀节目",
        "e": [],
        "u": "5",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "talk show",
        "p": "",
        "t": "n. phr.",
        "m": "脱口秀",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "travel show",
        "p": "",
        "t": "n. phr.",
        "m": "旅游节目",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "TV drama",
        "p": "",
        "t": "n. phr.",
        "m": "电视连续剧",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "shine upon",
        "p": "",
        "t": "v. phr.",
        "m": "照在身上",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "pass out",
        "p": "",
        "t": "v. phr.",
        "m": "昏迷",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "time zone",
        "p": "",
        "t": "n. phr.",
        "m": "时代/时区",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "hang out",
        "p": "",
        "t": "v. phr.",
        "m": "在某处闲逛",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "film review",
        "p": "",
        "t": "n. phr.",
        "m": "影评",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "board game",
        "p": "",
        "t": "n. phr.",
        "m": "棋类游戏",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "constant",
        "p": "/'kɒnstənt/",
        "t": "n.",
        "m": "比赛者; 竞争者",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "upon",
        "p": "/ə'pɒn/",
        "t": "prep.",
        "m": "(同 on)... 上",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "zone",
        "p": "/'zəʊn/",
        "t": "n.",
        "m": "区域",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "outdoors",
        "p": "/'aʊtdɔːz/",
        "t": "adv.",
        "m": "在户外; 在野外",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "conduct",
        "p": "/kən'dækt/",
        "t": "v.",
        "m": "实施",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "concert",
        "p": "/'kɒnsət/",
        "t": "n.",
        "m": "音乐会",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "gain",
        "p": "/ɡeɪn/",
        "t": "v.",
        "m": "获得",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "recommendation",
        "p": "/ˌrekəmen'deɪʃn/",
        "t": "n.",
        "m": "推荐",
        "e": [],
        "u": "5",
        "tags": [
          "suf_tion",
          "pre_re"
        ]
      },
      {
        "w": "plot",
        "p": "/plɒt/",
        "t": "n.",
        "m": "故事情节",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "calm",
        "p": "/kɑːm/",
        "t": "v.",
        "m": "使平静",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "snowboarding",
        "p": "/'snəʊbɔːrdɪŋ/",
        "t": "n.",
        "m": "滑雪板运动",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "challenging",
        "p": "/'tʃælɪndʒɪn/",
        "t": "adj.",
        "m": "挑战性的; 考验能力的",
        "e": [],
        "u": "5",
        "tags": []
      }
    ],
    "6": [
      {
        "w": "boil",
        "p": "/bɔɪl/",
        "t": "v.",
        "m": "沸水煮 (或洗)",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "biscuit",
        "p": "/ˈbɪskɪt/",
        "t": "n.",
        "m": "饼干",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "score",
        "p": "/skɔː(r)/",
        "t": "n.",
        "m": "分数; 得分",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "tofu",
        "p": "/ˈtɔːfu/",
        "t": "n.",
        "m": "豆腐",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "stuff",
        "p": "/stʌf/",
        "t": "n.",
        "m": "东西",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "overweight",
        "p": "/ˌəʊvəˈweɪt/",
        "t": "adj.",
        "m": "超重的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "sugary",
        "p": "/ˈʃʊɡəri/",
        "t": "adj.",
        "m": "含糖的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "menu",
        "p": "/ˈmenjuː/",
        "t": "n.",
        "m": "菜单",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "cucumber",
        "p": "/kjuːˈkʌmbə(r)/",
        "t": "n.",
        "m": "黄瓜",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "carrot",
        "p": "/ˈkærət/",
        "t": "n.",
        "m": "胡萝卜",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "cabbage",
        "p": "/ˈkæbɪdʒ/",
        "t": "n.",
        "m": "甘蓝; 卷心菜",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "preference",
        "p": "/ˈprefrəns/",
        "t": "n.",
        "m": "偏爱; 爱好",
        "e": [],
        "u": "6",
        "tags": [
          "pre_pre"
        ]
      },
      {
        "w": "waiter",
        "p": "/ˈweɪtə(r)/",
        "t": "n.",
        "m": "服务员",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "sweat",
        "p": "/swet/",
        "t": "n.",
        "m": "汗水",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "soil",
        "p": "/ˈsɔɪl/",
        "t": "n.",
        "m": "土壤",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "welcoming",
        "p": "/ˈwelkəmɪŋ/",
        "t": "adj.",
        "m": "令人感到惬意的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "gentle",
        "p": "/ˈdʒentl/",
        "t": "adj.",
        "m": "温和的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "whisper",
        "p": "/ˈwɪspə(r)/",
        "t": "v.",
        "m": "低语; 私语",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "tend",
        "p": "/tend/",
        "t": "v.",
        "m": "照料; 照管",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "fizzy drink",
        "p": "",
        "t": "n. phr.",
        "m": "起泡饮料",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "fried food",
        "p": "",
        "t": "n. phr.",
        "m": "油炸食品",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "a balanced diet",
        "p": "",
        "t": "n. phr.",
        "m": "均衡的饮食",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "in general",
        "p": "",
        "t": "adv. phr.",
        "m": "通常; 大体上",
        "e": [],
        "u": "6",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "dairy product",
        "p": "",
        "t": "n. phr.",
        "m": "乳制品",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "get through",
        "p": "",
        "t": "v. phr.",
        "m": "度过; 通过",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "would rather",
        "p": "",
        "t": "v. phr.",
        "m": "宁愿; 更喜欢",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "remind somebody of...",
        "p": "",
        "t": "v. phr.",
        "m": "使某人想起 (类似的人、地方、事物等)",
        "e": [],
        "u": "6",
        "tags": [
          "pre_re"
        ]
      }
    ],
    "7": [
      {
        "w": "associate",
        "p": "/ə'səʊsieɪt/",
        "t": "v.",
        "m": "联系",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "humour",
        "p": "/'hju:mə(r)/",
        "t": "n.",
        "m": "幽默；幽默感",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "fence",
        "p": "/fens/",
        "t": "n.",
        "m": "栅栏；篱笆",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "sink",
        "p": "/sɪŋk/",
        "t": "v.",
        "m": "下沉；下降",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "yard",
        "p": "/jɑ:d/",
        "t": "n.",
        "m": "码（长度单位）",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "survey",
        "p": "/'sɜ:veɪ/",
        "t": "v.",
        "m": "查看；视察；调查",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "aloud",
        "p": "/ə'laʊd/",
        "t": "adv.",
        "m": "大声地",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "pity",
        "p": "/'pɪti/",
        "t": "n.",
        "m": "遗憾；可惜",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "gentleman",
        "p": "/'dʒentlmən/",
        "t": "n.",
        "m": "彬彬有礼的人；有教养的人",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "concern",
        "p": "/kən'sɜːn/",
        "t": "n.",
        "m": "担心；忧虑",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "sort",
        "p": "/sɔ:t/",
        "t": "n.",
        "m": "种类；类别",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "spring",
        "p": "/sprɪŋ/",
        "t": "n.",
        "m": "活力；朝气",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "*bet",
        "p": "/bet/",
        "t": "n.",
        "m": "打赌",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "afterwards",
        "p": "/'ɑːftəwədz/",
        "t": "adv.",
        "m": "后来",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "brief",
        "p": "/brɪf/",
        "t": "adj.",
        "m": "简短的；简单的",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "journalist",
        "p": "/'dʒɜ:nəlɪst/",
        "t": "n.",
        "m": "新闻记者",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "pilot",
        "p": "/'paɪlət/",
        "t": "n.",
        "m": "领航员；引水员",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "failure",
        "p": "/'feɪljə(r)/",
        "t": "n.",
        "m": "失败",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "regard",
        "p": "/rɪ'ɡɑ:d/",
        "t": "v.",
        "m": "把......视为；看待",
        "e": [],
        "u": "7",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "first-person point of view",
        "p": "",
        "t": "phrase",
        "m": "第一人称视角",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "pick up",
        "p": "",
        "t": "v. phr.",
        "m": "捡起；重新开始",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "carry on with something",
        "p": "",
        "t": "v. phr.",
        "m": "继续做某事；坚持做某事",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "go on",
        "p": "",
        "t": "v. phr.",
        "m": "继续下去",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "be particular about",
        "p": "",
        "t": "v. phr.",
        "m": "讲究；挑剔",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "get tired with a spring in one's step",
        "p": "",
        "t": "idiom",
        "m": "迈着轻快的步伐 (注：此处合并了原文表达)",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "public speaker",
        "p": "",
        "t": "n. phr.",
        "m": "发言人；演讲者",
        "e": [],
        "u": "7",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "make a fortune",
        "p": "",
        "t": "v. phr.",
        "m": "发财；赚大钱",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "pay ... back",
        "p": "",
        "t": "v. phr.",
        "m": "(向某人)还钱",
        "e": [],
        "u": "7",
        "tags": []
      }
    ],
    "8": [
      {
        "w": "attack",
        "p": "/ə'tæk/",
        "t": "v.",
        "m": "攻击；袭击",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "faithful",
        "p": "/'feɪθfl/",
        "t": "adj.",
        "m": "忠实的；忠诚的",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "hold",
        "p": "/həʊld/",
        "t": "v.",
        "m": "抱着；拿着",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "responsible",
        "p": "/rɪ'spɒnsəbl/",
        "t": "adj.",
        "m": "可信任的；负责的",
        "e": [],
        "u": "8",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "awake",
        "p": "/ə'weɪk/",
        "t": "adj.",
        "m": "醒着的",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "flat",
        "p": "/flæt/",
        "t": "n.",
        "m": "公寓",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "choice",
        "p": "/tʃɔɪs/",
        "t": "n.",
        "m": "选择",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "advise",
        "p": "/əd'vaɪz/",
        "t": "v.",
        "m": "劝告；建议",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "cause",
        "p": "/kɔːz/",
        "t": "v.",
        "m": "引起；导致",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "complain",
        "p": "/kəm'pleɪn/",
        "t": "v.",
        "m": "抱怨；投诉",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "litter",
        "p": "/'lɪtə(r)/",
        "t": "n.",
        "m": "垃圾",
        "e": [],
        "u": "8",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "relieve",
        "p": "/rɪ'liːv/",
        "t": "v.",
        "m": "缓解；减轻",
        "e": [],
        "u": "8",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "unlikely",
        "p": "/ʌn'laɪkli/",
        "t": "adj.",
        "m": "不太可能的",
        "e": [],
        "u": "8",
        "tags": [
          "suf_ly",
          "pre_un"
        ]
      },
      {
        "w": "indoors",
        "p": "/ˌɪn'dɔːz/",
        "t": "adv.",
        "m": "在室内",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "magical",
        "p": "/'mædʒɪkl/",
        "t": "adj.",
        "m": "有魔力的",
        "e": [],
        "u": "8",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "wealth",
        "p": "/welθ/",
        "t": "n.",
        "m": "财富",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "queen",
        "p": "/kwiːn/",
        "t": "n.",
        "m": "女王；王后",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "servant",
        "p": "/'sɜːvənt/",
        "t": "n.",
        "m": "仆人",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "pride",
        "p": "/praɪd/",
        "t": "n.",
        "m": "自豪；骄傲",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "relationship",
        "p": "/rɪ'leɪʃnʃɪp/",
        "t": "n.",
        "m": "关系",
        "e": [],
        "u": "8",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "grow up",
        "p": "",
        "t": "v. phr.",
        "m": "长大；成熟",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "care for",
        "p": "",
        "t": "v. phr.",
        "m": "照料；喜爱",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "in short",
        "p": "",
        "t": "phrase",
        "m": "总之；简而言之",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "have no choice but to",
        "p": "",
        "t": "phrase",
        "m": "别无选择；只能",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "run free",
        "p": "",
        "t": "v. phr.",
        "m": "自由奔跑/生活",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "lie around",
        "p": "",
        "t": "v. phr.",
        "m": "闲逛；躺在周围",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "complain about",
        "p": "",
        "t": "v. phr.",
        "m": "抱怨",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "in addition",
        "p": "",
        "t": "phrase",
        "m": "此外；另外",
        "e": [],
        "u": "8",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "catch the eye of somebody",
        "p": "",
        "t": "v. phr.",
        "m": "引起某人的注意",
        "e": [],
        "u": "8",
        "tags": []
      }
    ]
  },
  "9B": {
    "1": [
      {
        "w": "voyage",
        "p": "/ˈvɔɪədʒ/",
        "t": "n.",
        "m": "航行",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "repetition",
        "p": "/ˌrepəˈtɪʃn/",
        "t": "n.",
        "m": "重复",
        "e": [],
        "u": "1",
        "tags": [
          "suf_tion",
          "pre_re"
        ]
      },
      {
        "w": "American",
        "p": "/əˈmerɪkən/",
        "t": "adj.",
        "m": "美国的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "continent",
        "p": "/ˈkɒntɪnənt/",
        "t": "n.",
        "m": "大陆",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "route",
        "p": "/ruːt/",
        "t": "n.",
        "m": "路线",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "discovery",
        "p": "/dɪˈskʌvəri/",
        "t": "n.",
        "m": "发现",
        "e": [],
        "u": "1",
        "tags": [
          "pre_dis"
        ]
      },
      {
        "w": "rise",
        "p": "/raɪz/",
        "t": "v.",
        "m": "变得成功 (或重要大帝)",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "official",
        "p": "/əˈfɪʃl/",
        "t": "n.",
        "m": "官员",
        "e": [],
        "u": "1",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "develop",
        "p": "/dɪˈveləp/",
        "t": "v.",
        "m": "增强；加强",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "relation",
        "p": "/rɪˈleɪʃn/",
        "t": "n.",
        "m": "关系",
        "e": [],
        "u": "1",
        "tags": [
          "suf_tion",
          "pre_re"
        ]
      },
      {
        "w": "trade",
        "p": "/treɪd/",
        "t": "v.",
        "m": "交易；以物易物",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "foreign",
        "p": "/ˈfɒrən/",
        "t": "adj.",
        "m": "外国的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "fleet",
        "p": "/fliːt/",
        "t": "n.",
        "m": "船队；机群",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "Africa",
        "p": "/ˈæfrɪkə/",
        "t": "n.",
        "m": "非洲",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "nowhere",
        "p": "/ˈnəʊweə(r)/",
        "t": "adv.",
        "m": "无处；哪里都不",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "silk",
        "p": "/sɪlk/",
        "t": "n.",
        "m": "织物；丝绸",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "giraffe",
        "p": "/dʒəˈrɑːf/",
        "t": "n.",
        "m": "长颈鹿",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "besides",
        "p": "/bɪˈsaɪdz/",
        "t": "prep.",
        "m": "除...之外 (也)",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "development",
        "p": "/dɪˈveləpmənt/",
        "t": "n.",
        "m": "发展",
        "e": [],
        "u": "1",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "region",
        "p": "/ˈriːdʒən/",
        "t": "n.",
        "m": "地区",
        "e": [],
        "u": "1",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "pioneer",
        "p": "/ˌpaɪəˈnɪə(r)/",
        "t": "n.",
        "m": "先锋；先驱",
        "e": [],
        "u": "1",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "people",
        "p": "/ˈpiːpl/",
        "t": "n.",
        "m": "民族；种族",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "wealth",
        "p": "/welθ/",
        "t": "n.",
        "m": "财富",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "spread",
        "p": "/spred/",
        "t": "v.",
        "m": "传播",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "open up",
        "p": "",
        "t": "v. phr.",
        "m": "开辟",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "go on a trip",
        "p": "",
        "t": "v. phr.",
        "m": "去旅行",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "set up",
        "p": "",
        "t": "v. phr.",
        "m": "建立；设立",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "set sail",
        "p": "",
        "t": "v. phr.",
        "m": "起航",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "(be) known as",
        "p": "",
        "t": "v. phr.",
        "m": "被称为",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "as well as",
        "p": "",
        "t": "conj.",
        "m": "也；还",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "lead to",
        "p": "",
        "t": "v. phr.",
        "m": "导致",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "compare ... with ...",
        "p": "",
        "t": "v. phr.",
        "m": "把...与...对比",
        "e": [],
        "u": "1",
        "tags": []
      }
    ],
    "2": [
      {
        "w": "culture shock",
        "p": "/ˈkʌltʃə ʃɒk/",
        "t": "n. phr.",
        "m": "文化冲击",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "international",
        "p": "/ˌɪntəˈnæʃnəl/",
        "t": "adj.",
        "m": "国际的",
        "e": [],
        "u": "2",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "national",
        "p": "/ˈnæʃnəl/",
        "t": "adj.",
        "m": "国家的",
        "e": [],
        "u": "2",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "schedule",
        "p": "/ˈʃedjuːl/",
        "t": "n.",
        "m": "时间表",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "set off",
        "p": "/set ɒf/",
        "t": "v. phr.",
        "m": "出发",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "take off",
        "p": "/teɪk ɒf/",
        "t": "v. phr.",
        "m": "起飞；脱掉",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "get used to",
        "p": "/ɡet juːst tu/",
        "t": "v. phr.",
        "m": "习惯于",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "under the weather",
        "p": "/ˈʌndə ðə ˈweðə/",
        "t": "idiom",
        "m": "身体不适",
        "e": [],
        "u": "2",
        "tags": [
          "suf_er",
          "pre_un"
        ]
      }
    ],
    "3": [
      {
        "w": "concern",
        "p": "/kənˈsɜːn/",
        "t": "n./v.",
        "m": "关心；担忧",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "atmosphere",
        "p": "/ˈætməsfɪə(r)/",
        "t": "n.",
        "m": "大气；气氛",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "greenhouse effect",
        "p": "/ˈɡriːnhaʊs ɪˈfekt/",
        "t": "n. phr.",
        "m": "温室效应",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "make a difference",
        "p": "/meɪk ə ˈdɪfrəns/",
        "t": "v. phr.",
        "m": "产生影响",
        "e": [],
        "u": "3",
        "tags": []
      }
    ],
    "4": [
      {
        "w": "asteroid",
        "p": "/ˈæstərɔɪd/",
        "t": "n.",
        "m": "小行星",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "typhoon",
        "p": "/taɪˈfuːn/",
        "t": "n.",
        "m": "台风",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "natural disaster",
        "p": "/ˈnætʃrəl dɪˈzɑːstə/",
        "t": "n. phr.",
        "m": "自然灾害",
        "e": [],
        "u": "4",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "survival kit",
        "p": "/səˈvaɪvl kɪt/",
        "t": "n. phr.",
        "m": "生存工具包",
        "e": [],
        "u": "4",
        "tags": []
      }
    ],
    "5": [
      {
        "w": "announcement",
        "p": "/əˈnaʊnsmənt/",
        "t": "n.",
        "m": "公告；通知",
        "e": [],
        "u": "5",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "passport",
        "p": "/ˈpɑːspɔːt/",
        "t": "n.",
        "m": "护照",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "be dying to",
        "p": "/bi ˈdaɪɪŋ tu/",
        "t": "v. phr.",
        "m": "渴望做...",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "keep one's balance",
        "p": "/kiːp wʌnz ˈbæləns/",
        "t": "v. phr.",
        "m": "保持平衡",
        "e": [],
        "u": "5",
        "tags": []
      }
    ],
    "6": [
      {
        "w": "regular",
        "p": "/'reɡjələ(r)/",
        "t": "adj.",
        "m": "有规律的；定时的",
        "e": [],
        "u": "6",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "cheer",
        "p": "/tʃɪə(r)/",
        "t": "v.",
        "m": "鼓励；鼓舞",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "low",
        "p": "/ləʊ/",
        "t": "adj.",
        "m": "沮丧的；消沉的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "eyesight",
        "p": "/'aɪsaɪt/",
        "t": "n.",
        "m": "视力",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "dentist",
        "p": "/'dentɪst/",
        "t": "n.",
        "m": "牙科医生",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "recovery",
        "p": "/rɪ'kʌvəri/",
        "t": "n.",
        "m": "恢复；愈合",
        "e": [],
        "u": "6",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "deal with",
        "p": "",
        "t": "v. phr.",
        "m": "解决；处理；应对",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "guard against",
        "p": "",
        "t": "v. phr.",
        "m": "防止；提防",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "cancel out",
        "p": "",
        "t": "v. phr.",
        "m": "抵消；消除",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "look on the bright side",
        "p": "",
        "t": "idiom",
        "m": "持乐观的态度",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "take up",
        "p": "",
        "t": "v. phr.",
        "m": "(尤指为消遣)学着做；开始做",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "be (busy) ...ing",
        "p": "",
        "t": "v. phr.",
        "m": "忙于做......",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "leave ... behind",
        "p": "",
        "t": "v. phr.",
        "m": "把......抛在后面",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "cheer up",
        "p": "",
        "t": "v. phr.",
        "m": "(使)振奋起来",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "focus",
        "p": "/'fəʊkəs/",
        "t": "v.",
        "m": "集中 (注意力、精力等)",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "peer",
        "p": "/pɪə(r)/",
        "t": "n.",
        "m": "同龄人；同辈",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "pressure",
        "p": "/'preʃə(r)/",
        "t": "n.",
        "m": "压力",
        "e": [],
        "u": "6",
        "tags": [
          "pre_pre"
        ]
      },
      {
        "w": "whether",
        "p": "/'weðə(r)/",
        "t": "conj.",
        "m": "是否",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "risk",
        "p": "/rɪsk/",
        "t": "n.",
        "m": "风险",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "guard",
        "p": "/ɡɑːd/",
        "t": "v.",
        "m": "守卫；保卫",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "positive",
        "p": "/'pɒzətɪv/",
        "t": "adj.",
        "m": "积极乐观的；自信的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "cancel",
        "p": "/'kænsl/",
        "t": "v.",
        "m": "取消；撤销；终止",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "bright",
        "p": "/braɪt/",
        "t": "adj.",
        "m": "有希望的；明亮的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "force",
        "p": "/fɔːs/",
        "t": "v.",
        "m": "强迫；迫使",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "concert",
        "p": "/'kɒnsət/",
        "t": "n.",
        "m": "音乐会；演奏会",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "private",
        "p": "/'praɪvət/",
        "t": "adj.",
        "m": "私人的；私有的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "silent",
        "p": "/'saɪlənt/",
        "t": "adj.",
        "m": "不说话的；沉默的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "musical instrument",
        "p": "/mju'zɪkl 'ɪnstrəmənt/",
        "t": "n.",
        "m": "乐器",
        "e": [],
        "u": "6",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "enemy",
        "p": "/'enəmi/",
        "t": "n.",
        "m": "危害物；敌人",
        "e": [],
        "u": "6",
        "tags": []
      }
    ]
  },
  "8B": {
    "1": [
      {
        "w": "elderly",
        "p": "/'eldəli/",
        "t": "adj.",
        "m": "上了年纪的",
        "e": [],
        "u": "1",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "offer",
        "p": "/'ɔːfə(r)/",
        "t": "v.",
        "m": "主动提出",
        "e": [],
        "u": "1",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "serious",
        "p": "/'sɪəriəs/",
        "t": "adj.",
        "m": "严重的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "disease",
        "p": "/dɪ'ziːz/",
        "t": "n.",
        "m": "病；疾病",
        "e": [],
        "u": "1",
        "tags": [
          "pre_dis"
        ]
      },
      {
        "w": "cancer",
        "p": "/'kænsə(r)/",
        "t": "n.",
        "m": "癌症",
        "e": [],
        "u": "1",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "spirits",
        "p": "/'spɪrɪts/",
        "t": "[pl.] n.",
        "m": "情绪；心境",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "pain",
        "p": "/peɪn/",
        "t": "n.",
        "m": "痛苦；苦恼",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "belong",
        "p": "/bɪ'lɒŋ/",
        "t": "v.",
        "m": "适应；合得来",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "spare",
        "p": "/'speə(r)/",
        "t": "adj.",
        "m": "空闲的；可余下的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "volunteer",
        "p": "/ˌvɒlən'tɪə(r)/",
        "t": "v.",
        "m": "自愿做",
        "e": [],
        "u": "1",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "joy",
        "p": "/dʒɔɪ/",
        "t": "n.",
        "m": "高兴；愉快；喜悦",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "lose",
        "p": "/luːz/",
        "t": "v.",
        "m": "丧失",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "raise",
        "p": "/reɪz/",
        "t": "v.",
        "m": "筹募",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "pound",
        "p": "/paʊnd/",
        "t": "n.",
        "m": "英镑",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "structure",
        "p": "/'strʌktʃə(r)/",
        "t": "n.",
        "m": "结构",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "permission",
        "p": "/pə'mɪʃn/",
        "t": "n.",
        "m": "准许；许可",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "arrange",
        "p": "/ə'reɪndʒ/",
        "t": "v.",
        "m": "排列；布置",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "donate",
        "p": "/dəʊ'neɪt/",
        "t": "v.",
        "m": "赠送",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "mighty",
        "p": "/'maɪti/",
        "t": "adj.",
        "m": "巨大的；非凡的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "pleasant",
        "p": "/'pleznt/",
        "t": "adj.",
        "m": "宜人的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "ease",
        "p": "/iːz/",
        "t": "v.",
        "m": "减轻；缓解",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "ache",
        "p": "/eɪk/",
        "t": "n.",
        "m": "疼痛；隐痛",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "voluntary work",
        "p": "",
        "t": "phrase",
        "m": "志愿工作",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "disabled people",
        "p": "",
        "t": "phrase",
        "m": "残障人士",
        "e": [],
        "u": "1",
        "tags": [
          "pre_dis"
        ]
      },
      {
        "w": "look after",
        "p": "",
        "t": "v. phr.",
        "m": "照料；照顾",
        "e": [],
        "u": "1",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "suffer from",
        "p": "",
        "t": "v. phr.",
        "m": "遭受；患...",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "cheer somebody up",
        "p": "",
        "t": "v. phr.",
        "m": "(使)高兴起来",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "raise one's spirits",
        "p": "",
        "t": "v. phr.",
        "m": "使某人振奋",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "in low spirits",
        "p": "",
        "t": "phrase",
        "m": "情绪低落",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "in order to",
        "p": "",
        "t": "phrase",
        "m": "目的是；为了",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "in need",
        "p": "",
        "t": "phrase",
        "m": "在危难中；在穷困中的",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "flower arranging",
        "p": "",
        "t": "phrase",
        "m": "插花",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "tidy up",
        "p": "",
        "t": "v. phr.",
        "m": "收拾；整理",
        "e": [],
        "u": "1",
        "tags": []
      },
      {
        "w": "in vain",
        "p": "",
        "t": "phrase",
        "m": "枉费心机；徒劳无益",
        "e": [],
        "u": "1",
        "tags": []
      }
    ],
    "2": [
      {
        "w": "shake",
        "p": "/ʃeɪk/",
        "t": "v.",
        "m": "摇动；抖动",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "clap",
        "p": "/klæp/",
        "t": "v.",
        "m": "鼓掌；拍手",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "madam",
        "p": "/'mædəm/",
        "t": "n.",
        "m": "夫人；女士",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "boss",
        "p": "/bɒs/",
        "t": "n.",
        "m": "老板",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "expression",
        "p": "/ɪk'spreʃn/",
        "t": "n.",
        "m": "表情；神色",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "impression",
        "p": "/ɪm'preʃn/",
        "t": "n.",
        "m": "印象",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "hold",
        "p": "/həʊld/",
        "t": "v.",
        "m": "保持（在某位置）",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "observe",
        "p": "/əb'zɜːv/",
        "t": "v.",
        "m": "观察；注意到",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "remind",
        "p": "/rɪ'maɪnd/",
        "t": "v.",
        "m": "提醒；使想起",
        "e": [],
        "u": "2",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "emphasize",
        "p": "/'emfəsaɪz/",
        "t": "v.",
        "m": "强调",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "nod",
        "p": "/nɒd/",
        "t": "v.",
        "m": "点头",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "meaning",
        "p": "/'miːnɪŋ/",
        "t": "n.",
        "m": "意思；意义",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "immediately",
        "p": "/ɪ'miːdiətli/",
        "t": "adv.",
        "m": "马上；立即",
        "e": [],
        "u": "2",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "shy",
        "p": "/ʃaɪ/",
        "t": "adj.",
        "m": "腼腆的；羞怯的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "case",
        "p": "/keɪs/",
        "t": "n.",
        "m": "情况；事例",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "direct",
        "p": "/də'rekt/",
        "t": "adj.",
        "m": "直接的",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "truly",
        "p": "/'truːli/",
        "t": "adv.",
        "m": "真诚地",
        "e": [],
        "u": "2",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "suppose",
        "p": "/sə'pəʊz/",
        "t": "v.",
        "m": "假设；认为",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "behaviour",
        "p": "/bɪ'heɪvjə(r)/",
        "t": "n.",
        "m": "行为；举止",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "uncomfortable",
        "p": "/ʌn'kʌmftəbl/",
        "t": "adj.",
        "m": "不舒服的；局促不安的",
        "e": [],
        "u": "2",
        "tags": [
          "pre_un"
        ]
      },
      {
        "w": "hold something up",
        "p": "",
        "t": "v. phr.",
        "m": "抬起；举起",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "sit up",
        "p": "",
        "t": "v. phr.",
        "m": "坐起来；坐直",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "make a good impression on",
        "p": "",
        "t": "v. phr.",
        "m": "给……留下好印象",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "eye contact",
        "p": "",
        "t": "n. phr.",
        "m": "目光接触",
        "e": [],
        "u": "2",
        "tags": []
      },
      {
        "w": "pay attention",
        "p": "",
        "t": "v. phr.",
        "m": "注意",
        "e": [],
        "u": "2",
        "tags": [
          "suf_tion"
        ]
      }
    ],
    "3": [
      {
        "w": "performance",
        "p": "/pə'mɑːns/",
        "t": "n.",
        "m": "表演；演出",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "combine",
        "p": "/kəm'baɪn/",
        "t": "v.",
        "m": "组合；联合",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "theme",
        "p": "/θiːm/",
        "t": "n.",
        "m": "主题",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "advanced",
        "p": "/əd'vɑːnst/",
        "t": "adj.",
        "m": "先进的",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "artificial intelligence",
        "p": "/ˌɑːtɪ'fɪʃl ɪn'telɪdʒəns/",
        "t": "n. phr.",
        "m": "人工智能",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "hero",
        "p": "/'hɪərəʊ/",
        "t": "n.",
        "m": "男主角；英雄",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "classic",
        "p": "/'klæsɪk/",
        "t": "adj.",
        "m": "经典的",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "speech",
        "p": "/spiːtʃ/",
        "t": "n.",
        "m": "讲说；演说",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "text",
        "p": "/tekst/",
        "t": "n.",
        "m": "正文；文本",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "warning",
        "p": "/'wɔːnɪŋ/",
        "t": "n.",
        "m": "警告；警示",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "mind",
        "p": "/maɪnd/",
        "t": "n.",
        "m": "心；注意",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "gap",
        "p": "/ɡæp/",
        "t": "n.",
        "m": "裂口；间隙",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "crash",
        "p": "/kræʃ/",
        "t": "v.",
        "m": "碰撞；破裂",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "wave",
        "p": "/weɪv/",
        "t": "n.",
        "m": "波浪",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "folk",
        "p": "/fəʊk/",
        "t": "adj.",
        "m": "民间的；民俗的",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "risk",
        "p": "/rɪsk/",
        "t": "n.",
        "m": "危险；风险",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "leather",
        "p": "/'leðə(r)/",
        "t": "n.",
        "m": "皮革",
        "e": [],
        "u": "3",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "advantage",
        "p": "/əd'vɑːntɪdʒ/",
        "t": "n.",
        "m": "优点；优势",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "breath",
        "p": "/breθ/",
        "t": "n.",
        "m": "呼吸",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "element",
        "p": "/'elɪmənt/",
        "t": "n.",
        "m": "要素；基本部分",
        "e": [],
        "u": "3",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "shadow puppet",
        "p": "",
        "t": "n. phr.",
        "m": "皮影",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "ink-wash painting",
        "p": "",
        "t": "n. phr.",
        "m": "水墨画",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "come to life",
        "p": "",
        "t": "v. phr.",
        "m": "变得生动；活过来",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "with the help of",
        "p": "",
        "t": "phrase",
        "m": "在...的帮助下",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "comic strip",
        "p": "",
        "t": "n. phr.",
        "m": "连环漫画",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "watch out",
        "p": "",
        "t": "v. phr.",
        "m": "小心；留神",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "look out",
        "p": "",
        "t": "v. phr.",
        "m": "小心；当心",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "weather forecast",
        "p": "",
        "t": "n. phr.",
        "m": "天气预报",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "life jacket",
        "p": "",
        "t": "n. phr.",
        "m": "救生衣",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "thank goodness",
        "p": "",
        "t": "phrase",
        "m": "谢天谢地",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "make it",
        "p": "",
        "t": "v. phr.",
        "m": "侥幸做到；渡过难关",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "date back",
        "p": "",
        "t": "v. phr.",
        "m": "追溯到；始于",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "have an influence on",
        "p": "",
        "t": "v. phr.",
        "m": "对...有影响",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "at risk",
        "p": "",
        "t": "phrase",
        "m": "处在危险中",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "in the shape of",
        "p": "",
        "t": "phrase",
        "m": "以...的形状",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "take one's eyes off something",
        "p": "",
        "t": "v. phr.",
        "m": "把视线移开",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "hold one's breath",
        "p": "",
        "t": "v. phr.",
        "m": "屏住呼吸",
        "e": [],
        "u": "3",
        "tags": []
      },
      {
        "w": "neither...nor...",
        "p": "",
        "t": "conjunction",
        "m": "既不...也不...",
        "e": [],
        "u": "3",
        "tags": []
      }
    ],
    "4": [
      {
        "w": "master",
        "p": "/'mɑːstə(r)/",
        "t": "n.",
        "m": "著名画家；大师",
        "e": [],
        "u": "4",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "masterpiece",
        "p": "/'mɑːstəpiːs/",
        "t": "n.",
        "m": "代表作；杰作",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "whenever",
        "p": "/wen'evə(r)/",
        "t": "conj.",
        "m": "每当；每次",
        "e": [],
        "u": "4",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "alone",
        "p": "/ə'luːn/",
        "t": "adv.",
        "m": "独自",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "copy",
        "p": "/'kɒpi/",
        "t": "v.",
        "m": "仿造；临摹",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "unfortunately",
        "p": "/ʌn'fɔːtʃənətli/",
        "t": "adv.",
        "m": "不幸地；遗憾地",
        "e": [],
        "u": "4",
        "tags": [
          "suf_ly",
          "pre_un"
        ]
      },
      {
        "w": "graduate",
        "p": "/'ɡrædʒueɪt/",
        "t": "v.",
        "m": "毕业",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "limit",
        "p": "/'lɪmɪt/",
        "t": "n./v.",
        "m": "限量；量",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "temperature",
        "p": "/'temprətʃə(r)/",
        "t": "n.",
        "m": "温度",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "preserve",
        "p": "/prɪ'zɜːv/",
        "t": "v.",
        "m": "维持；保存",
        "e": [],
        "u": "4",
        "tags": [
          "pre_pre"
        ]
      },
      {
        "w": "exhibition",
        "p": "/ˌeksɪ'bɪʃn/",
        "t": "/eks'bɪʃn/",
        "m": "展览",
        "e": [],
        "u": "4",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "lifestyle",
        "p": "/'laɪfstaɪl/",
        "t": "n.",
        "m": "生活方式",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "detail",
        "p": "/'diːteɪl/",
        "t": "n.",
        "m": "细节；局部",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "daughter",
        "p": "/'dɔːtə(r)/",
        "t": "n.",
        "m": "女儿",
        "e": [],
        "u": "4",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "priceless",
        "p": "/'praɪsləs/",
        "t": "adj.",
        "m": "无价的",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "sculpture",
        "p": "/'skʌlptʃə(r)/",
        "t": "n.",
        "m": "雕塑",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "in one sitting",
        "p": "",
        "t": "phrase",
        "m": "一口气地",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "once in a while",
        "p": "",
        "t": "phrase",
        "m": "偶尔；间或",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "once in a blue moon",
        "p": "",
        "t": "phrase",
        "m": "罕见地；千载难逢",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "be related to",
        "p": "",
        "t": "v. phr.",
        "m": "与...有关；与...相关",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "be proud of",
        "p": "",
        "t": "v. phr.",
        "m": "为...感到自豪",
        "e": [],
        "u": "4",
        "tags": [
          "phr_be_of"
        ]
      },
      {
        "w": "be full of",
        "p": "",
        "t": "v. phr.",
        "m": "充满...",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "even so",
        "p": "",
        "t": "phrase",
        "m": "尽管如此",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "in addition",
        "p": "",
        "t": "phrase",
        "m": "此外；另外",
        "e": [],
        "u": "4",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "all the time",
        "p": "",
        "t": "phrase",
        "m": "一直；始终",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "make up one's mind",
        "p": "",
        "t": "v. phr.",
        "m": "下定决心",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "take part in",
        "p": "",
        "t": "v. phr.",
        "m": "参加；参与",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "be interested in",
        "p": "",
        "t": "v. phr.",
        "m": "对...感兴趣",
        "e": [],
        "u": "4",
        "tags": [
          "phr_be_in"
        ]
      },
      {
        "w": "the first time",
        "p": "",
        "t": "phrase",
        "m": "第一次",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "at least",
        "p": "",
        "t": "phrase",
        "m": "至少",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "as well as",
        "p": "",
        "t": "phrase",
        "m": "也；以及",
        "e": [],
        "u": "4",
        "tags": []
      },
      {
        "w": "instead of",
        "p": "",
        "t": "phrase",
        "m": "代替",
        "e": [],
        "u": "4",
        "tags": []
      }
    ],
    "5": [
      {
        "w": "recognize",
        "p": "/rɪ'kɒɡnaɪz/",
        "t": "v.",
        "m": "认出；识别",
        "e": [],
        "u": "5",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "mirror",
        "p": "/'mɪrə(r)/",
        "t": "n.",
        "m": "镜子",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "gun",
        "p": "/ɡʌn/",
        "t": "n.",
        "m": "枪；炮",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "handsome",
        "p": "/'hænsəm/",
        "t": "adj.",
        "m": "英俊的；潇洒的",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "force",
        "p": "/fɔːs/",
        "t": "v.",
        "m": "强迫；迫使",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "corn",
        "p": "/kɔːn/",
        "t": "n.",
        "m": "谷物；玉米",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "direction",
        "p": "/də'rekʃn/",
        "t": "n.",
        "m": "方向；方位",
        "e": [],
        "u": "5",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "progress",
        "p": "/'prəʊɡres/",
        "t": "n.",
        "m": "进步；进展",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "value",
        "p": "/'væljuː/",
        "t": "v.",
        "m": "重视；珍视",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "official",
        "p": "/ə'fɪʃl/",
        "t": "adj.",
        "m": "官方的",
        "e": [],
        "u": "5",
        "tags": [
          "suf_al"
        ]
      },
      {
        "w": "throw",
        "p": "/θrəʊ/",
        "t": "v.",
        "m": "扔；抛",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "law",
        "p": "/lɔː/",
        "t": "n.",
        "m": "法律；法规",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "punish",
        "p": "/'pʌnɪʃ/",
        "t": "v.",
        "m": "处罚；惩罚",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "blog",
        "p": "/blɒɡ/",
        "t": "n.",
        "m": "博客",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "publish",
        "p": "/'pʌblɪʃ/",
        "t": "v.",
        "m": "发表；公布",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "shoulder",
        "p": "/'ʃəʊldə(r)/",
        "t": "n.",
        "m": "肩膀",
        "e": [],
        "u": "5",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "perhaps",
        "p": "/pə'hæps/",
        "t": "adv.",
        "m": "也许；可能",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "slim",
        "p": "/slɪm/",
        "t": "adj.",
        "m": "苗条的；纤细的",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "alarmed",
        "p": "/ə'lɑːmd/",
        "t": "adj.",
        "m": "惊吓的；受惊的",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "doubt",
        "p": "/daʊt/",
        "t": "v.",
        "m": "怀疑",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "sore",
        "p": "/sɔː(r)/",
        "t": "adj.",
        "m": "疼痛的；酸痛的",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "red-crowned crane",
        "p": "",
        "t": "n. phr.",
        "m": "丹顶鹤",
        "e": [],
        "u": "5",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "polar bear",
        "p": "",
        "t": "n. phr.",
        "m": "北极熊",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "more than",
        "p": "",
        "t": "phrase",
        "m": "多于；超过",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "shoot",
        "p": "",
        "t": "v.",
        "m": "射击；拍摄",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "make use of",
        "p": "",
        "t": "v. phr.",
        "m": "利用",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "without a care in the world",
        "p": "",
        "t": "phrase",
        "m": "无忧无虑",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "ought to",
        "p": "",
        "t": "v. phr.",
        "m": "应该",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "up to",
        "p": "",
        "t": "phrase",
        "m": "达到（某数量、程度等）",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "nature reserve",
        "p": "",
        "t": "n. phr.",
        "m": "自然保护区",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "day and night",
        "p": "",
        "t": "phrase",
        "m": "日以继夜",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "go missing",
        "p": "",
        "t": "v. phr.",
        "m": "失踪；不见",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "look for",
        "p": "",
        "t": "v. phr.",
        "m": "寻找",
        "e": [],
        "u": "5",
        "tags": []
      },
      {
        "w": "lose one's life",
        "p": "",
        "t": "v. phr.",
        "m": "丧命；失去生命",
        "e": [],
        "u": "5",
        "tags": []
      }
    ],
    "6": [
      {
        "w": "stage",
        "p": "/steɪdʒ/",
        "t": "n.",
        "m": "舞台",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "clue",
        "p": "/kluː/",
        "t": "n.",
        "m": "线索；提示",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "promote",
        "p": "/prə'məʊt/",
        "t": "v.",
        "m": "促进；推动",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "abroad",
        "p": "/ə'brɔːd/",
        "t": "adv.",
        "m": "在国外；到国外",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "costume",
        "p": "/'kɒstjuːm/",
        "t": "n.",
        "m": "戏装；服装",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "opportunity",
        "p": "/ˌɒpə'tjuːnəti/",
        "t": "n.",
        "m": "机会",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "acquire",
        "p": "/ə'kwaɪə(r)/",
        "t": "v.",
        "m": "获得；得到",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "sheet",
        "p": "/ʃiːt/",
        "t": "n.",
        "m": "一张（通常指纸张尺寸）",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "conclusion",
        "p": "/kən'kluːʒn/",
        "t": "n.",
        "m": "结束；结尾",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "poetry",
        "p": "/'pəʊətri/",
        "t": "n.",
        "m": "诗歌",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "concerned",
        "p": "/kən'sɜːnd/",
        "t": "adj.",
        "m": "关切的；注意的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "creative",
        "p": "/kri'eɪtɪv/",
        "t": "adj.",
        "m": "创造（性）的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "pleasure",
        "p": "/'pleʒə(r)/",
        "t": "n.",
        "m": "高兴；快乐",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "rosy",
        "p": "/'rəʊzi/",
        "t": "adj.",
        "m": "粉红色的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "youth",
        "p": "/juːθ/",
        "t": "n.",
        "m": "年轻；青春",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "worth",
        "p": "/wɜːθ/",
        "t": "adj.",
        "m": "有价值的",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "Christmas",
        "p": "/'krɪsməs/",
        "t": "n.",
        "m": "圣诞节",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "eve",
        "p": "/iːv/",
        "t": "n.",
        "m": "前夕；前夕",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "spelling bee",
        "p": "",
        "t": "n. phr.",
        "m": "拼写比赛",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "treasure hunt",
        "p": "",
        "t": "n. phr.",
        "m": "寻宝游戏",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "cultural exchange",
        "p": "",
        "t": "n. phr.",
        "m": "文化交流",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "technological development",
        "p": "",
        "t": "n. phr.",
        "m": "技术发展",
        "e": [],
        "u": "6",
        "tags": [
          "suf_ment"
        ]
      },
      {
        "w": "no wonder",
        "p": "",
        "t": "phrase",
        "m": "不难理解；难怪",
        "e": [],
        "u": "6",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "high-speed train",
        "p": "",
        "t": "n. phr.",
        "m": "高铁",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "pass by",
        "p": "",
        "t": "v. phr.",
        "m": "经过；驶过",
        "e": [],
        "u": "6",
        "tags": []
      },
      {
        "w": "go by",
        "p": "",
        "t": "v. phr.",
        "m": "（时间）流逝；过去",
        "e": [],
        "u": "6",
        "tags": []
      }
    ],
    "7": [
      {
        "w": "beef",
        "p": "/biːf/",
        "t": "n.",
        "m": "牛肉",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "seafood",
        "p": "/'siːfuːd/",
        "t": "n.",
        "m": "海鲜",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "afford",
        "p": "/ə'fɔːd/",
        "t": "v.",
        "m": "承担得起（后果）",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "mistake",
        "p": "/mɪ'steɪk/",
        "t": "n.",
        "m": "错误；失误",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "stick",
        "p": "/stɪk/",
        "t": "v.",
        "m": "粘；贴",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "hit",
        "p": "/hɪt/",
        "t": "v.",
        "m": "碰撞；撞击",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "muscle",
        "p": "/'mʌsl/",
        "t": "n.",
        "m": "肌肉",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "gym",
        "p": "/dʒɪm/",
        "t": "n.",
        "m": "健身房",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "satellite",
        "p": "/'sætəlaɪt/",
        "t": "n.",
        "m": "人造卫星",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "destroy",
        "p": "/dɪ'strɔɪ/",
        "t": "v.",
        "m": "摧毁",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "average",
        "p": "/'ævərɪdʒ/",
        "t": "adj.",
        "m": "平均的",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "probable",
        "p": "/'prɒbəbl/",
        "t": "adj.",
        "m": "很可能发生的（或存在的）",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "select",
        "p": "/sɪ'lekt/",
        "t": "v.",
        "m": "挑选；选拔",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "spaceflight",
        "p": "/'speɪsflaɪt/",
        "t": "n.",
        "m": "宇宙飞行",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "serve",
        "p": "/sɜːv/",
        "t": "v.",
        "m": "为……工作；服务",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "deliver",
        "p": "/dɪ'lɪvə(r)/",
        "t": "v.",
        "m": "传送；运送",
        "e": [],
        "u": "7",
        "tags": [
          "suf_er"
        ]
      },
      {
        "w": "fuel",
        "p": "/'fjuːəl/",
        "t": "n.",
        "m": "燃料",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "supplies",
        "p": "/sə'plaɪz/",
        "t": "[pl.]",
        "m": "补给；供给",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "point",
        "p": "/pɔɪnt/",
        "t": "n.",
        "m": "时刻；阶段",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "lab",
        "p": "/læb/",
        "t": "n.",
        "m": "实验室",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "take off",
        "p": "",
        "t": "v. phr.",
        "m": "起飞",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "for a while",
        "p": "",
        "t": "phrase",
        "m": "一段时间",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "turn out",
        "p": "",
        "t": "v. phr.",
        "m": "证明是；结果是",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "cannot afford to",
        "p": "",
        "t": "v. phr.",
        "m": "承担不起（后果）",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "by mistake",
        "p": "",
        "t": "phrase",
        "m": "错误地；无意中",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "daily routine",
        "p": "",
        "t": "n. phr.",
        "m": "日常生活",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "space junk",
        "p": "",
        "t": "n. phr.",
        "m": "太空垃圾",
        "e": [],
        "u": "7",
        "tags": []
      },
      {
        "w": "naked eye",
        "p": "",
        "t": "n. phr.",
        "m": "肉眼",
        "e": [],
        "u": "7",
        "tags": []
      }
    ],
    "8": [
      {
        "w": "mention",
        "p": "/'menʃn/",
        "t": "v.",
        "m": "提到；说到",
        "e": [],
        "u": "8",
        "tags": [
          "suf_tion"
        ]
      },
      {
        "w": "tonight",
        "p": "/tə'naɪt/",
        "t": "adv.",
        "m": "在今夜；在今晚",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "frightened",
        "p": "/'fraɪtnd/",
        "t": "adj.",
        "m": "受惊的；害怕的",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "throat",
        "p": "/θrəʊt/",
        "t": "n.",
        "m": "喉咙",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "cough",
        "p": "/kɒf/",
        "t": "v./n.",
        "m": "咳嗽",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "fear",
        "p": "/fɪə(r)/",
        "t": "n./v.",
        "m": "恐惧；害怕",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "creature",
        "p": "/'kriːtʃə(r)/",
        "t": "n.",
        "m": "生物；动物",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "ugly",
        "p": "/'ʌɡli/",
        "t": "adj.",
        "m": "丑陋的",
        "e": [],
        "u": "8",
        "tags": [
          "suf_ly"
        ]
      },
      {
        "w": "crazy",
        "p": "/'kreɪzi/",
        "t": "adj.",
        "m": "疯狂的；不理智的",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "refuse",
        "p": "/rɪ'fjuːz/",
        "t": "v.",
        "m": "拒绝；回绝",
        "e": [],
        "u": "8",
        "tags": [
          "pre_re"
        ]
      },
      {
        "w": "emergency",
        "p": "/ɪ'mɜːdʒənsi/",
        "t": "n.",
        "m": "紧急情况",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "camera",
        "p": "/'kæmərə/",
        "t": "n.",
        "m": "照相机",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "tent",
        "p": "/tent/",
        "t": "n.",
        "m": "帐篷",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "unnecessary",
        "p": "/ʌn'nesəsəri/",
        "t": "adj.",
        "m": "不必要的；多余的",
        "e": [],
        "u": "8",
        "tags": [
          "pre_un"
        ]
      },
      {
        "w": "because of",
        "p": "",
        "t": "phrase",
        "m": "因为；由于",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "in fear",
        "p": "",
        "t": "phrase",
        "m": "恐惧地",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "landing site",
        "p": "",
        "t": "n. phr.",
        "m": "着陆地点；降落场",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "agree with",
        "p": "",
        "t": "v. phr.",
        "m": "同意；与...一致",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "disagree with",
        "p": "",
        "t": "v. phr.",
        "m": "不同意",
        "e": [],
        "u": "8",
        "tags": [
          "pre_dis"
        ]
      },
      {
        "w": "move about",
        "p": "",
        "t": "v. phr.",
        "m": "四处走动",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "in all directions",
        "p": "",
        "t": "phrase",
        "m": "向四面八方",
        "e": [],
        "u": "8",
        "tags": []
      },
      {
        "w": "speed up",
        "p": "",
        "t": "v. phr.",
        "m": "加速",
        "e": [],
        "u": "8",
        "tags": []
      }
    ]
  }
};;
;

// --- Original App Logic ---
alert('🚀 App.js is running!');
alert('App.js Running!');
// app.js — 广州初中英语 · 沪教版 (艾宾浩斯算法版 + 规律学习模式)
(function() {
  const $ = (s) => document.querySelector(s);
  const $$ = (s) => document.querySelectorAll(s);

  // --- 调试工具 ---
  function showErrorToUI(msg, error) {
    let errDiv = $("#debug-error");
    if (!errDiv) {
      errDiv = document.createElement('div');
      errDiv.id = 'debug-error';
      errDiv.style = "position:fixed;top:0;left:0;width:100%;background:rgba(255,0,0,0.9);color:white;z-index:9999;padding:15px;font-family:monospace;font-size:12px;overflow:auto;max-height:40%;";
      document.body.prepend(errDiv);
    }
    errDiv.innerHTML = `<strong style="font-size:16px">❌ App Error:</strong><br>${msg}<br><hr><pre style="white-space:pre-wrap;word-break:break-all;">${error?.stack || error}</pre>`;
    console.error("App Error:", msg, error);
  }

  // --- 状态管理 ---
  let state = {
    currentGrade: "7A",
    currentUnit: null,
    mode: "word", 
    progress: 0,
    reviewQueue: [],
    currentWordList: [],
    currentIndex: 0,
    currentWord: null,
    patternId: null 
  };

  let memoryMap = {};
  let patternsData = [];

  // --- 初始化 ---
  document.addEventListener('DOMContentLoaded', () => {
    try {
      if (typeof WORDS === 'undefined') throw new Error("WORDS 数据加载失败");
      
      if (typeof PATTERNS !== 'undefined') {
        patternsData = PATTERNS;
        console.log("Patterns loaded:", patternsData);
      } else {
        console.warn("PATTERNS 未找到");
      }

      loadMemory();
      initTabs();
      initModeSwitch();
      updateProgressDisplay();
      
    } catch (e) {
      showErrorToUI("应用启动失败", e);
    }
  });

  function loadMemory() {
    const saved = localStorage.getItem('guangzhou_eng_memory');
    if (saved) {
      try { memoryMap = JSON.parse(saved); } catch (e) { memoryMap = {}; }
    }
  }

  function saveMemory() {
    try { localStorage.setItem('guangzhou_eng_memory', JSON.stringify(memoryMap)); } catch (e) { console.error(e); }
  }

  // --- UI 逻辑 ---
  function initTabs() {
    const tabs = $("#grade-tabs");
    if (!tabs) return;
    tabs.innerHTML = '';
    const grades = Object.keys(WORDS).sort();
    grades.forEach(g => {
      const btn = document.createElement('button');
      btn.innerText = g;
      btn.onclick = () => setGrade(g);
      if (g === state.currentGrade) btn.classList.add('on');
      tabs.appendChild(btn);
    });
  }

  function initModeSwitch() {
    const btnWord = $("#btn-word");
    const btnGrammar = $("#btn-grammar");
    const btnPattern = $("#btn-pattern");
    if (btnWord) btnWord.onclick = () => setMode('word');
    if (btnGrammar) btnGrammar.onclick = () => setMode('grammar');
    if (btnPattern) btnPattern.onclick = () => setMode('pattern');
  }

  function setGrade(g) {
    state.currentGrade = g;
    state.currentUnit = null;
    state.patternId = null;
    $$("#grade-tabs button").forEach(b => {
      b.classList.toggle('on', b.innerText === g);
    });
    renderUnitSelector();
    renderMain();
  }

  function setMode(m) {
    state.mode = m;
    state.patternId = null;
    $("#btn-word").classList.toggle('on', m === 'word');
    $("#btn-grammar").classList.toggle('on', m === 'grammar');
    $("#btn-pattern").classList.toggle('on', m === 'pattern');
    
    const unitSelector = $("#unit-selector");
    if (unitSelector) {
        unitSelector.style.display = (m === 'grammar' || m === 'pattern') ? 'none' : 'flex';
    }
    
    renderMain();
  }

  function setUnit(u) {
    try {
      state.currentUnit = u;
      state.patternId = null;
      $$(".sel-btn").forEach(b => {
        const btnUnit = b.dataset.unit;
        b.classList.toggle('on', String(btnUnit) === String(u));
      });
      prepareQueue();
      renderMain();
    } catch (e) {
      showErrorToUI("切换单元失败", e);
    }
  }

  // --- 单词队列逻辑 ---
  function prepareQueue() {
    try {
      let allWordsData = WORDS[state.currentGrade] || [];
      const targetUnit = String(state.currentUnit);
      let targetWords = [];

      if (Array.isArray(allWordsData)) {
        targetWords = allWordsData;
      } else if (typeof allWordsData === 'object' && allWordsData !== null) {
        Object.values(allWordsData).forEach(uList => {
            if (Array.isArray(uList)) targetWords.push(...uList);
        });
      }

      const unitWords = targetWords.filter(w => {
        const wu = String(w.u || "");
        return wu === targetUnit || wu.startsWith(targetUnit + ".");
      });
      
      const now = Date.now();
      const reviewList = [];
      const learningList = [];

      unitWords.forEach(word => {
        const key = `${state.currentGrade}_${word.u}_${word.w}`;
        const mem = memoryMap[key];
        if (mem && mem.nextReview <= now) {
          reviewList.push(word);
        } else {
          learningList.push(word);
        }
      });

      const shuffledReview = reviewList.sort(() => Math.random() - 0.5);
      const shuffledLearning = learningList.sort(() => Math.random() - 0.5);
      
      state.reviewQueue = [...shuffledReview, ...shuffledLearning];
      state.currentWordList = [...state.reviewQueue];
      state.currentIndex = 0;
      state.currentWord = state.currentWordList.length > 0 ? state.currentWordList[0] : null;
      
      const masteredCount = unitWords.filter(w => {
        const key = `${state.currentGrade}_${w.u}_${w.w}`;
        return memoryMap[key] && memoryMap[key].level > 0;
      }).length;
      
      state.progress = unitWords.length > 0 ? (masteredCount / unitWords.length) * 100 : 0;
      updateProgressDisplay();
    } catch (e) {
      showErrorToUI("准备队列时崩溃", e);
    }
  }

  function getNextWord() {
    if (state.currentIndex >= 0 && state.currentIndex < state.currentWordList.length) {
      return state.currentWordList[state.currentIndex];
    }
    return null;
  }

  // --- 渲染逻辑 ---
  function renderUnitSelector() {
    const area = $("#unit-selector");
    if (!area) return;
    area.innerHTML = '';
    if (state.mode === 'grammar' || state.mode === 'pattern') {
        area.style.display = 'none';
        return;
    }
    area.style.display = 'flex';

    try {
      let allWordsData = WORDS[state.currentGrade] || [];
      const units = [];
      if (Array.isArray(allWordsData)) {
        const unitSet = new Set(allWordsData.map(w => w.u).filter(u => u !== undefined && u !== null));
        unitSet.forEach(u => units.push(String(u)));
      } else if (typeof allWordsData === 'object' && allWordsData !== null) {
        Object.keys(allWordsData).forEach(u => units.push(u));
      }

      units.sort((a, b) => {
        const na = parseInt(String(a).replace(/[^0-9]/g, '')) || 0;
        const nb = parseInt(String(b).replace(/[^0-9]/g, '')) || 0;
        return na - nb;
      });
      
      if (units.length === 0) {
        area.innerHTML = '<p style="color:var(--text-sub); font-size:14px;">暂无单词数据</p>';
        return;
      }

      units.forEach(u => {
        const btn = document.createElement('button');
        btn.className = 'sel-btn';
        btn.innerText = `U${u}`;
        btn.dataset.unit = u;
        btn.onclick = (e) => { e.preventDefault(); setUnit(u); };
        if (String(state.currentUnit) === String(u)) btn.classList.add('on');
        area.appendChild(btn);
      });
    } catch (e) {
      showErrorToUI("渲染单元区域崩溃", e);
    }
  }

  function renderMain() {
    const pane = $("#main-pane");
    if (!pane) return;
    pane.innerHTML = '';

    if (state.mode === 'word') {
      if (!state.currentUnit) {
        pane.innerHTML = `<div class="empty-state animate-pop"><div class="empty-icon">👋</div><p>请从上方选择一个单元开始学习吧！</p></div>`;
      } else {
        renderWordCard(pane);
      }
    } else if (state.mode === 'grammar') {
      renderGrammarList(pane);
    } else if (state.mode === 'pattern') {
      renderPatternMode(pane);
    }
  }

  // --- 单词卡片渲染 ---
  function renderWordCard(pane) {
    try {
      const word = getNextWord();
      if (!word) {
        pane.innerHTML = `
          <div class="card animate-pop" style="padding: 40px 20px; text-align:center;">
            <div style="font-size: 50px; margin-bottom: 15px;">🎉</div>
            <h3 style="margin:0">学习完成！</h3>
            <p style="color:var(--text-sub)">本单元单词已全部完成复习</p>
            <button onclick="location.reload()" style="margin-top:20px; background:var(--primary); color:white; border:none; padding:10px 25px; border-radius:50px; font-weight:700;">重新开始</button>
          </div>`;
        return;
      }
      const key = `${state.currentGrade}_${word.u}_${word.w}`;
      const mem = memoryMap[key] || { level: 0, nextReview: 0 };
      const card = document.createElement('div');
      card.className = 'card word-card animate-pop';
      const displayW = word.w || "???";
      const displayP = word.p || "";
      const displayM = word.m || "";
      const displayE = (Array.isArray(word.e) && word.e.length > 0) ? word.e[0] : "暂无例句";
      card.innerHTML = `
        <span class="w">${displayW}</span><span class="p">${displayP}</span><span class="m">${displayM}</span>
        <div class="example">${displayE}</div>
        <div style="font-size:12px; color:var(--text-sub); margin-top:10px; text-align:center;">进度: ${state.currentIndex + 1} / ${state.currentWordList.length}</div>
        <div style="margin-top:20px; display:flex; gap:8px; justify-content:center; flex-wrap:wrap;">
          <button id="btn-prev" class="action-btn" style="background:#95a5a6; color:white; border:none; padding:8px 15px; border-radius:50px; font-weight:700;">上一个</button>
          <button id="btn-wrong" class="action-btn" style="background:#ff6b6b; color:white; border:none; padding:8px 15px; border-radius:50px; font-weight:700;">记不住 ❌</button>
          <button id="btn-right" class="action-btn" style="background:#4ecdc4; color:white; border:none; padding:8px 15px; border-radius:50px; font-weight:700;">掌握了 ✅</button>
          <button id="btn-next" class="action-btn" style="background:#95a5a6; color:white; border:none; padding:8px 15px; border-radius:50px; font-weight:700;">下一个</button>
        </div>`;
      pane.appendChild(card);
      card.querySelector("#btn-prev").onclick = () => { if (state.currentIndex > 0) { state.currentIndex--; renderMain(); } };
      card.querySelector("#btn-next").onclick = () => { if (state.currentIndex < state.currentWordList.length - 1) { state.currentIndex++; renderMain(); } else { renderMain(); } };
      card.querySelector("#btn-wrong").onclick = () => {
        mem.level = Math.max(0, mem.level - 1);
        mem.nextReview = Date.now() + (24 * 60 * 60 * 1000);
        memoryMap[key] = mem; saveMemory(); renderMain();
      };
      card.querySelector("#btn-right").onclick = () => {
        mem.level = Math.min(6, mem.level + 1);
        const days = [0, 1, 2, 4, 7, 15, 30][mem.level];
        mem.nextReview = Date.now() + (days * 24 * 60 * 60 * 1000);
        memoryMap[key] = mem; saveMemory();
        const allWords = WORDS[state.currentGrade] || [];
        let unitWords = Array.isArray(allWords) ? allWords.filter(w => String(w.u) === String(state.currentUnit)) : (allWords[state.currentUnit] || []);
        const masteredCount = unitWords.filter(w => {
          const k = `${state.currentGrade}_${w.u}_${w.w}`;
          return memoryMap[k] && memoryMap[k].level > 0;
        }).length;
        state.progress = unitWords.length > 0 ? (masteredCount / unitWords.length) * 100 : 0;
        updateProgressDisplay();
        if (state.currentIndex < state.currentWordList.length - 1) { state.currentIndex++; renderMain(); } else { renderMain(); }
      };
    } catch (e) { showErrorToUI("渲染单词卡片时崩溃", e); }
  }

  // --- 语法逻辑 ---
  function renderGrammarList(pane) {
    try {
      const grammarModules = GRAMMAR_BY_GRADE[state.currentGrade] || [];
      if (grammarModules.length === 0) {
        pane.innerHTML = `<div class="empty-state animate-pop"><div class="empty-icon">📚</div><p>该年级暂无语法知识点。</p></div>`;
        return;
      }
      const container = document.createElement('div');
      container.className = 'grammar-container';
      container.style.cssText = 'display:flex; flex-direction:column; gap:15px; padding:10px 0;';
      container.innerHTML = `<h3 style="margin:10px 0; padding-left:10px; color:var(--text-main)">${state.currentGrade} 核心语法知识点</h3>`;
      grammarModules.forEach(key => {
        const data = GRAMMAR[key];
        if (!data) return;
        const item = document.createElement('div');
        item.className = 'card animate-pop';
        item.style.cssText = 'cursor:pointer; padding:20px; position:relative; border-left:6px solid var(--primary);';
        item.innerHTML = `<h4 style="margin:0 0 5px 0; font-size:18px; color:var(--primary);">${data.title}</h4><p style="margin:0; font-size:14px; color:var(--text-sub);">${data.desc}</p><div style="position:absolute; right:15px; top:50%; transform:translateY(-50%); font-size:20px; color:#ddd;">📖</div>`;
        item.onclick = () => renderGrammarDetail(data);
        container.appendChild(item);
      });
      pane.appendChild(container);
    } catch (e) { showErrorToUI("渲染语法列表时崩溃", e); }
  }

  function renderGrammarDetail(data) {
    const pane = $("#main-pane");
    pane.innerHTML = '';
    const card = document.createElement('div');
    card.className = 'card animate-pop';
    card.style.cssText = 'padding:25px;';
    const rulesHtml = data.rules.map(r => `<li>${r}</li>`).join('');
    const examplesHtml = data.example.map(ex => `<div>${ex}</div>`).join('');
    card.innerHTML = `
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
        <h3 style="margin:0; color:var(--primary); font-size:22px;">${data.title}</h3>
        <button id="btn-back-grammar" style="background:none; border:none; font-size:24px; cursor:pointer;">⬅️</button>
      </div>
      <p style="font-size:16px; color:var(--text-main); line-height:1.5; margin-bottom:20px;">${data.desc}</p>
      <div style="margin-bottom:25px;"><h4 style="margin:0 0 10px 0; font-size:16px; color:var(--text-sub);">💡 核心规则</h4><ul style="padding-left:20px; margin:0; line-height:1.6; color:var(--text-main);">${rulesHtml}</ul></div>
      <div style="margin-bottom:25px;"><h4 style="margin:0 0 10px 0; font-size:16px; color:var(--text-sub);">📝 例句示范</h4><div style="background:rgba(108, 197, 163, 0.1); padding:15px; border-radius:12px; line-height:1.6; color:var(--text-main);">${examplesHtml}</div></div>
      ${data.note ? `<div style="background:#fff3cd; padding:12px; border-radius:8px; font-size:14px; color:#856404;"><strong style="color:#856404">⚠️ 注意：</strong><br>${data.note}</div>` : ''}
    `;
    pane.appendChild(card);
    card.querySelector("#btn-back-grammar").onclick = () => renderMain();
  }

  // --- 规律模式逻辑 (Pattern Mode) ---
  function renderPatternMode(pane) {
    if (!patternsData || patternsData.length === 0) {
      pane.innerHTML = `<div class="empty-state animate-pop"><div class="empty-icon">⏳</div><p>规律数据加载中...</p></div>`;
      return;
    }

    if (!state.patternId) {
      const container = document.createElement('div');
      container.className = 'grammar-container';
      container.style.cssText = 'display:flex; flex-direction:column; gap:15px; padding:10px 0;';
      container.innerHTML = `<h3 style="margin:10px 0; padding-left:10px; color:var(--text-main)">💡 规律学习：选择一个维度</h3>`;

      patternsData.forEach(p => {
        const item = document.createElement('div');
        item.className = 'card animate-pop';
        item.style.cssText = 'cursor:pointer; padding:20px; position:relative; border-left:6px solid var(--primary);';
        item.innerHTML = `<h4 style="margin:0 0 5px 0; font-size:18px; color:var(--primary);">${p.title}</h4><p style="margin:0; font-size:14px; color:var(--text-sub);">${p.desc}</p><div style="position:absolute; right:15px; top:50%; transform:translateY(-50%); font-size:20px; color:#ddd;">🔍</div>`;
        item.onclick = () => { 
            console.log("Pattern Selected:", p);
            state.patternId = p.id; 
            renderPatternWordsBySelection(p); 
        };
        container.appendChild(item);
      });
      pane.appendChild(container);
    } else {
      renderPatternWordsBySelection(pane);
    }
  }

  function renderPatternWordsBySelection(pane) {
    const pattern = patternsData.find(p => p.id === state.patternId);
    if (!pattern) {
        state.patternId = null;
        renderMain();
        return;
    }

    console.log("Pattern Matcher Running for:", pattern.title, "Pattern:", pattern.pattern, "Type:", pattern.type);

    const matchedWords = [];
    const gradeData = WORDS[state.currentGrade] || {};
    const allWords = [];

    // Step 1: Robust flattened word extraction
    if (Array.isArray(gradeData)) {
      allWords.push(...gradeData);
    } else if (typeof gradeData === 'object' && gradeData !== null) {
      Object.keys(gradeData).forEach(key => {
        const val = gradeData[key];
        if (Array.isArray(val)) {
          allWords.push(...val);
        } else if (typeof val === 'object' && val !== null) {
          Object.values(val).forEach(innerList => {
            if (Array.isArray(innerList)) allWords.push(...innerList);
          });
        }
      });
    }
    
    console.log(`Words extracted for grade ${state.currentGrade}: ${allWords.length}`);

    // Step 2: Advanced Matching Engine
    allWords.forEach(w => {
      let isMatch = false;
      const wStr = (w.w || "").toLowerCase().trim();
      const pStr = (pattern.pattern || "").toLowerCase().trim();

      if (pattern.type === 'suffix') {
          if (wStr.endsWith(pStr)) isMatch = true;
      } else if (pattern.type === 'prefix') {
          if (wStr.startsWith(pStr)) isMatch = true;
      } else if (pattern.type === 'phrase') {
          if (wStr.includes(pStr)) isMatch = true;
      } else if (pattern.type === 'exact') {
          if (wStr === pStr || wStr.includes(pStr)) isMatch = true;
      }
      if (isMatch) matchedWords.push(w);
    });

    console.log(`Final Match Count for ${pattern.id}: ${matchedWords.length}`);

    const container = document.createElement('div');
    container.className = 'grammar-container';
    container.style.cssText = 'display:flex; flex-direction:column; gap:15px; padding:10px 0;';
    container.innerHTML = `
      <div style="display:flex; align-items:center; gap:10px;">
        <button id="btn-back-pattern" style="background:none; border:none; font-size:20px; cursor:pointer;">⬅️</button>
        <h3 style="margin:0; color:var(--text-main)">${pattern.title} 单词列表</h3>
      </div>
      <p style="font-size:14px; color:var(--text-sub); padding-left:10px;">共找到 ${matchedWords.length} 个匹配单词</p>
    `;

    if (matchedWords.length === 0) {
        container.innerHTML += `<div class="empty-state animate-pop" style="padding:40px 0;"><p>暂无匹配单词</p><div style="font-size:30px;">🔍</div></div>`;
    } else {
        matchedWords.forEach(word => {
            const item = document.createElement('div');
            item.className = 'card animate-pop';
            item.style.cssText = 'cursor:pointer; padding:15px; display:flex; justify-content:space-between; align-items:center;';
            item.innerHTML = `<div><strong style="font-size:18px;">${word.w}</strong><div style="font-size:13px; color:var(--text-sub);">${word.p}</div></div><div style="color:var(--primary); font-size:14px;">学习 📖</div>`;
            item.onclick = () => {
                console.log("Word clicked:", word);
                state.mode = 'word';
                state.currentUnit = word.u;
                state.currentWordList = [word];
                state.currentIndex = 0;
                state.currentWord = word;
                
                // Sync UI
                $$("#btn-word").classList.add('on');
                $$("#btn-grammar").classList.remove('on');
                $$("#btn-pattern").classList.remove('on');
                $("#unit-selector").style.display = 'flex';
                renderMain();
                renderWordCard(pane);
            };
            container.appendChild(item);
        });
    }
    pane.appendChild(container);
    container.querySelector("#btn-back-pattern").onclick = () => { state.patternId = null; renderMain(); };
  }

  // --- 通用工具 ---
  function updateProgressDisplay() {
    const progressEl = $("#progress-bar");
    const dayProgressEl = $("#day-progress");
    if (progressEl) progressEl.style.width = (state.progress || 0) + '%';
    if (dayProgressEl) dayProgressEl.innerText = Math.round(state.progress || 0) + '%';
  }

})();


